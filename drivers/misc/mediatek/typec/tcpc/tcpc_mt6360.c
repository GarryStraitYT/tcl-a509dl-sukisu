...
/* Add-start by baiwei.peng for type-c plug orientation node on 2022/03/08 */
struct mt6360_chip *chip;
static ssize_t show_plug_orient(struct device *dev, struct device_attribute *attr, char *buf)
{
        int ret;
                u8 data = 0;

                ret = mt6360_i2c_read8(chip->tcpc, TCPC_V10_REG_TCPC_CTRL, &data);
        if (ret < 0)
          return ret;

        data &= 0x01;
//begin add by zhiquan.wen.hz for xr10956704 for typec nod on 20210403
        data =data+1;  //new define is 1 and 2
//end add by zhiquan.wen.hz for xr10956704 for typec nod on 20210403
        return sprintf(buf, "%u\n", data);
}

static DEVICE_ATTR(plug_orient, 0664, show_plug_orient, NULL);
/* Add-end by baiwei.peng for type-c plug orientation node on 2022/03/08 */
...
/* Modify-start by baiwei.peng for type-c plug orientation node on 2022/03/08 */
	//struct mt6360_chip *chip;
/* Modify-end by baiwei.peng for type-c plug orientation node on 2022/03/08 */
	int ret, chip_id;
	...
	//Added by baiwei.peng for type-c plug orientation node on 2022/03/08
	ret = device_create_file(chip->dev,&dev_attr_plug_orient);
