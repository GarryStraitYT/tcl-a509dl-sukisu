...
/*Begin add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
...
#endif
#endif
/*End add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
...
static int test_cc_patch(struct wusb3801_chip *chip)
{
	...
	if ( i >= 5)
		dev_err(cdev, "%s write WUSB3801_REG_TEST_02=[0x%02x] failed\n", __func__, rc_reg_08);
	i = 0;
	do{
		msleep(100);
		wusb3801_i2c_write8(chip->tcpc,
	...
	if ( i >= 5)
		dev_err(cdev, "%s write WUSB3801_REG_TEST_09=[0x%02x] failed\n", __func__, rc_reg_0f);
	//end
...
static void wusb3801_irq_work_handler(struct kthread_work *work)
{
	...
	int rc;
	//Begin add by bing-zhang for BORASPECTRUM-1334/BORATF-4871 on 2022/12/01
	#if defined(CONFIG_TCT_PROJECT_BORATF)
	int ret = -1;
	#endif
	//End add by bing-zhang for BORASPECTRUM-1334/BORATF-4871 on 2022/12/01
	int int_sts;
	uint8_t status, type;
	struct tcpc_device *tcpc;
	...
	//Begin add by bing-zhang for 11693754 on 2021/12/28
	tct_vbus_detect = ((rc >> 7) & 0x01) ? 1 : 0;//Estimate whether wusb3801 detect vbus or not
	if (tct_vbus_detect) {
/*Begin add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
#if defined(CONFIG_TCT_PROJECT_BORATF)
		//Begin modify by bing-zhang for BORASPECTRUM-1334/BORATF-4871 on 2022/12/01
		ret = i2c_smbus_write_byte_data(w_client,WUSB3801_REG_CONTROL0, 0x24);
		if (ret < 0) {
			pr_info("%s[%d] set WUSB3801_REG_CONTROL0 fail!!!, ret = %d\n",__func__,__LINE__,ret);
		}
		//End modify by bing-zhang for BORASPECTRUM-1334/BORATF-4871 on 2022/12/01
#endif
/*End add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
		printk("USB is ready!\n");
	}
	...
	case WUSB3801_TYPE_DBG_ACC:
		if (tcpc->typec_attach_new != TYPEC_ATTACHED_DEBUG) {
                                ...
                                unsigned long event, void *data)
{
    ...
    case FB_BLANK_UNBLANK:
        if (FB_EARLY_EVENT_BLANK == event) {
            ...
            rc_status = i2c_smbus_read_byte_data(w_client, WUSB3801_REG_STATUS);
	    rc_status &=~0x7C;
	    if( rc_status !=0x1  &&  rc_status !=0x2 ){
                rc = i2c_smbus_read_byte_data(w_client, WUSB3801_REG_CONTROL0);
		if (rc < 0) {
		    pr_err("%s: fail to read mode\n", __func__);
		    return rc;
		}
		pr_info("[WUSB3801] resume control0: 0x%x\n",rc);
		rc &= ~WUSB3801_MODE_MASK;
		rc &= ~WUSB3801_INT_MASK;//Disable the chip interrupt
                rc |= 0x24;
	        rc = i2c_smbus_write_byte_data(w_client,
    ...
    case FB_BLANK_POWERDOWN:
	  ...
	  rc_status = i2c_smbus_read_byte_data(w_client, WUSB3801_REG_STATUS);
	  rc_status &=~0x7C;
	  if(rc_status !=0x80 && rc_status !=0x1  &&  rc_status !=0x2 ){
              pr_info("[WUSB3801] suspend reg status =%x \n",rc_status);
	      rc = i2c_smbus_read_byte_data(w_client, WUSB3801_REG_CONTROL0);
		if (rc < 0) {
			pr_err("%s: fail to read mode\n", __func__);
			return rc;
		}
		pr_err("[WUSB3801] suspend control0: 0x%x\n",rc);
		rc &= ~WUSB3801_MODE_MASK;
		rc &= ~WUSB3801_INT_MASK;//Disable the chip interrupt
                rc |= 0x80;
                rc = i2c_smbus_write_byte_data(w_client,
		   ...
		   return rc;
            ...
            pr_err("suspend: event = %lu, not care\n", event);
        }
        break;
    default:
...
static int mt_parse_dt(struct wusb3801_chip *chip, struct device *dev)
{
	...
	if (ret < 0) {
		pr_err("%s no intr_gpio info, ret=%d\n", __func__, ret);
		return ret;
	}
	chip->irq_gpio = ret;
...
static void wusb3801_first_check_typec_work(struct work_struct *work)
{
	...
	if (status) {
		...
		case WUSB3801_TYPE_AUD_ACC:
			...
			break;
		case WUSB3801_TYPE_DBG_ACC:
			chip->tcpc->typec_attach_new = TYPEC_ATTACHED_DEBUG;
			tcpci_notify_typec_state(chip->tcpc);
			//tcpci_report_usb_port_changed(chip->tcpc);
			chip->tcpc->typec_attach_old = TYPEC_ATTACHED_DEBUG;
			break;
		default:
			pr_err("%s: Unknwon type[0x%02x]\n", __func__, type);
			break;
...
static int wusb3801_tcpcdev_init(struct wusb3801_chip *chip, struct device *dev)
{
	...
	schedule_delayed_work(&chip->first_check_typec_work, msecs_to_jiffies(35000));
	return 0;
}
				...
				const struct i2c_device_id *id)
{
	...
	if (use_dt) {
		rc = mt_parse_dt(chip, &client->dev);
		if (rc < 0)
			return rc;
	} else {
	...
	chip->irq_wake_lock = wakeup_source_register(NULL, "wusb3801_irq_wakelock");
	chip->i2c_wake_lock = wakeup_source_register(NULL, "wusb3801_i2c_wakelock");
#else
...
/*Begin add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
		...
		if (ret) {
			pr_err("[USB]Unable to register fb_notifier: %d", ret);
		}
#endif
#endif
/*End add by mingbo.feng for BORATMO-6083 on 2022/10/25*/
...
static void wusb3801_shutdown(struct i2c_client *client)
{
	...
	/* [BSP]Begin modified by bitao.xiong for BORATF-3336 on 2022/10/28 */
	wusb3801_i2c_write8(chip->tcpc,
			WUSB3801_REG_CONTROL0, 0x80); //0x00
	/* [BSP]End modified by bitao.xiong for BORATF-3336 on 2022/10/28 */
	if (chip != NULL) {
		if (chip->irq)
