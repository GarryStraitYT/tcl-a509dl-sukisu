				...
				int brightness)
{
	...
	printk("%s[%d] final set brightness to pwm:%d\n",__func__,__LINE__,brightness);
					  ...
					  enum led_brightness brightness)
{
	...
	//Begin add by bing-zhang for ENCORETF-19 on 2022/06/23
	#if defined(CONFIG_TCT_FEATURE_BACKLIGHT_MAPPING)
	#if defined(CONFIG_TCT_PROJECT_ENCORE)
	if ((1 <= brightness) && (5 >= brightness)) {
		trans_level = brightness + 2;
	}else if ((6 <= brightness) && (1548 >= brightness)) {
		trans_level = (int) (brightness/2 + 5);
	}else if ((1549 <= brightness) && (2047 >= brightness)) {
		trans_level = (int) (brightness/3 + 263);
	}else if (2047 < brightness) {
		trans_level = (int) (brightness*37/1000 + 871);
	}else {
		trans_level = 0;
	}
	#else
	//End add by bing-zhang for ENCORETF-19 on 2022/06/23
	...
	printk("%s[%d] brightness = %d trans_level = %d\n",__func__,__LINE__,brightness, trans_level);
