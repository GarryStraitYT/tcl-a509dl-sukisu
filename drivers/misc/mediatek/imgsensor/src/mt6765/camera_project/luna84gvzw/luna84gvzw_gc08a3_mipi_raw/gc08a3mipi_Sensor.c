...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = LUNA84GVZW_GC08A3_SENSOR_ID,
...
struct gc08a3_otp_t cxtvm18846_gc08a3_otp_info;
...
{
	write_cmos_sensor_8bit(0x0316, 0x00);
...
static int gc08a3_iReadData(unsigned int ui4_offset, unsigned int ui4_length, unsigned char *pinputdata)
{
	...
	i4RetValue =gc08a3_otp_read_group((u16) u4CurrentOffset, pBuff, i4ResidueDataLength);
	if (i4RetValue != 0) {
...
static bool check_sum_awb(kal_uint8 *buf, unsigned int size, kal_uint8 chksum)
{
	...
	{
		sum += buf[i];
		pr_err("awbData[%d] = 0x%x %d", i, buf[i], buf[i]);
	}

	if ((sum % 256) != chksum)
	{
		pr_err("chksum fail size = %d sum=%d sum-in-eeprom=%d", size, (sum % 256), chksum);
		return false;
	}
	return true;
}
...
static bool check_sum_af(kal_uint8 *buf, unsigned int size, kal_uint8 chksum)
{
	int i, sum = 0;
	...
	{
		sum += buf[i];
		pr_err("afData[%d] = 0x%x %d", i, buf[i], buf[i]);
	}
...
static bool check_gc08a3_otp(void)
{
	...
	unsigned int otpStatus = 0;
        ...
        memset(&cxtvm18846_gc08a3_otp_info, 0, sizeof(struct gc08a3_otp_t));
	...
	gc08a3_iReadData(MODULE_INFO_FLAG, 1, &moduleInoramtionFlag);
	if ((moduleInoramtionFlag & 0x03) == 0x01)
	{
		pr_err("[ljt]gc08a3OTP:module info group1 valid");
		otpStatus |= EEPROM_MODULEINFO_FLAG;
                gc08a3_iReadData(MODULE_INFO_FLAG + 8, sizeof(moduleInformation_struct), (unsigned char *)pModuleInfo);

	}
	else if((moduleInoramtionFlag & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:module info group2 valid");
		otpStatus |= EEPROM_MODULEINFO_FLAG;
                gc08a3_iReadData(MODULE_INFO_FLAG + (MODULE_LENGTH+1)*8, sizeof(moduleInformation_struct), (unsigned char *)pModuleInfo);
	}
	else
	{
		pr_err("[ljt]gc08a3OTP:moduleInoramtionFlag=%d", moduleInoramtionFlag);
	}

	if(otpStatus == EEPROM_MODULEINFO_FLAG)
	{
		if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			otpStatus |= EEPROM_MODULEINFO_CHKSUM;
			if (pModuleInfo->moduleID == 0x47 && pModuleInfo->LENSID == 0xC2 && pModuleInfo->VCMID == 0x51 && pModuleInfo->DriverICID == 0x1c)
			{
				otpStatus |= EEPROM_MODULEINFO_VALUE;
				pr_err("[ljt]gc08a3OTP:module info flag chksum value pass");
			}
			else
			{
				pr_err("[ljt]gc08a3OTP:moduleID =0x%x,LENSID=0x%x,VCMID=0x%x,DriverICID=0x%x",
	...
	gc08a3_iReadData(AWB_INFO_FLAG, 1, &awbFlag);
	if ((awbFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]gc08a3OTP:awb info group1 valid");

                cxtvm18846_gc08a3_otp_info.awb_flag = 0x01;
                gc08a3_iReadData(AWB_INFO_FLAG + 8, AWB_LENGTH-1, &cxtvm18846_gc08a3_otp_info.awb_param[0]);
                gc08a3_iReadData(AWB_INFO_FLAG + AWB_LENGTH*8, 1, &cxtvm18846_gc08a3_otp_info.awbChksum);
	}
	else if ((awbFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:awb info group2 valid");

                cxtvm18846_gc08a3_otp_info.awb_flag = 0x04;
                gc08a3_iReadData(AWB_INFO_FLAG + (AWB_LENGTH+1)*8, AWB_LENGTH-1, &cxtvm18846_gc08a3_otp_info.awb_param[0]);
                gc08a3_iReadData(AWB_INFO_FLAG + (AWB_LENGTH+1)*8 + (AWB_LENGTH-1)*8, 1, &cxtvm18846_gc08a3_otp_info.awbChksum);
	}
	else
	{
		pr_err("awbFlag=%d", awbFlag);
	}

	if ((cxtvm18846_gc08a3_otp_info.awb_flag == 0x01) || (cxtvm18846_gc08a3_otp_info.awb_flag == 0x04))
	{
		otpStatus |= EEPROM_AWB_FLAG;
                //pAWBInfo->awbInformation = cxtvm18846_gc08a3_otp_info.awb_param;

		if (check_sum_awb(&cxtvm18846_gc08a3_otp_info.awb_param[0], AWB_LENGTH-1, cxtvm18846_gc08a3_otp_info.awbChksum))
		{
			otpStatus |= EEPROM_AWB_CHKSUM;
			pr_err("[ljt]gc08a3OTP:awb flag chksum pass");
		}
		else
		{
			int i;
			for (i = 0; i < AWB_LENGTH-1; i++)
			     pr_err("[ljt]gc08a3OTP:awb[%d]=0x%x  %d\n", i, cxtvm18846_gc08a3_otp_info.awb_param[i], cxtvm18846_gc08a3_otp_info.awb_param[i]);
		}
	}
	//kfree(pAWBInfo);
	...
	gc08a3_iReadData(LSC_INFO_FLAG, 1, &lscFlag);
	if ((lscFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]gc08a3OTP:lsc info group1 valid");

                cxtvm18846_gc08a3_otp_info.lsc_flag = 0x01;
                gc08a3_iReadData(LSC_INFO_FLAG + 8, LSC_LENGTH-1, &cxtvm18846_gc08a3_otp_info.lsc_param[0]);
                gc08a3_iReadData(LSC_INFO_FLAG + LSC_LENGTH*8, 1, &cxtvm18846_gc08a3_otp_info.lscChksum);
	}
	else if ((lscFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:lsc info group2 valid");

                cxtvm18846_gc08a3_otp_info.lsc_flag = 0x04;
                gc08a3_iReadData(LSC_INFO_FLAG + (LSC_LENGTH+1)*8, LSC_LENGTH-1, &cxtvm18846_gc08a3_otp_info.lsc_param[0]);
                gc08a3_iReadData(LSC_INFO_FLAG + (LSC_LENGTH+1)*8+ (LSC_LENGTH-1)*8, 1, &cxtvm18846_gc08a3_otp_info.lscChksum);
	}
	else
	{
		pr_err("[ljt]gc08a3OTP:lscFlag=%d", lscFlag);
	}

	if ((cxtvm18846_gc08a3_otp_info.lsc_flag == 0x01) || (cxtvm18846_gc08a3_otp_info.lsc_flag == 0x04))
	{
		otpStatus |= EEPROM_LSC_FLAG;
		//pLSCInfo->lscData = cxtvm18846_gc08a3_otp_info.lsc_param;

		if (check_sum_lsc(&cxtvm18846_gc08a3_otp_info.lsc_param[0], LSC_LENGTH-1, cxtvm18846_gc08a3_otp_info.lscChksum))
		{
			otpStatus |= EEPROM_LSC_CHKSUM;
			pr_err("[ljt]gc08a3OTP:lsc flag chksum pass");
		}
	}
	...
	gc08a3_iReadData(AF_INFO_FLAG, 1, &afFlag);
	if ((afFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]gc08a3OTP:af info group1 valid");

                cxtvm18846_gc08a3_otp_info.af_flag = 0x01;
                gc08a3_iReadData(AF_INFO_FLAG + 8, AF_LENGTH-1, &cxtvm18846_gc08a3_otp_info.af_param[0]);
                gc08a3_iReadData(AF_INFO_FLAG + AF_LENGTH*8, 1, &cxtvm18846_gc08a3_otp_info.afChksum);
	}
	else if ((afFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:af info group2 valid");

                cxtvm18846_gc08a3_otp_info.af_flag = 0x04;
                gc08a3_iReadData(AF_INFO_FLAG + (AF_LENGTH+1)*8, AF_LENGTH-1, &cxtvm18846_gc08a3_otp_info.af_param[0]);
                gc08a3_iReadData(AF_INFO_FLAG + (AF_LENGTH+1)*8 + (AF_LENGTH-1)*8, 1, &cxtvm18846_gc08a3_otp_info.afChksum);
	}
	else
	{
		pr_err("[ljt]gc08a3OTP:afFlag=%d", afFlag);
	}

	if ((cxtvm18846_gc08a3_otp_info.af_flag == 0x01) ||(cxtvm18846_gc08a3_otp_info.af_flag == 0x04))
	{

		otpStatus |= EEPROM_AF_FLAG;
                //pAFInfo->afData = cxtvm18846_gc08a3_otp_info.af_param;
		if (check_sum_af(&cxtvm18846_gc08a3_otp_info.af_param[0], AF_LENGTH-1, cxtvm18846_gc08a3_otp_info.afChksum))
		{
			otpStatus |= EEPROM_AF_CHKSUM;
			pr_err("[ljt]gc08a3OTP:af flag chksum pass");
		}
	}
	//kfree(pAFInfo);
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(CamNameB_module_name, "GC08A3:CXT:8M:ASA8001138C1");
