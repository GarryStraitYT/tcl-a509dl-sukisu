...
static enum IMGSENSOR_RETURN mclk_release(void *pinstance)
{
    ...
    mdelay(2);
    pr_info("%s : delay release mclk!\n", __func__);
    for(i = IMGSENSOR_SENSOR_IDX_MIN_NUM;
        i < IMGSENSOR_SENSOR_IDX_MAX_NUM;
        i++) {
        if(pinst->ppinctrl_state[i][MCLK_STATE_DISABLE] != NULL && !IS_ERR(pinst->ppinctrl_state[i][MCLK_STATE_DISABLE])) {
    ...
    enum IMGSENSOR_HW_PIN_STATE pin_state)
{
    ...
    if(pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_0 || pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH) {
        ret = IMGSENSOR_RETURN_ERROR;
    } else {
        ...
        if (state_index == MCLK_STATE_DISABLE) {
            mdelay(2);
            pr_info("%s : delay release mclk!\n", __func__);
        }
