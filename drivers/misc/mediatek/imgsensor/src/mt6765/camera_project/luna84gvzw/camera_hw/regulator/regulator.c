...
//end 20200411 liujunting add for camera sensor regulator
    ...
    pr_info("[%s] sensor_idx:%d, pin:%d, pin_state:%d, offset:%d", __func__, sensor_idx, pin, pin_state, reg_type_offset + pin - IMGSENSOR_HW_PIN_AVDD);
    if (pregulator) {
...
//end 20200411 liujunting add for camera sensor regulator
        ...
        pr_err("regulator == NULL offset:%d pin:%d IMGSENSOR_HW_PIN_AVDD:%d\n",
               reg_type_offset,
               pin,
