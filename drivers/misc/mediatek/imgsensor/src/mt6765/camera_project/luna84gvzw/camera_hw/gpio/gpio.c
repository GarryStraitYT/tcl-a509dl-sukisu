...
#if 0
#define FUNC_ENTER() pr_info("[%s] E", __func__)
#define FUNC_EXIT() pr_info("[%s] X", __func__)
#else
#define FUNC_ENTER()
#define FUNC_EXIT()
#endif
...
static enum IMGSENSOR_RETURN gpio_release(void *pinstance)
{
    ...
    for(j = IMGSENSOR_SENSOR_IDX_MIN_NUM;
        ...
        for(i = GPIO_CTRL_STATE_PDN_L;
            ...
            if(lookup_names != NULL && pgpio->ppinctrl_state_cam[j][i] != NULL && !IS_ERR(pgpio->ppinctrl_state_cam[j][i]) && pinctrl_select_state(pgpio->ppinctrl, pgpio->ppinctrl_state_cam[j][i])) {
                pr_info(
...
static enum IMGSENSOR_RETURN gpio_init(void *pinstance)
{
    ...
    for(j = IMGSENSOR_SENSOR_IDX_MIN_NUM;
        ...
        for(i = 0; i < GPIO_CTRL_STATE_MAX_NUM_CAM; i++) {
            ...
            if(lookup_names) {
                ...
                if(pgpio->ppinctrl_state_cam[j][i] == NULL || IS_ERR(pgpio->ppinctrl_state_cam[j][i])) {
                    pr_info(
                        "%s : pinctrl err, %s\n",
                        __func__,
                        str_pinctrl_name);
    ...
    for(i = 0; i < GPIO_CTRL_STATE_MAX_NUM_SWITCH; i++) {
        ...
        if(pgpio->ppinctrl_state_switch[i] == NULL || IS_ERR(pgpio->ppinctrl_state_switch[i])) {
            pr_info(
                "%s : pinctrl err, %s\n",
                __func__,
                gpio_pinctrl_list_switch[i].ppinctrl_lookup_names);
    ...
    enum IMGSENSOR_HW_PIN_STATE pin_state)
{
...
#endif
       pin_state < IMGSENSOR_HW_PIN_STATE_LEVEL_0 || pin_state > IMGSENSOR_HW_PIN_STATE_LEVEL_HIGH)
        return IMGSENSOR_RETURN_ERROR;
    ...
    if(pin == IMGSENSOR_HW_PIN_MIPI_SWITCH_EN)
        ppinctrl_state = pgpio->ppinctrl_state_switch[GPIO_CTRL_STATE_MIPI_SWITCH_EN_H + gpio_state];

    else if(pin == IMGSENSOR_HW_PIN_MIPI_SWITCH_SEL)
        ppinctrl_state = pgpio->ppinctrl_state_switch[GPIO_CTRL_STATE_MIPI_SWITCH_SEL_H + gpio_state];

    else
#endif
    {
        ppinctrl_state =
            pgpio->ppinctrl_state_cam[(unsigned int) sensor_idx][((pin - IMGSENSOR_HW_PIN_PDN) << 1) + gpio_state];
    }
    ...
    if(ppinctrl_state == NULL || IS_ERR(ppinctrl_state) || pinctrl_select_state(pgpio->ppinctrl, ppinctrl_state))
        pr_info(
            ...
            __func__,
            pin, pin_state);
    pr_info("[%s] sensor_idx:%d, pin:%d, pin_state:%d", __func__, sensor_idx, pin, pin_state);
    mutex_unlock(&pinctrl_mutex);
