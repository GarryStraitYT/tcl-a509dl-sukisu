...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = LUNA84GVZW_HI846_SENSOR_ID,
...
static bool check_hi846_otp(void)
{
    ...
    if(otpStatus == EEPROM_MODULEINFO_FLAG)
    {
        if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
        {
            ...
            if (pModuleInfo->moduleID == 0x48 && pModuleInfo->LENSID == 0xB2 && pModuleInfo->VCMID == 0xE3 && pModuleInfo->DriverICID == 0x1B)
            {
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                strcpy(CamNameB_module_name, "HI846:TSP:8M:ASA8001139C1");
