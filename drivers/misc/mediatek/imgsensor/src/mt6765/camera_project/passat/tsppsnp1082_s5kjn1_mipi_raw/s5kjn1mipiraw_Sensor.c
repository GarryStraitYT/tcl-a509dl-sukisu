...
#define LOG_INF(format, args...)                                               \
	pr_err(PFX "[JW][%s] " format, __func__, ##args)
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = TSPPSNP1082_S5KJN1_SENSOR_ID,
...
static kal_uint16 hwggc_table_write_cmos_sensor(kal_uint16 uw_addr,kal_uint16 *para, kal_uint32 len)
{
	...
	while (len > IDX) {
		...
		/* Write when remain buffer size is less than 4 bytes or reach end of data */
		if ((I2C_BUFFER_LEN - tosend) < 4 || IDX == len /*||addr != addr_last*/) {
			LOG_INF("mmmi2c_write_id end[0x%x] Addr[0x%x] Data[0x%x] addr_last[0x%x], tosend[%d], IDX[%d], len[%d]\n",
				imgsensor.i2c_write_id, addr, data, addr_last,tosend, IDX, len);
...
static void sensor_init(void)
{
	...
	if(hwggc.hwGGCFlag){
		//int i;
		kal_uint16 *phwGGCData = (kal_uint16 *)&hwggc.hwGGCData[0];

		write_cmos_sensor(0x6028, 0x2400);
		write_cmos_sensor(0x602A, 0x0CFC);
	/* Begin binchang.liang optimize ggc write reg operation 2021/11/18 */
		//for(i = 0; i < sizeof(hwggc.hwGGCData)/2 ; i++)
			//write_cmos_sensor(0x6F12, phwGGCData[i]);
		hwggc_table_write_cmos_sensor(0x6F12,phwGGCData,sizeof(hwggc.hwGGCData)/2);	
	/* End binchang.liang optimize ggc write reg operation 2021/11/18 */
	}
...
static kal_uint32 return_sensor_id(void)
{
	...
	if(physicalID == 0x38e1 && (get_tsppsnp1082_s5kjn1_lensID() == 0x01))
	{
		return physicalID;
	}
	return 0;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
    ...
    "S5KJN11082:TSP:50M:ASA5000000C1",
