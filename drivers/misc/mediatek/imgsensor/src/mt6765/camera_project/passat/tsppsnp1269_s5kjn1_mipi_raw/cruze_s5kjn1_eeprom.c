...
BYTE get_tsppsnp1269_s5kjn1_lensID(void)
{
	...
	kfree(pModuleInfo);
	pr_err("get_tsppsnp1269_s5kjn1_lensID retVal = %d", retVal);
	return retVal;
}
/*Begin junyiliu for [Task][PASSAT-1876][passat MINI otp display] */
#ifdef TARGET_BUILD_MMITEST
extern char CamOTPB_module_name[256];
void tsppsnp1269_get_opt_info(void)
{
	af_struct  *pAFInfo;
	UINT16 macro;
	UINT16 inifi;
	...
	if (pAFInfo->afFlag == 0x01)
	{
		...
		{
			macro=(pAFInfo->afData[0]<<8|pAFInfo->afData[1]);
			inifi=(pAFInfo->afData[2]<<8|pAFInfo->afData[3]);
			sprintf(CamOTPB_module_name,"1:macro=%d:macrorange={%d,%d}",macro, macro-100,macro+100);
			sprintf(CamOTPB_module_name+strlen(CamOTPB_module_name),":inifi=%d:inifirange={%d,%d}\n",inifi,inifi-100,inifi+100);
		}
		else
		{
			pr_err("error  check_sum_ af ");
			sprintf(CamOTPB_module_name,"1:af_read_error");
		}
	}
	else
	{
...
#endif
/*End junyiliu for [Task][PASSAT-1876][passat MINI otp display] */

bool check_tsppsnp1269_s5kjn1_otp(void)
{
    moduleInformation_struct *pModuleInfo;
    awb_struct *pAWBInfo;
    lsc_struct *pLSCInfo;
	...
	read_tsp_s5kjn1_eeprom(HWGGC_START_ADDR, (BYTE *)&tsnp1269_hwggc, sizeof(hwggc_struct));
	if (tsnp1269_hwggc.hwGGCFlag == 0x01)
	{
		otpStatus |= EEPROM_HWGGC_FLAG;
		if (check_sum((BYTE *)&tsnp1269_hwggc, sizeof(hwggc_struct)))
		{
			otpStatus |= EEPROM_HWGGC_CHKSUM;
			pr_info("tsnp1269_hwggc  chksum pass");
		}
		else
		{
			pr_err(" error  check_sum_ tsnp1269_hwggc ");
		}
	}
	else
	{
		pr_err("error pdafFlag=%d", tsnp1269_hwggc.hwGGCFlag);
	}
