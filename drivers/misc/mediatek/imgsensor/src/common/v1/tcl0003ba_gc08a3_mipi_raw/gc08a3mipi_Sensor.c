...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = TCL0003BA_GC08A3_SENSOR_ID,
...
//
/*Begin junyiliu for passat bf821b_08a3otp check  20211123*/
extern struct gc08a3_otp_t gc08a3_otp_info;
static int otp_enable_log =0;
...
bool check_gc08a3_otp(void)
{
	...
	{
		pr_err("gc08a3OTP:module info group1 valid");
	...
	if(otpStatus == EEPROM_MODULEINFO_FLAG)
	{
		if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID == 0x51 && pModuleInfo->LENSID == 0xc2 && pModuleInfo->VCMID == 0xd2 && pModuleInfo->DriverICID == 0x1b)
			{
				otpStatus |= EEPROM_MODULEINFO_VALUE;
				pr_err("gc08a3OTP:module info flag chksum value pass");
			}
			else
			{
				pr_err("gc08a3OTP:moduleID =0x%x,LENSID=0x%x,VCMID=0x%x,DriverICID=0x%x",
	...
	{
		pr_err("gc08a3OTP:awb info group1 valid");
        otp_enable_log=1;
	...
	{
		pr_err("gc08a3OTP:awb info group2 valid");
        otp_enable_log=1;
	...
	{
		pr_err("gc08a3OTP:awbFlag=%d", awbFlag);
	}
	...
	{
		pr_err("gc08a3OTP:lsc info group1 valid");
	...
	{
		pr_err("gc08a3OTP:lsc info group2 valid");
	...
	{
		pr_err("gc08a3OTP:lscFlag=%d", lscFlag);
	}
	...
	if ((gc08a3_otp_info.lsc_flag == 0x01) || (gc08a3_otp_info.lsc_flag == 0x04))
	{
		...
		if (check_sum_lsc(&gc08a3_otp_info.lsc_param[0], LSC_LENGTH-1, gc08a3_otp_info.lscChksum))
		{
			...
			pr_err("gc08a3OTP:lsc flag chksum pass");
		}
	}
	...
	if ((afFlag  & 0x03) == 0x01)
	{
		pr_err("gc08a3OTP:af info group1 valid");
                otp_enable_log=1;
	...
	{
		pr_err("gc08a3OTP:af info group2 valid");
                otp_enable_log=1;
	...
	{
		pr_err("gc08a3OTP:afFlag=%d", afFlag);
	}
	...
	if ((gc08a3_otp_info.af_flag == 0x01) ||(gc08a3_otp_info.af_flag == 0x04))
	{
		...
		if (check_sum_af(&gc08a3_otp_info.af_param[0], AF_LENGTH-1, gc08a3_otp_info.afChksum))
		{
			...
			pr_err("gc08a3OTP:af flag chksum pass");
		}
	}
	...
	{
        pr_info("gc08a3OTP:otp check all  ok !!!!  otpStatus=0x%x", otpStatus);
		return true;
	}
	else
	{
		pr_err("gc08a3OTP:otp check fail otpStatus=0x%x", otpStatus);
		return false;
	}
}
/*end 20210805 ljt add for otp check*/
...
static kal_uint32 return_sensor_id(void)
{
    ...
    sensor_id = (read_cmos_sensor(0x03f0) << 8) | read_cmos_sensor(0x03f1) + 4;
    return sensor_id;
}
...
kal_uint16 gc08a3_1280x720_addr_data[] = {
    /*system*/
    0x031c,
    0x60,
    0x0337,
