...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = TSPPH8A1327_HI846_SENSOR_ID,
...
LOG_INF("HI846 addr = 0x%x \n", addr);
	if (addr == 0) {
		...
		HI846_otp.F_num_id = OTP_read_cmos_sensor(addr + 9);
		HI846_otp.infocheck = OTP_read_cmos_sensor(addr + 16);
	}
	checksum = (HI846_otp.module_integrator_id + HI846_otp.AF_FLAG + HI846_otp.prodyction_year + HI846_otp.production_month + HI846_otp.production_day + HI846_otp.sensor_id + HI846_otp.lens_id + HI846_otp.vcm_id + HI846_otp.Driver_ic_id + HI846_otp.F_num_id) % 0xFF + 1;
...
LOG_INF("HI846 module_integrator_id = 0x%x, AF_FLAG = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, vcm_id = 0x%x, Driver_ic_id = 0x%x, F_num_id, 0x%x, infocheck = 0x%x \n",\
		HI846_otp.module_integrator_id,HI846_otp.AF_FLAG, HI846_otp.prodyction_year, HI846_otp.production_month, HI846_otp.production_day, HI846_otp.sensor_id, HI846_otp.lens_id,HI846_otp.vcm_id, HI846_otp.Driver_ic_id, HI846_otp.F_num_id, HI846_otp.infocheck);
...
LOG_INF("HI846 wb_start_addr = 0x%x \n", wb_start_addr);
	...
	checksum = checksum % 0xFF + 1;
        ...
        if (checksum == HI846_otp.wb_data[29])
	{
	...
	checksum = checksum % 0xFF + 1;
        ...
        if (checksum == HI846_otp.af_data[4])
	{
	...
	if (lsc_start_addr != 0) {
		for (i = 0; i < 867; i++) {
			...
			LOG_INF("HI846 lsc_data[%d] = 0x%x  ", i, HI846_otp.lsc_data[i]);
		}
	}
        checksum = 0;
	for(i=0;i<866;i++)
	...
	checksum = checksum % 0xFF + 1;
        ...
        if (checksum == HI846_otp.lsc_data[866])
	{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
			        ...
			        if(HI846_otp_cali())
			        {
			              LOG_INF("[ljt]hi846,check OTP pass\n");
