...
#define LSC_INFO_START_ADDR 0x0708
...
static bool check_sum_pdaf(pdaf_struct *buf, unsigned int size)
{
    ...
    if((sum1 % 256) != buf->chksum1 || (sum2 % 256) != buf->chksum2) {
        pr_err("chksum fail size = %d sum1=%d sum1-in-eeprom=%d sum2=%d sum2-in-eeprom=%d", size, (sum1 % 256), buf->chksum1, (sum2 % 256 + 1), buf->chksum2);
        return false;
    }
    return true;
}
...
bool check_hi1336_otp(void)
{
    ...
    if(otpStatus == (EEPROM_MODULEINFO_CHKSUM | EEPROM_MODULEINFO_FLAG | EEPROM_MODULEINFO_VALUE | EEPROM_AWB_CHKSUM | EEPROM_AWB_FLAG | EEPROM_LSC_CHKSUM | EEPROM_LSC_FLAG )) {
        pr_err("otp check pass otpStatus=0x%x", otpStatus);
        return true;
    } else {
        pr_err("otp check fail 1otpStatus=0x%x", otpStatus);
        return false;
    }
}
