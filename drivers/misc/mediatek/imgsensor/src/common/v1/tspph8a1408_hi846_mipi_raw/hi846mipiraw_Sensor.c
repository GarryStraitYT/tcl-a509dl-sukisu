...
#define PFX "hi846new_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = TSPPH8A1408_HI846_SENSOR_ID,
...
static kal_int16 read_cmos_sensor_1byte(kal_uint16 addr, kal_uint8 *data)
{
    kal_uint16 get_byte = 0;
	char pu_send_cmd[2] = {(char)(addr >> 8), (char)(addr & 0xFF) };
...
static kal_uint32 return_sensor_id(void)
{
	...
	physicalID =(read_cmos_sensor(0x0F17) << 8) | read_cmos_sensor(0x0F16);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x000E, 0x0103);
...
static kal_int16 OTP_read_cmos_sensor_continuous(kal_uint16 addr, kal_uint16 length, kal_uint8 *data)
{
    kal_uint8 *pData;
	kal_uint16 otp_addr;
	int i;
    ...
    for(i = 0; i<length; i++)
	{
	    ...
	    //mDELAY(10);
	    read_cmos_sensor_1byte(0x708, pData); //OTP data read
...
extern struct hi846_otp_t hi846_otp_info;
...
static bool check_sum(kal_uint8 *buf, unsigned int size)
{
	...
	{
		sum += buf[i];
		pr_debug("buf[%d] = 0x%x %d", i, buf[i], buf[i]);
	}
...
static bool check_hi846_otp(void)
{
    ...
    memset(&hi846_otp_info, 0, sizeof(struct hi846_otp_t));

    //module info
    pModuleInfo = kmalloc(sizeof(moduleInformation_struct), GFP_KERNEL);

	OTP_read_cmos_sensor_continuous(MODULE_INFO_FLAG, 1, &moduleInoramtionFlag);
	if ((moduleInoramtionFlag & 0x03) == 0x01)
	{
		pr_err("[ljt]hi846OTP:module info group1 valid");
		otpStatus |= EEPROM_MODULEINFO_FLAG;
        OTP_read_cmos_sensor_continuous(MODULE_INFO_FLAG + 1, sizeof(moduleInformation_struct), (unsigned char *)pModuleInfo);

	}
	else if((moduleInoramtionFlag & 0x0C) == 0x04)
	{
		pr_err("[ljt]hi846OTP:module info group2 valid");
		otpStatus |= EEPROM_MODULEINFO_FLAG;
        OTP_read_cmos_sensor_continuous(MODULE_INFO_FLAG + MODULE_LENGTH  + 1, sizeof(moduleInformation_struct), (unsigned char *)pModuleInfo);
	}
	else
	{
		pr_err("[ljt]hi846OTP:moduleInoramtionFlag=%d", moduleInoramtionFlag);
	}
	...
	if(otpStatus == EEPROM_MODULEINFO_FLAG)
	{
		if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID == 0x48 && pModuleInfo->LENSID == 0xB2 && pModuleInfo->VCMID == 0xE1 && pModuleInfo->DriverICID == 0x1B)
			{
				otpStatus |= EEPROM_MODULEINFO_VALUE;
				pr_err("[ljt]hi846OTP:module info flag chksum value pass");
			}
			else
			{
				pr_err("[ljt]hi846OTP:moduleID =0x%x,LENSID=0x%x,VCMID=0x%x,DriverICID=0x%x",
	...
	OTP_read_cmos_sensor_continuous(AWB_INFO_FLAG, 1, &awbFlag);
	if ((awbFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]hi846OTP:awb info group1 valid");

        hi846_otp_info.awb_flag = 0x01;
        OTP_read_cmos_sensor_continuous(AWB_INFO_FLAG + 1, AWB_LENGTH, &hi846_otp_info.awb_param[0]);
	}
	else if ((awbFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]hi846OTP:awb info group2 valid");

        hi846_otp_info.awb_flag = 0x04;
        OTP_read_cmos_sensor_continuous(AWB_INFO_FLAG + AWB_LENGTH + 1, AWB_LENGTH, &hi846_otp_info.awb_param[0]);
	}
	else
	{
	...
	if ((hi846_otp_info.awb_flag == 0x01) || (hi846_otp_info.awb_flag == 0x04))
	{
        ...
        //pAWBInfo->awbInformation = hi846_otp_info.awb_param;

		if (check_sum((kal_uint8 *)&hi846_otp_info.awb_param[0], AWB_LENGTH))
        {
			otpStatus |= EEPROM_AWB_CHKSUM;
			pr_err("[ljt]hi846OTP:awb flag chksum pass");
		}
		else
		{
			int i;
			for (i = 0; i < AWB_LENGTH; i++)
			     pr_err("[ljt]hi846OTP:awb[%d]=0x%x\n", i, hi846_otp_info.awb_param[i]);
		}
	}
	//kfree(pAWBInfo);
	...
	OTP_read_cmos_sensor_continuous(LSC_INFO_FLAG, 1, &lscFlag);
	if ((lscFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]hi846OTP:lsc info group1 valid");

        hi846_otp_info.lsc_flag = 0x01;
        OTP_read_cmos_sensor_continuous(LSC_INFO_FLAG + 1, LSC_LENGTH, &hi846_otp_info.lsc_param[0]);
	}
	else if ((lscFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]hi846OTP:lsc info group2 valid");

        hi846_otp_info.lsc_flag = 0x04;
        OTP_read_cmos_sensor_continuous(LSC_INFO_FLAG + LSC_LENGTH+1, LSC_LENGTH, &hi846_otp_info.lsc_param[0]);
	}
	else
	{
		pr_err("[ljt]hi846OTP:lscFlag=%d", lscFlag);
	}

	if ((hi846_otp_info.lsc_flag == 0x01) || (hi846_otp_info.lsc_flag == 0x04))
	{
		otpStatus |= EEPROM_LSC_FLAG;
		//pLSCInfo->lscData = hi846_otp_info.lsc_param;

		if (check_sum(&hi846_otp_info.lsc_param[0], LSC_LENGTH))
		{
			otpStatus |= EEPROM_LSC_CHKSUM;
			pr_err("[ljt]hi846OTP:lsc flag chksum pass");
		}
	}
	...
	OTP_read_cmos_sensor_continuous(AF_INFO_FLAG, 1, &afFlag);
	if ((afFlag  & 0x03) == 0x01)
	{
		pr_err("[ljt]hi846OTP:af info group1 valid");

        hi846_otp_info.af_flag = 0x01;
        OTP_read_cmos_sensor_continuous(AF_INFO_FLAG + 1, AF_LENGTH, &hi846_otp_info.af_param[0]);
	}
	else if ((afFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]hi846OTP:af info group2 valid");

        hi846_otp_info.af_flag = 0x04;
        OTP_read_cmos_sensor_continuous(AF_INFO_FLAG + AF_LENGTH + 1, AF_LENGTH, &hi846_otp_info.af_param[0]);
	}
	else
	{
		pr_err("[ljt]hi846OTP:afFlag=%d", afFlag);
	}
	...
	if ((hi846_otp_info.af_flag == 0x01) ||(hi846_otp_info.af_flag == 0x04))
	{
		...
		otpStatus |= EEPROM_AF_FLAG;
                //pAFInfo->afData = hi846_otp_info.af_param;
		if (check_sum(&hi846_otp_info.af_param[0], AF_LENGTH))
		{
			otpStatus |= EEPROM_AF_CHKSUM;
			pr_err("[ljt]hi846OTP:af flag chksum pass");
		}
	}
	//kfree(pAFInfo);
...
static bool HI846_otp_cali(void)
{
    ...
    if(check_hi846_otp()){
        pr_err("TCT 2P check_hi846_otp =1 ");
        HI846_OTP_uninit();
        return true;
	} else {
        pr_err("TCT 2P check_hi846_otp =0  ");
        HI846_OTP_uninit();
        return false;
	}
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
    //begin 20211215 ljt add for kona camera lens pwdn mode
    #ifdef CONFIG_MTK_KONA_CAMERA
    char pu_send_cmd[4] = {(char)(0x00 >> 8), (char)(0x00 & 0xFF), (char)(0x01 & 0xFF)};
    #endif
    //end 20211215 ljt add for kona camera lens pwdn mode
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
			        ...
			        if(HI846_otp_cali())
			        {
			              printk("[ljt]hi846,check OTP pass\n");
				          *sensor_id |= 0x01000000;
			        }else{
			              printk("[ljt]hi846,check OTP fail\n");
                          *sensor_id = 0xFFFFFFFF;
                        //   return ERROR_SENSOR_CONNECT_FAIL;
                    }
                //begin 20211215 ljt add for kona camera lens pwdn mode
                #ifdef CONFIG_MTK_KONA_CAMERA
	            iWriteRegI2C(pu_send_cmd, 3, 0x18);
                #endif
                //end 20211215 ljt add for kona camera lens pwdn mode
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		printk("Read id fail,sensor id: 0x%x\n", *sensor_id);
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
	return ERROR_NONE;
