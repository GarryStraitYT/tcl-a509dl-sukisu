...
//#define AF_INFO_START_ADDR 0x078e
//#define PDAF_INFO_START_ADDR 0x0799
...
//begin 20200311 liujunting add for milan otp log info
//end 20200311 liujunting add for milan otp log info

#define otp_log pr_err

static bool read_s5k3l6_eeprom(kal_uint16 addr, BYTE *data, int size)
{
    int i = 0;
    int offset = addr;
    int ret;
...
static void dump_data(void)
{
	...
	read_s5k3l6_eeprom(0x0, buf, DATA_SIZE);
...
//begin 20200311 liujunting modify for milan otp log info
static bool check_sum_awb(awb_struct *buf, unsigned int size)
{
    ...
    otp_log("awb_flag = 0x%x", buf->awbFlag);

    /*ratio[0] = buf->awbInformation[0];
        ratio[1] = buf->awbInformation[1];
	otp_log("R_Gr_ratio=0x%x",*(int*)(ratio));

        ratio[0] = buf->awbInformation[2];
        ratio[1] = buf->awbInformation[3];
        otp_log("RB_Gr_ratio=0x%x",*(int*)(ratio));

        ratio[0] = buf->awbInformation[6];
        ratio[1] = buf->awbInformation[7];
	otp_log("g_R_Gr_ratio=0x%x",*(int*)(ratio));

        ratio[0] = buf->awbInformation[8];
        ratio[1] = buf->awbInformation[9];
	otp_log("g_B_Gr_ratio=0x%x",*(int*)(ratio));*/

    otp_log("R_Gr_ratio=0x%x, B_Gr_ratio=0x%x", ((buf->awbInformation[1] << 8) + (buf->awbInformation[0])), ((buf->awbInformation[3] << 8) + (buf->awbInformation[2])));
    otp_log("g_R_Gr_ratio=0x%x, g_B_Gr_ratio=0x%x", ((buf->awbInformation[7] << 8) + (buf->awbInformation[6])), ((buf->awbInformation[9] << 8) + (buf->awbInformation[8])));
    otp_log("awb_checksum = 0x%x, real awb_checksum = 0x%x", buf->chksum, (sum % 255 + 1));
...
static bool check_sum_lsc(lsc_struct *buf, unsigned int size)
{
    ...
    for(i = 0; i < sizeof(buf->lscData); i++) {
        sum += buf->lscData[i];
        otp_log("lsc_Data[%d] = 0x%x", i, buf->lscData[i]);
    }
    ...
    otp_log("lsc_checksum = 0x%x, real lsc_checksum = 0x%x", buf->chksum, (sum % 255 + 1));
    return true;
}
//end 20200311 liujunting modify for milan otp log info
...
static bool check_sum_pdaf(pdaf_struct *buf, unsigned int size)
{
	...
	{
		pr_err("chksum fail size = %d sum1=%d sum1-in-eeprom=%d ", size, (sum1 % 256), buf->chksum1);
		return false;
	}
	...
	{
		pr_err("chksum fail size = %d sum2=%d sum2-in-eeprom=%d", size, (sum2 % 256), buf->chksum2);
		return false;
	}
...
bool check_tsp_s5k3l6_otp(void)
{
    ...
    read_s5k3l6_eeprom(MODULE_INFO_START_ADDR, (BYTE *) pModuleInfo, sizeof(moduleInformation_struct));
    if(pModuleInfo->moduleInoramtionFlag == 0x01) {
        ...
        if(check_sum((BYTE *) pModuleInfo, sizeof(moduleInformation_struct))) {
            ...
            if(pModuleInfo->moduleID == 0x48 && pModuleInfo->LENSID == 0x82 && pModuleInfo->VCMID == 0x00 && pModuleInfo->DriverICID == 0x00) {
    ...
    read_s5k3l6_eeprom(AWB_INFO_START_ADDR, (BYTE *) pAWBInfo, sizeof(awb_struct));
    ...
    read_s5k3l6_eeprom(LSC_INFO_START_ADDR, (BYTE *) pLSCInfo, sizeof(lsc_struct));
	...
	read_s5k3l6_eeprom(AF_INFO_START_ADDR, (BYTE *)pAFInfo, sizeof(af_struct));
	...
	read_s5k3l6_eeprom(PDAF_INFO_START_ADDR, (BYTE *)pPDAFInfo, sizeof(pdaf_struct));
    ...
    if(otpStatus == (EEPROM_MODULEINFO_FLAG | EEPROM_AWB_CHKSUM | EEPROM_AWB_FLAG |
