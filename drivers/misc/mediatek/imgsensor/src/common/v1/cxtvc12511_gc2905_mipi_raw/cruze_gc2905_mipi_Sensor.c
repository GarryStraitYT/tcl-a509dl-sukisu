...
#include "cruze_gc2905_mipi_Sensor.h"

/************************** Modify Following Strings for Debug **************************/
#define PFX "gc2905_camera_sensor"
#define LOG_1 LOG_INF("GC2905, MONO MIPI 1LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = CXTVC12511_GC2905_SENSOR_ID,  	/* record sensor id defined in Kd_imgsensor.h */
    .checksum_value = 0xf7375923,       	/* checksum value for Camera Auto Test */
#if defined(BINNING_SIZE)
	.pre = {
		.pclk = 42000000,					/* record different mode's pclk */
		.linelength = 1092,					/* record different mode's linelength */
	...
	.pre = {
		...
		.linelength = 2184, 				/* record different mode's linelength */
		.framelength = 1248,				/* record different mode's framelength */
		.startx = 0,						/* record different mode's startx of grabwindow */
		.starty = 0,						/* record different mode's starty of grabwindow */
	...
	.margin = 16,						/* sensor framelength & shutter margin maybe try 16 */
	.min_shutter = 4,					/* min shutter */
	.max_frame_length = 4800,			/* max framelength by sensor register's limitation */
...
#if defined(BINNING_SIZE)
	{ 1600, 1200, 0, 0, 1600, 1200, 800, 600, 0, 0, 800, 600, 0, 0, 800, 600}, /* preview */
#else
...
static void set_dummy(void)
{
	...
	if (BorF == 0)
		frame_length = imgsensor.frame_length >> 2 << 2;
	else
	...
	LOG_INF("BorF = %d(0: B, 1: F), frame_length = %d\n", BorF, frame_length);
}
...
static void set_shutter(kal_uint16 shutter)
{
	...
	LOG_INF("realtime_fps = %d, cal_shutter = %d, framelength = %d\n",
		realtime_fps, cal_shutter, imgsensor.frame_length);
} 
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	static kal_uint16 GC2905_AGC_Param[MAX_GAIN_INDEX][2] = {
		{ 256, 0x00},
		{ 303, 0x10},
		{ 358, 0x01},
		{ 424, 0x11},
	...
	for (gain_index = MAX_GAIN_INDEX - 1; gain_index >= 0; gain_index--) {
		if(reg_gain >= GC2905_AGC_Param[gain_index][0])
			break;	
	}
	LOG_INF("gain_index = %d, reg_gain = %d", gain_index, reg_gain);

	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb3, GC2905_AGC_Param[gain_index][1]);
	temp_gain = reg_gain * Dgain_ratio / GC2905_AGC_Param[gain_index][0];
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xd2, 0x41);
	write_cmos_sensor(0xd3, 0xfb);
	write_cmos_sensor(0xe0, 0xe0);
...
#if defined(SENSOR_ISP_ON)
	write_cmos_sensor(0x60, 0x00); /* OB = 0 */
	write_cmos_sensor(0x88, 0x20); /* GAMMA & LSC ON */
#else
	write_cmos_sensor(0x60, 0x40); /* OB = 64 */
	write_cmos_sensor(0x88, 0x28); /* GAMMA & LSC OFF */
#endif
	write_cmos_sensor(0x89, 0x03);
	
	/* gain */
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xb0, 0x5b);
	write_cmos_sensor(0xb1, 0x01);
	write_cmos_sensor(0xb2, 0x00);
	write_cmos_sensor(0xb6, 0xc0);
	write_cmos_sensor(0xb3, 0x00);
	write_cmos_sensor(0xfe, 0x04);
	write_cmos_sensor(0x14, 0x75);
	write_cmos_sensor(0x15, 0x75);
	write_cmos_sensor(0x16, 0x75);
	write_cmos_sensor(0x17, 0x75);
	
	/* LSC & Gamma */
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0x01, 0x80);
	write_cmos_sensor(0x04, 0x98);
	write_cmos_sensor(0x05, 0xca);
	write_cmos_sensor(0x0c, 0x01);
	write_cmos_sensor(0x20, 0x02);
	write_cmos_sensor(0x21, 0x0e);
	write_cmos_sensor(0x22, 0x1c);
	write_cmos_sensor(0x23, 0x34);
	write_cmos_sensor(0x24, 0x5a);
	write_cmos_sensor(0x25, 0x7b);
	write_cmos_sensor(0x26, 0x95);
	write_cmos_sensor(0x27, 0xae);
	write_cmos_sensor(0x28, 0xc4);
	write_cmos_sensor(0x29, 0xd9);
	write_cmos_sensor(0x2a, 0xec);
	write_cmos_sensor(0x2b, 0xf7);
	write_cmos_sensor(0x2c, 0xfd);
	...
	/* slave mode */
	/*write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x7f, 0x09);
	write_cmos_sensor(0x82, 0x0e);
	write_cmos_sensor(0x83, 0x84);
	write_cmos_sensor(0x84, 0x80);
	write_cmos_sensor(0x86, 0x02);
	write_cmos_sensor(0x87, 0x19);
	write_cmos_sensor(0x89, 0x00);
	write_cmos_sensor(0x8b, 0x80);
	write_cmos_sensor(0x85, 0x51);*/
	/* master mode */
	/*write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x7f, 0x09);
	write_cmos_sensor(0x83, 0x04);
	write_cmos_sensor(0x84, 0x80);
...
static void binning_setting(void)
{
	...
	write_cmos_sensor(0x53, 0xf0);
	write_cmos_sensor(0xd0, 0xbf);
	write_cmos_sensor(0xd3, 0xdb);
	...
	/* LSC & Gamma */
	write_cmos_sensor(0xfe, 0x01);
	write_cmos_sensor(0x04, 0x4c);
	write_cmos_sensor(0x05, 0x65);
...
//begin  add for prague dual cam
static void set_shutter_frame_length(kal_uint16 shutter, kal_uint16 target_frame_length)
{

	//pr_err("gc02m1b 8.5 frame sync shutter: %d, target_frame_length: %d", shutter, target_frame_length);
	spin_lock(&imgsensor_drv_lock);
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
    ...
    "GC2905:SHN:2M:ASA2001238C1",
    2,//main:0 front:1 sub:2 
    {
		{PDN,Vol_Low, 0},
		{DOVDD, Vol_1800, 1},
		{DVDD, Vol_1200, 1},
