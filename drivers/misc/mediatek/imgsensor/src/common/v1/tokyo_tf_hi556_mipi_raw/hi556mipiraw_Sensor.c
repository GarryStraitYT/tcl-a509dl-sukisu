...
static void read_hi556_otp(void)
{
	...
	pr_err("otp_flag=0x%x\n",otp_flag);
	if (otp_flag == 0x01)
		start_addr = 0x0401;
	else if((otp_flag&0x0c)==0x04)
		start_addr = 0x0411;
	else if((otp_flag&0xf0)==0x10)
		start_addr = 0x0421;
	else
		pr_err("no OTP data");
	pr_err("start_addr=0x%x\n",start_addr);
...
//		sensor_id_otp = data[5];
	...
	if (awb_flag == 0x01)
		...
		pr_err("no AWB OTP data");
	pr_err("awb_start_addr=0x%x\n",awb_start_addr);
...
//		gGbGr = ((data_awb[10] << 8) & 0xff00) | (data_awb[11] & 0xff);
	...
	pr_err("check_sum_awb=0x%x,check_sum_awb_cal=0x%x\n", check_sum_awb, check_sum_awb_cal);
	if ((otp_flag != 0) && (awb_flag != 0) && (module_id == 0x44) && (lens_id == 0xe7) && (check_sum == check_sum_cal) && (check_sum_awb == check_sum_awb_cal))
	{
		hi556_otp_flag = 1;
	}
	else
		...
		do
		{
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id)
			{
				...
				read_hi556_otp();
//			apply_hi556_otp();
			sprintf(CamNameF_module_name,"HI556:EWELLY:5M_FF");
#if 1
...
static kal_uint32 open(void)
{
	...
	printk("bo_liu, seoul_att front hi556 open start\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
