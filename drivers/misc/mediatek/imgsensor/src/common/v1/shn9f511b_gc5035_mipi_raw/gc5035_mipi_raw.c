...
#include "gc5035_mipi_raw.h"
/************************** Modify Following Strings for Debug **************************/
#define PFX "gc5035_mipi_raw"
/****************************   Modify end    *******************************************/
#define GC5035_DEBUG
...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SHN9F511B_GC5035_SENSOR_ID, /*record sensor id defined in Kd_imgsensor.h*/
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
    ...
    {2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0, 0, 2592, 1944, 0, 0, 2592, 1944},/*full size normal video*/
...
static int module_otp_flag = 0;
static int wb_otp_flag = 0;
static int wb_golden_otp_flag = 0;
static struct gc5035_module_info_t module_info = {0};
...
static kal_uint8 gc5035_otp_identify(void)
{
    ...
    write_cmos_sensor(0xfe, 0x00);
    write_cmos_sensor(0xfa, 0x00);
    printk("bo_liu, front camera gc5035, module_info.module_id = 0x%x, module_info.lens_id = 0x%x, module_otp_flag = %d, wb_otp_flag = %d, wb_golden_otp_flag = %d\n", module_info.module_id, module_info.lens_id, module_otp_flag, wb_otp_flag, wb_golden_otp_flag);
    if((module_info.module_id == 0x43) && (module_info.lens_id == 0x32) && (module_otp_flag == 1) && (wb_otp_flag == 1) && (wb_golden_otp_flag == 1)) {
        printk("bo_liu, front camera gc5035, GC5035A_OTP_Flag = 1\n");
        GC5035A_OTP_Flag = 1;
    } else {
        printk("bo_liu, front camera gc5035, GC5035A_OTP_Flag = 0\n");
        GC5035A_OTP_Flag = 0;
    }
    return moduleID;
}
                                     ...
                                     kal_uint16 target_frame_length)
{
    ...
    imgsensor.min_frame_length = imgsensor.frame_length;
    pr_debug("shutter: %d, frame_length: %d, pre.framelength: %d dummy_line: %d\n",
             shutter, imgsensor.frame_length,
...
//Begin deleted by xiaoming-zhong for [defect][11694519][change normal video to full size] on 2022/01/06
#if 0
...
#endif
//End deleted by xiaoming-zhong for [defect][11694519][change normal video to full size] on 2022/01/06
        ...
        do {
            *sensor_id = return_sensor_id();
            if(*sensor_id == imgsensor_info.sensor_id) {
                ...
                sprintf(CamNameF_module_name, "GC5035:SHINETECH:5M_FF");
                if(GC5035A_OTP_Flag == 1) {
...
static kal_uint32 open(void)
{
    ...
    while(imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            pr_debug("[gc5035_camera_sensor front]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
                     imgsensor.i2c_write_id, sensor_id);
            retry--;
        } while(retry > 0);
        i++;
                               ...
                               MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    ...
    imgsensor.autoflicker_en = KAL_TRUE;
    spin_unlock(&imgsensor_drv_lock);
    //Begin modified by xiaoming-zhong for [defect][11694519][change normal video to full size] on 2022/01/06
    //normal_video_setting(imgsensor.current_fps);
    capture_setting(imgsensor.current_fps);
    //End modified by xiaoming-zhong for [defect][11694519][change normal video to full size] on 2022/01/06
    /* set_mirror_flip(sensor_config_data->SensorImageMirror); */
    return ERROR_NONE;
}
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
    ...
    "GC5035:SHN:5M:ASA5001197C1",
    1,//main:0 front:1 sub:2 
    {
	{PDN, Vol_Low, 1},
			{RST, Vol_Low, 1},
			{DOVDD, Vol_1800, 1},
