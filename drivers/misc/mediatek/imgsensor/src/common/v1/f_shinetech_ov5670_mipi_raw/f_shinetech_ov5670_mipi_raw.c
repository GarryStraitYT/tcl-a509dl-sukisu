...
#include "f_shinetech_ov5670_mipi_raw.h"

#ifdef CONFIG_COMPAT
#include <linux/compat.h>
#endif
#define PFX "f_shinetech_ov5670_mipi_raw"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = F_SHINETECH_OV5670_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	...
	.pre = {
		...
		.framelength = 2038,			//record different mode's framelength
...
static int check_otp_golden_wb(int index)
{
		...
		//partial mode OTP write start address
		write_cmos_sensor(0x3d88, 0x70);
		write_cmos_sensor(0x3d89, 0x40);
		// partial mode OTP write end address
		write_cmos_sensor(0x3d8A, 0x70);
		write_cmos_sensor(0x3d8B, 0x40);
		// read otp into buffer
		write_cmos_sensor(0x3d81, 0x01);
		mDELAY(10);

		//select group
		flag = read_cmos_sensor(0x7040);
		...
		// clear otp buffer
		write_cmos_sensor( 0x7040, 0x00);
		//set 0x5002[3] to 1
		temp1 = read_cmos_sensor(0x5002);
		write_cmos_sensor(0x5002, 0x08 | (temp1 & (~0x08)));
		if (flag == 0x00) {
			LOG_INF("pxs_ov5670_otp golden retun 0,group index is empty; %s \n",__func__); //pei_add
			return 0;
		}
		else if (flag & 0x02) {
			LOG_INF("pxs_ov5670_otp golden return 1,group index has invalid data; %s \n", __func__); //pei_add
			return 1;
		}
		else {
			LOG_INF("pxs_ov5670_otp  golden return 2 ,group index has valid data ; %s \n", __func__); //pei_add
			return 2;
		}
}
// index: index of otp group. (1, 2, 3)
...
static int read_otp_info(int index, struct otp_struct *otp_ptr)
{
		...
		(*otp_ptr).checksum_info = read_cmos_sensor(start_addr + 5);


		checksum += (*otp_ptr).module_integrator_id;
		checksum %= 255;
		checksum += (*otp_ptr).lens_id;
		checksum %= 255;
		checksum += (*otp_ptr).production_year;
		checksum %= 255;
		checksum += (*otp_ptr).production_month;
		checksum %= 255;
		checksum += (*otp_ptr).production_day;
		checksum %= 255;
		checksum += 1;
	    printk("cuckoo pxs_ov5670_otp moduleID is : 0x%x,lens_id:0x%x,year:%d,month:%d,day:%d, checksum_info:%d,checksum:%d  %s \n",(*otp_ptr).module_integrator_id, (*otp_ptr).lens_id, (*otp_ptr).production_year, (*otp_ptr).production_month, (*otp_ptr).production_day, (*otp_ptr).checksum_info, checksum, __func__); //pei_add
		...
		temp1 = read_cmos_sensor(0x5002);
		write_cmos_sensor(0x5002,  0x08 | (temp1 & (~0x08)));
		if((checksum == (*otp_ptr).checksum_info) && ((*otp_ptr).module_integrator_id == OV5670_MODULE_ID))
			return 1;
		else
			return 0;
}
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
		...
		awb0 = read_cmos_sensor(start_addr);
		awb1 = read_cmos_sensor(start_addr + 1);
		awb2 = read_cmos_sensor(start_addr + 2);
		(*otp_ptr).checksum_awb = read_cmos_sensor(start_addr + 3);
		(*otp_ptr).rg_ratio = (awb0<<2) + ((awb2>>6) & 0x03);
		(*otp_ptr).bg_ratio = (awb1<<2) + ((awb2>>4) & 0x03);

		printk("pxs_ov5670_otp awb:0x%x,0x%x,0x%x\n", awb0, awb1, awb2);
		...
		LOG_INF("pxs_ov5670_otp rg_ratio =%x,bg_ratio=%x,checksum_awb:%d, checksum:%d %s \n",(*otp_ptr).rg_ratio ,(*otp_ptr).bg_ratio, (*otp_ptr).checksum_awb, checksum, __func__); //pei_add		

		if((*otp_ptr).checksum_awb == checksum)
			return 1;
		else
			return 0;
}

static int read_otp_golden_wb(int index, struct otp_struct *otp_ptr)
{
		...
		awb1 = read_cmos_sensor(start_addr + 1);
		awb2 = read_cmos_sensor(start_addr + 2);
		(*otp_ptr).checksum_awb_golden = read_cmos_sensor(start_addr + 3);
		(*otp_ptr).golden_rg_ratio = (awb0<<2) + ((awb2>>6) & 0x03);
		(*otp_ptr).golden_bg_ratio = (awb1<<2) + ((awb2>>4) & 0x03);
		LOG_INF("pxs_ov5670_otp awb_golden:0x%x,0x%x,0x%x\n", awb0, awb1, awb2);
		...
		printk("pxs_ov5670_otp golden rg_ratio =%x,golden bg_ratio=%x, checksum_awb_golden:%d, checksum:%d, %s \n",(*otp_ptr).golden_rg_ratio ,(*otp_ptr).golden_bg_ratio, (*otp_ptr).checksum_awb_golden, checksum, __func__); //pei_add


		if((*otp_ptr).checksum_awb_golden == checksum)
			return 1;
		else
			return 0;
}
...
// return 0;
static int update_awb_gain(void)
{
	int AA1, AA2, AA3, AA4, AA5, AA6, AA11, AA12, AA13, AA14, AA15, AA16;

	AA1 = read_cmos_sensor(0x5032);
	AA2 = read_cmos_sensor(0x5033);
	AA3 = read_cmos_sensor(0x5034);
	AA4 = read_cmos_sensor(0x5035);
	AA5 = read_cmos_sensor(0x5036);
	AA6 = read_cmos_sensor(0x5037);
...
static int get_moduleID(void)
{
	...
	LOG_INF("pxs_ov5670_otp info_index=%d\n",info_index); //pei_add
	if (i>3) {
		// no valid wb OTP data
		printk("pxs_ov5670_otp no valid info OTP data\n"); //pei_add
		return -1;
	}

	flag = read_otp_info(info_index, &current_otp);
...
static int get_otp_wb(void)
{
	...
	flag_awb = read_otp_wb(otp_index, &current_otp);
...
//zhongzhu add to read gloden wb data start
	for(i=1;i<=3;i++) {
		temp = check_otp_golden_wb(i);
	...
	LOG_INF("pxs_ov5670_otp golden otp_index=%d,  %s \n",otp_index, __func__); //pei_add
	if (i>3) {
		// no valid wb OTP data
		printk("pxs_ov5670_otp golden no valid wb OTP data  %s \n", __func__); //pei_add
		return 1;
	}

	flag_awb_golden = read_otp_golden_wb(otp_index, &current_otp);
	RG_Ratio_Typical = current_otp.golden_rg_ratio ;
	BG_Ratio_Typical = current_otp.golden_bg_ratio;
//zhongzhu add to read gloden wb data end
LOG_INF("leihou_ov5670_otp	RG_Ratio_Typical=%x,BG_Ratio_Typical=%x,  %s \n",RG_Ratio_Typical,BG_Ratio_Typical, __func__);
	//xuejian.zhong change this
	if (RG_Ratio_Typical == 0 || BG_Ratio_Typical == 0){
		printk("pxs_ov5670_otp	read golden wb OTP data error  %s \n", __func__); //pei_add
		return 1;
	}
	// change end
	//calculate G gain
	...
	return (flag_awb & flag_awb_golden);
}
...
#define  IMG_DIR  IMAGE_HV_MIRROR
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x380f, 0xf6); //; fps fine adjustment
...
//	write_cmos_sensor(0x3623, 0x00);  //
	...
	write_cmos_sensor(0x380f, 0xf6);  //;e0
...
*
*************************************************************************/
static int otp_check(void)
{
	int flag_info, flag_wb;
	flag_info = get_moduleID();
	flag_wb = get_otp_wb();

	return (flag_info & flag_wb);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	pr_err("leihou-front ov5670 shinetech get_imgsensor_id!\n");
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C)) + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
                                ...
                                LOG_INF("leihou-i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
#ifdef OV5670_OTP
				otp_result = otp_check();
				if(otp_result)
				{
					LOG_INF("otp ok");
					...
					*sensor_id = 0xFFFFFFFF;
					LOG_INF("otp nok ,or not this module?");
				}
#else
