...
/*Start junyiliu for Jetta VZW ov13b10 otp check 20211123*/
...
/*End junyiliu for Jetta VZW ov13b10 otp check 20211123*/
...
static bool check_sum(BYTE *buf, unsigned int size)
{
	...
	{
		pr_err("check_sum fail size = %d sum=%d sum-in-eeprom=%d", size, (sum %256), buf[size - 1]);
		return false;
	}
	return true;
}
...
static bool check_sum_awb(awb_struct *buf, unsigned int size)
{
	...
	{
		pr_err("check_sum_awb fail size = %d sum=%d sum-in-eeprom=%d", size, (sum %256), buf->chksum);
		return false;
	}
	return true;
}
...
static bool check_sum_lsc(lsc_struct *buf, unsigned int size)
{
	...
	if ((sum %256)  != buf->chksum)
	{
		pr_err("check_sum_lsc fail size = %d sum=%d sum-in-eeprom=%d", size,(sum %256), buf->chksum);
		return false;
	}
	return true;
}
...
static bool check_sum_pdaf(pdaf_struct *buf, unsigned int size)
{
	...
	{
		pr_err("ov13b10 pdaf chksum1 fail size = %d sum1=%d sum1-in-eeprom=%d", size, (sum1 % 256), buf->chksum1);
		return false;
	}
        if ((sum2 % 256) != buf->chksum2)
	{
		pr_err("ov13b10 pdaf chksum2 fail size = %d sum2=%d sum2-in-eeprom=%d", size, (sum2 % 256), buf->chksum2);
		return false;
	}
	return true;
}
	...
	if (pModuleInfo->moduleInoramtionFlag == 0x01)
	{
		...
		if (check_sum((BYTE *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID   == 0x43 &&
				pModuleInfo->LENSID     == 0x43 &&
				pModuleInfo->VCMID      == 0xD5 &&
...
/*Start junyiliu for Jetta VZW ov13b10 otp check 20211123*/
	...
	read_shinetech_ov13b10_eeprom(AF_INFO_START_ADDR, (BYTE *)pAFInfo, sizeof(af_struct));
	...
	read_shinetech_ov13b10_eeprom(PDAF_INFO_START_ADDR, (BYTE *)pPDAFInfo, sizeof(pdaf_struct));
