...
#define AWB_INFO_START_ADDR 0x000C
#define LSC_INFO_START_ADDR 0x0016
#define AF_INFO_START_ADDR 0x0764
#define PDAF_INFO_START_ADDR 0x076E
...
static bool check_sum_pdaf(pdaf_struct *buf, unsigned int size)
{
	...
	{
		LOG_INF("chksum fail size = %d sum1=%d sum1-in-eeprom=%d sum2=%d sum2-in-eeprom=%d", size, (sum1 % 255) + 1, buf->chksum1, (sum2 % 255) + 1, buf->chksum2);
		return false;
	}
	return true;
}
...
bool check_hi1336_otp(void)
{
	...
	if (pModuleInfo->moduleInoramtionFlag == 0x55)
	{
		...
		if (check_sum((BYTE *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID == 0x01 && pModuleInfo->LENSID == 0x01 && pModuleInfo->VCMID == 0x01 && pModuleInfo->DriverICID == 0x01)
			{
