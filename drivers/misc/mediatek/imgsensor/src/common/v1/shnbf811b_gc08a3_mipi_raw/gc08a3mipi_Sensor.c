...
static struct imgsensor_info_struct imgsensor_info = {
    .sensor_id = SHNBF811B_GC08A3_SENSOR_ID,
    ...
    .i2c_addr_table = {0x20, 0x22, 0x62, 0x24, 0xff},
    .i2c_speed = 400,
};
...
bool check_gc08a3_otp(void)
{
	...
	if(otpStatus == EEPROM_MODULEINFO_FLAG)
	{
		if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID == 0x43 && pModuleInfo->LENSID == 0x33 && pModuleInfo->VCMID == 0x0 && pModuleInfo->DriverICID == 0x0)
			{
	...
	else if ((awbFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:awb info group2 valid");
                ...
                gc08a3_iReadData(AWB_INFO_FLAG + (AWB_LENGTH+1)*8 + AWB_LENGTH*8, 1, &gc08a3_otp_info.awbChksum);
	}
	else
	{
	...
	else if ((lscFlag  & 0x0C) == 0x04)
	{
		pr_err("[ljt]gc08a3OTP:lsc info group2 valid");
                ...
                gc08a3_iReadData(LSC_INFO_FLAG + (LSC_LENGTH+1)*8+ LSC_LENGTH*8, 1, &gc08a3_otp_info.lscChksum);
	}
	else
	{
    ...
    //             gc08a3_iReadData(AF_INFO_FLAG + (AF_LENGTH+1)*8 + AF_LENGTH*8, 1, &gc08a3_otp_info.afChksum);
	// }
	// else
	// {
