	...
	for (index = 0; index < 2; index++) {
		...
		case 0x04:
			...
			if ((check % 255 + 1) == info[INFO_WIDTH - 1]) {
				...
				gc8034_otp_info.vcm_driver_id = info[3];
				gc8034_otp_info.vcm_id = info[2];
		...
		case 0x04:
			...
			if ((check % 255 + 1) == af[AF_WIDTH - 1]) {
				...
				LOG_INF("af_infinity:0x%x af_macro:0x%x\n", gc8034_otp_info.af_infinity,gc8034_otp_info.af_macro);
				LOG_INF("af_infinity limit(200,450)af_macro limit(330,630)\n");
				if((gc8034_otp_info.af_infinity >= 200 && gc8034_otp_info.af_infinity <= 450)&&
				   (gc8034_otp_info.af_macro    >= 330 && gc8034_otp_info.af_macro    <= 630)){
...
static bool gc8034_gcore_check_otp(void)
{
    bool ret = false;
    write_cmos_sensor(0xfc, 0x00);
    write_cmos_sensor(0xf7, 0x97);
    ...
    if((gc8034_otp_info.module_id     == 0x48 &&
	    gc8034_otp_info.lens_id       == 0xb1 &&
		gc8034_otp_info.vcm_driver_id == 0x12 &&
		gc8034_otp_info.vcm_id        == 0xe2)
       && ((gc8034_otp_info.wb_flag & 0x01) == 0x01)
       && ((gc8034_otp_info.af_flag & 0x01) == 0x01)
       && ((gc8034_otp_info.golden_flag & 0x01) == 0x01)
       && ((gc8034_otp_info.lsc_flag & 0x01) == 0x01))
        ret = true;
    return ret;
}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				//gc8034_otp_identify();
				pr_err("[gc8034_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
                sensor_init();
			    pr_err("GC8034 check otp Begin!!\n");
                GC8034_OTP_Flag = gc8034_gcore_check_otp();
                if(GC8034_OTP_Flag == 1) {
    			    pr_err("GC8034 check otp ok!!\n");
