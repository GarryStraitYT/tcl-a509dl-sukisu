...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = CXTVM15502_GC5035_SENSOR_ID,
...
{
	kal_uint8 dpcFlag = 0;
	kal_uint16 ddtemp0,ddtemp1,ddtemp2,ddtemp3,k;
	...
	case GC5035_OTP_FLAG_VALID: {
		...
		LOG_INF("total_num = %d\n", pDPC->total_num);
		// fae qinyuanyi add dd x,y 20200509 begin
		for(k=0; k<pDPC->total_num; k++)
		{
		    ...
		    LOG_INF("GC5035_OTP_DD:index = %d, data[0] = %x , data[1] = %x , data[2] = %x ,data[3] = %x \n",\
			    k, ddtemp0, ddtemp1, ddtemp2, ddtemp3);
		    LOG_INF("GC5035_OTP_DD x = %d , y = %d ,type = %d \n",((ddtemp1&0x0f)<<8) + ddtemp0,
				((ddtemp2&0x7f)<<4) + ((ddtemp1&0xf0)>>4),
				(((ddtemp3&0x03)<<1)+((ddtemp2&0x80)>>7)));
		}
		// fae qinyuanyi add dd x,y 20200509 end
		break;
	}
	default:
...
static kal_uint8 gc5035_otp_read_sensor_info(void)
{
	...
	for (i = 0; i < GC5035_OTP_DATA_LENGTH; i++)
		if(debug[i] != 0)  LOG_INF("addr = 0x%x, data = 0x%x\n", GC5035_OTP_START_ADDR + i * 8, debug[i]);
#endif
...
static kal_uint8 gc5035_otp_identify(void)
{
	...
	printk("bo_liu, back camera gc5035, module_info.module_id = 0x%x, module_info.lens_id = 0x%x, module_otp_flag = %d, wb_otp_flag = %d, wb_golden_otp_flag = %d\n", module_info.module_id, module_info.lens_id, module_otp_flag, wb_otp_flag, wb_golden_otp_flag);
	if((module_info.module_id == 0x43 || module_info.module_id == 0x47) && (module_info.lens_id == 0xe1 || module_info.lens_id == 0xe3) 
        && (module_otp_flag == 1) && (wb_otp_flag == 1) && (wb_golden_otp_flag == 1))
	{
		printk("bo_liu, back camera gc5035, GC5035A_OTP_Flag = 1\n");
		GC5035A_OTP_Flag = 1;
	}else{
		printk("bo_liu, back camera gc5035, GC5035A_OTP_Flag = 0\n");
		GC5035A_OTP_Flag = 0;
	}
...
static kal_uint32 set_test_pattern_mode(kal_bool enable)
{
 ...
 //char Back_Camera_Name[256];
 //char Back_Camera_OTP[256];
...
#if 1
                                        /*Begin modified by xiaoming-zhong for [Task][11669054][change macau pro sensor info] on 2021/11/11*/
					sprintf(CamNameB_module_name,"GC5035:CXT:5M_FF:ASA5001248C1");
                                        /*End modified by xiaoming-zhong for [Task][11669054][change macau pro sensor info] on 2021/11/11*/
					printk("GC5035A_OTP_Flag:%d", GC5035A_OTP_Flag);
					#ifdef TARGET_BUILD_MMITEST
										
                    			printk("in mmitest mode");
					if(GC5035A_OTP_Flag==1){
						*sensor_id |= 0x01000000;  ////add by junyiliu 20220120
					}else{
						sprintf(CamOTPB_module_name,"0");
					}
					return ERROR_NONE;
					#else
					if(GC5035A_OTP_Flag==1){
...
UINT32 CXTVM15502_GC5035_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
