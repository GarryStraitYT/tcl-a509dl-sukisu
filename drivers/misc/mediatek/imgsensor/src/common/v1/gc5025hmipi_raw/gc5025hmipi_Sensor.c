...
static int wb_otp_golden_flag = 0;

static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5025HMIPI_SENSOR_ID,
	...
	for (index = 0; index < 2; index++) {
		...
		case 0x01:
			...
			if ((check % 255 + 1) == info[INFO_WIDTH-1]) {
				...
				if((0x47==gc5025h_otp_info.module_id) && (0xe8==gc5025h_otp_info.lens_id) && (0x00==gc5025h_otp_info.vcm_driver_id) && (0x00==gc5025h_otp_info.vcm_id))
				{
					module_otp_flag = 1;
					LOG_INF("leihou module_otp_flag %d\n", module_otp_flag);
				}
			} else
		...
		case 0x01:
			...
			if ((check % 255 + 1) == golden[GOLDEN_WIDTH - 1]) {
				...
				wb_otp_golden_flag = 1;
				LOG_INF("leihou wb_otp_golden_flag %d\n", wb_otp_golden_flag);
			} else
...
static void gc5025h_gcore_update_dd(void)
{
	...
	if (gc5025h_otp_info.dd_flag == 0x01) {
#if GC5025H_MIRROR_FLIP_ENABLE
...
static void gc5025h_gcore_update_wb(void)
{
	...
	if (gc5025h_otp_info.golden_flag == 0x02)
		return;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if((1 == module_otp_flag) && (1 == wb_otp_flag) && (1 == wb_otp_golden_flag))
				{
					*sensor_id |= 0x01000000;
					LOG_INF("leihou 5025H otp ok ");
                                        return ERROR_NONE;
				}
                                else{
			...
			LOG_INF("leihou-Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
