...
static bool check_sum_awb(awb_struct *buf, unsigned int size)
{
	...
	for (i = 0; i < sizeof(buf->awbInformation); i++)
	{
		...
		pr_debug("awbData[%d] = 0x%x %d",i,buf->awbInformation[i],buf->awbInformation[i]);
	}
...
static bool check_sum_af(af_struct *buf, unsigned int size)
{
	...
	{
		sum += buf->afData[i];
		pr_debug("afData[%d] = 0x%x %d",i,buf->afData[i],buf->afData[i]);
	}
...
bool check_tsp_s5kjn1new_otp(void)
{
    ...
    if (pModuleInfo->moduleInoramtionFlag == 0x01)
    {
		...
		if (check_sum((BYTE *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if ((pModuleInfo->moduleID   == 0x48 &&
				...
				(pModuleInfo->moduleID   == 0x48 &&
				pModuleInfo->LENSID     == 0x1 &&
				pModuleInfo->VCMID      == 0x1 &&
				pModuleInfo->EEPROMID   == 0x1 &&
				pModuleInfo->DriverICID == 0x1))
			{
			...
			{
				pr_err("new moduleID =0x%x,LENSID=0x%x,VCMID=0x%x,EEPROMID=0x%x,DriverICID=0x%x",
	...
	read_tsp_s5kjn1_eeprom(HWGGC_START_ADDR, (BYTE *)&hwggc1, sizeof(hwggc_struct1));
	if (hwggc1.hwGGCFlag == 0x01)
	{
		otpStatus |= EEPROM_HWGGC_FLAG;
		if (check_sum((BYTE *)&hwggc1, sizeof(hwggc_struct1)))
		{
	...
	{
		pr_err("error pdafFlag=%d", hwggc1.hwGGCFlag);
	}
