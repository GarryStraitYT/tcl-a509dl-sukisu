			...
			*sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1);
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				if(GC5035A_OTP_Flag == 1) {
				    //sprintf(CamOTPF_module_name, "OTP:OK");
				    *sensor_id |= 0x01000000;//zhuyuantao add for otp detect
				} else {
				    //sprintf(CamOTPF_module_name, "OTP:NOK");
				}				
...
UINT32 GC5035_ODIN_TSP_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
