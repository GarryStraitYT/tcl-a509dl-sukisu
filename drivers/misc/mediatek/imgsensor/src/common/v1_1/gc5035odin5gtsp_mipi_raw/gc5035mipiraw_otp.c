	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 255 + 1) == info[GC5035_OTP_MODULE_DATA_SIZE - 1]) {
				...
				module_info.lens_id = info[1];   //  year month day lens_id
...
kal_uint8 gc5035_otp_identify(void)
{
	...
	if((module_info.module_id == 0x48) && (module_info.lens_id == 0xc2) && (module_otp_flag == 1) && (wb_otp_flag == 1) && (wb_golden_otp_flag == 1)) {
        printk("zyt,camera gc5035, GC5035A_OTP_Flag = 1\n");
        GC5035A_OTP_Flag = 1;
    } else {
        printk("zyt,camera gc5035, GC5035A_OTP_Flag = 0\n");
        GC5035A_OTP_Flag = 0;
    }
