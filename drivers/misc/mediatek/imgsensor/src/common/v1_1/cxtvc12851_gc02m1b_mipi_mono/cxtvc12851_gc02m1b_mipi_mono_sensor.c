...
#include "cxtvc12851_gc02m1b_mipi_mono_sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = CXTVC12851_GC02M1B_SENSOR_ID,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	0, 0, 1600, 1200, 0, 0, 1600, 1200}, /* Preview */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
	0, 0, 1600, 1200, 0, 0, 1600, 1200}, /* capture */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
	0, 0, 1600, 1200, 0, 0, 1600, 1200}, /* video */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
	0, 0, 1600, 1200, 0, 0, 1600, 1200}, /* HS video */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
	160, 240, 1280, 720, 0, 0, 1280, 720},  /* slim video */
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200,
	0, 0, 1600, 1200, 0, 0, 1600, 1200}, /* MTF */
};
...
static void write_shutter(kal_uint32 shutter)
{
	...
	if (shutter >= 0x3fef) {
		...
		write_cmos_sensor(0xc7, long_exp_h);
		write_cmos_sensor(0xc8, long_exp_m);
		write_cmos_sensor(0xc9, long_exp_l);
		write_cmos_sensor(0xef, 0x01);
		write_cmos_sensor(0x87, 0x51);
		write_cmos_sensor(0x8c, 0xb0);
				     ...
				     kal_uint16 frame_length,kal_bool auto_extend_en)
{
	...
	if (imgsensor.autoflicker_en) {
		if (realtime_fps >= 297 && realtime_fps <= 305)
			...
			set_dummy();//set_max_framerate(realtime_fps, 0);
	} else
		set_dummy();
...
static kal_uint16 gc02m1b_init_setting[] = {
	/*system*/
	0xfc, 0x01,
	0xf4, 0x41,
...
static kal_uint16 gc02m1b_preview_setting[] = {
	0xfe, 0x00,
	0x3e, 0x90,

};

static kal_uint16 gc02m1b_slim_video_setting[] = {
	0xfe, 0x01,
	0x90, 0x01,
	0x92, 0xf6,
	0x94, 0xa6,
...
{
	pr_err("[%s] init_start\n", __func__);
	gc02m1b_table_write_cmos_sensor(gc02m1b_init_setting,
		sizeof(gc02m1b_init_setting)/sizeof(kal_uint16));
	pr_err("[%s] init_End\n", __func__);
}	/*	  sensor_init  */

static void preview_setting(void)
{
	pr_err("%s preview_Start\n", __func__);
	gc02m1b_table_write_cmos_sensor(gc02m1b_preview_setting,
		sizeof(gc02m1b_preview_setting)/sizeof(kal_uint16));
	pr_err("%s preview_End\n", __func__);
}
...
{
	pr_err("[%s] slim_video_Start, 1280x720@30fps\n", __func__);
	gc02m1b_table_write_cmos_sensor(gc02m1b_slim_video_setting,
		sizeof(gc02m1b_slim_video_setting)/sizeof(kal_uint16));
	pr_err("[%s] slim_video_End\n", __func__);
}
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1)+7;
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc02m1b_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_err("[gc02m1b_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1)+7;
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc02m1b_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("[gc02m1b_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	"GC02M1B:CXT:2M:ASA2001251C1",
	4,//main:0 front:1 main2:2  front2:3  main3:4
	{
...
UINT32 CXTVC12851_GC02M1B_MIPI_MONO_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
