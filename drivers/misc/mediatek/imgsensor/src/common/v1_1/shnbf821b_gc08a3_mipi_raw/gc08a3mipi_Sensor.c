...
#define LOG_1 LOG_INF("GC08A3MIPI, 4LANE\n")
/****************************   Modify end    *******************************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = SHNBF821B_GC08A3_SENSOR_ID,       //+3
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] = {
	...
	{ 3264, 2448, 0, 0, 3264, 2448, 3264, 2448, 0, 314, 3264, 1836, 0, 0, 3264, 1836}, /* video */
...
static void table_write_cmos_sensor(kal_uint16 *para, kal_uint32 len)
{
	...
	while (len > idx) {
		...
		idx += 2;
#if MULTI_WRITE
		if (tosend >= (I2C_BUFFER_LEN - 1) || idx == len) {
...
static bool check_sum(kal_uint8 *buf, unsigned int size)
{
	...
	{
		LOG_INF("chksum fail size = %d sum=%d sum-in-eeprom=%d", size, (sum % 256), buf[size - 1]);
		return false;
	}
	return true;
}
...
static bool check_sum_awb(kal_uint8 *buf, unsigned int size, kal_uint8 chksum)
{
	...
	{
		sum += buf[i];
		LOG_INF("awbData[%d] = 0x%x %d", i, buf[i], buf[i]);
	}

	if ((sum % 256) != chksum)
	{
		LOG_INF("chksum fail size = %d sum=%d sum-in-eeprom=%d", size, (sum % 256), chksum);
		return false;
	}
	return true;
}
...
static bool check_sum_af(kal_uint8 *buf, unsigned int size, kal_uint8 chksum)
{
	...
	{
		sum += buf[i];
		LOG_INF("afData[%d] = 0x%x %d", i, buf[i], buf[i]);
	}
...
static bool check_gc08a3_otp(void)
{
	...
	unsigned int otpStatus = 0;
        ...
        memset(&gc08a3_otp_info, 0, sizeof(struct gc08a3_otp_t));
	...
	{
		LOG_INF("[ljt]gc08a3OTP:module info group1 valid");
	...
	{
		LOG_INF("[ljt]gc08a3OTP:module info group2 valid");
		otpStatus |= EEPROM_MODULEINFO_FLAG;
                gc08a3_iReadData(MODULE_INFO_FLAG + (MODULE_LENGTH+1)*8, sizeof(moduleInformation_struct), (unsigned char *)pModuleInfo);
	}
	else
	{
		LOG_INF("[ljt]gc08a3OTP:moduleInoramtionFlag=%d", moduleInoramtionFlag);
	}
	...
	if(otpStatus == EEPROM_MODULEINFO_FLAG)
	{
		if (check_sum((kal_uint8 *)pModuleInfo, sizeof(moduleInformation_struct)))
		{
			...
			if (pModuleInfo->moduleID == 0x43 && pModuleInfo->LENSID == 0x33 
				#if AF_OTP_SUPPORT
					&& pModuleInfo->VCMID == 0xD2 && pModuleInfo->DriverICID == 0x13
				#endif
				)
			{
				otpStatus |= EEPROM_MODULEINFO_VALUE;
				LOG_INF("[ljt]gc08a3OTP:module info flag chksum value pass");
			}
			else
			{
				LOG_INF("[ljt]gc08a3OTP:moduleID =0x%x,LENSID=0x%x,VCMID=0x%x,DriverICID=0x%x",
	...
	{
		LOG_INF("[ljt]gc08a3OTP:awb info group1 valid");

                gc08a3_otp_info.awb_flag = 0x01;
                gc08a3_iReadData(AWB_INFO_FLAG + 8, AWB_LENGTH-1, &gc08a3_otp_info.awb_param[0]);
                gc08a3_iReadData(AWB_INFO_FLAG + AWB_LENGTH*8, 1, &gc08a3_otp_info.awbChksum);
	}
	else if ((awbFlag  & 0x0C) == 0x04)
	{
		LOG_INF("[ljt]gc08a3OTP:awb info group2 valid");

                gc08a3_otp_info.awb_flag = 0x04;
                gc08a3_iReadData(AWB_INFO_FLAG + (AWB_LENGTH+1)*8, AWB_LENGTH-1, &gc08a3_otp_info.awb_param[0]);
                gc08a3_iReadData(AWB_INFO_FLAG + (AWB_LENGTH+1)*8 + (AWB_LENGTH-1)*8, 1, &gc08a3_otp_info.awbChksum);
	}
	else
	{
		LOG_INF("awbFlag=%d", awbFlag);
	}

	if ((gc08a3_otp_info.awb_flag == 0x01) || (gc08a3_otp_info.awb_flag == 0x04))
	{
		otpStatus |= EEPROM_AWB_FLAG;
                //pAWBInfo->awbInformation = gc08a3_otp_info.awb_param;

		if (check_sum_awb(&gc08a3_otp_info.awb_param[0], AWB_LENGTH-1, gc08a3_otp_info.awbChksum))
		{
			otpStatus |= EEPROM_AWB_CHKSUM;
			LOG_INF("[ljt]gc08a3OTP:awb flag chksum pass");
		}
		else
		{
			int i;
			for (i = 0; i < AWB_LENGTH-1; i++)
			     LOG_INF("[ljt]gc08a3OTP:awb[%d]=0x%x  %d\n", i, gc08a3_otp_info.awb_param[i], gc08a3_otp_info.awb_param[i]);
		}
	}
	//kfree(pAWBInfo);
	...
	{
		LOG_INF("[ljt]gc08a3OTP:lsc info group1 valid");

                gc08a3_otp_info.lsc_flag = 0x01;
                gc08a3_iReadData(LSC_INFO_FLAG + 8, LSC_LENGTH-1, &gc08a3_otp_info.lsc_param[0]);
                gc08a3_iReadData(LSC_INFO_FLAG + LSC_LENGTH*8, 1, &gc08a3_otp_info.lscChksum);
	}
	else if ((lscFlag  & 0x0C) == 0x04)
	{
		LOG_INF("[ljt]gc08a3OTP:lsc info group2 valid");

                gc08a3_otp_info.lsc_flag = 0x04;
                gc08a3_iReadData(LSC_INFO_FLAG + (LSC_LENGTH+1)*8, LSC_LENGTH-1, &gc08a3_otp_info.lsc_param[0]);
                gc08a3_iReadData(LSC_INFO_FLAG + (LSC_LENGTH+1)*8+ (LSC_LENGTH-1)*8, 1, &gc08a3_otp_info.lscChksum);
	}
	else
	{
		LOG_INF("[ljt]gc08a3OTP:lscFlag=%d", lscFlag);
	}

	if ((gc08a3_otp_info.lsc_flag == 0x01) || (gc08a3_otp_info.lsc_flag == 0x04))
	{
		otpStatus |= EEPROM_LSC_FLAG;
		//pLSCInfo->lscData = gc08a3_otp_info.lsc_param;

		if (check_sum_lsc(&gc08a3_otp_info.lsc_param[0], LSC_LENGTH-1, gc08a3_otp_info.lscChksum))
		{
			otpStatus |= EEPROM_LSC_CHKSUM;
			LOG_INF("[ljt]gc08a3OTP:lsc flag chksum pass");
		}
	}
	...
	if ((afFlag  & 0x03) == 0x01)
	{
		LOG_INF("[ljt]gc08a3OTP:af info group1 valid");

                gc08a3_otp_info.af_flag = 0x01;
                gc08a3_iReadData(AF_INFO_FLAG + 8, AF_LENGTH-1, &gc08a3_otp_info.af_param[0]);
                gc08a3_iReadData(AF_INFO_FLAG + AF_LENGTH*8, 1, &gc08a3_otp_info.afChksum);
	}
	else if ((afFlag  & 0x0C) == 0x04)
	{
		LOG_INF("[ljt]gc08a3OTP:af info group2 valid");

                gc08a3_otp_info.af_flag = 0x04;
                gc08a3_iReadData(AF_INFO_FLAG + (AF_LENGTH+1)*8, AF_LENGTH-1, &gc08a3_otp_info.af_param[0]);
                gc08a3_iReadData(AF_INFO_FLAG + (AF_LENGTH+1)*8 + (AF_LENGTH-1)*8, 1, &gc08a3_otp_info.afChksum);
	}
	else
	{
		LOG_INF("[ljt]gc08a3OTP:afFlag=%d", afFlag);
	}
	...
	if ((gc08a3_otp_info.af_flag == 0x01) ||(gc08a3_otp_info.af_flag == 0x04))
	{
		...
		otpStatus |= EEPROM_AF_FLAG;
                //pAFInfo->afData = gc08a3_otp_info.af_param;
		if (check_sum_af(&gc08a3_otp_info.af_param[0], AF_LENGTH-1, gc08a3_otp_info.afChksum))
		{
			otpStatus |= EEPROM_AF_CHKSUM;
			LOG_INF("[ljt]gc08a3OTP:af flag chksum pass");
		}
	}
	//kfree(pAFInfo);
#endif
...
static kal_uint16 gc08a3_3264x1836_addr_data[] = {
	/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
...
{
	gc08a3_stream_off();
	table_write_cmos_sensor(gc08a3_3264x1836_addr_data,
		sizeof(gc08a3_3264x1836_addr_data)/sizeof(kal_uint16));
	gc08a3_stream_on();
}
