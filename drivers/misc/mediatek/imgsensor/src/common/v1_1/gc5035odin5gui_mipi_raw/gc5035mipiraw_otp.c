...
static kal_uint16 read_cmos_sensor(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 1, (u8 *)&get_byte, 1, otp_i2c_write_id2);
...
static void write_cmos_sensor(kal_uint32 addr, kal_uint32 para)
{
	...
	iWriteRegI2C(pu_send_cmd, 2, otp_i2c_write_id2);
}
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 255 + 1) == info[GC5035_OTP_MODULE_DATA_SIZE - 1]) {
				...
				module_info.year = info[1];   //  year month day lens_id
				module_info.month = info[2];
				module_info.day = info[3];
				module_info.lens_id = info[4];
				module_otp_flag = 1;
				pr_err("zyt_module_id = 0x%x\n", module_info.module_id);
				pr_err("zyt_lens_id = 0x%x\n", module_info.lens_id);
				pr_err("zyt_data = %d-%d-%d\n", module_info.year, module_info.month, module_info.day);
			} else
				pr_err("zyt_check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1, info[GC5035_OTP_MODULE_DATA_SIZE - 1], (check % 255 + 1));
			break;
		}
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			pr_debug("zyt_group %d is invalid!\n", idx + 1);
			break;
		}
		default:
			break;
...
static void gc5035_otp_read_wb_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		switch (GC5035_OTP_GET_2BIT_FLAG(flag, 2 * (1 - idx))) {
		case GC5035_OTP_FLAG_EMPTY: {
			pr_err("zyt_wb group %d is empty!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			pr_err("zyt_wb group %d is valid!\n", idx + 1);
			...
			if ((wb_check % 255 + 1) == wb[GC5035_OTP_WB_DATA_SIZE - 1]) {
				...
				wb_otp_flag = 1;
				pr_err("zyt_wb r/g = 0x%x\n", pWB->rg);
				pr_err("zyt_wb b/g = 0x%x\n", pWB->bg);
			} else {
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
				pr_err("zyt_wb check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
		...
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			pr_err("zyt_wb group %d is invalid!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
			break;
		}
		default:
		...
		case GC5035_OTP_FLAG_EMPTY: {
			pr_debug("zyt_golden group %d is empty!\n", idx + 1);
			pGolden->flag = pGolden->flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			pr_debug("zyt_golden group %d is valid!\n", idx + 1);
...
kal_uint8 gc5035_otp_identify2(void)
{
	kal_uint8 moduleID = 0;
	...
	printk("zyt,camera gc5035, module_info.module_id = 0x%x, module_info.lens_id = 0x%x, module_otp_flag = %d, wb_otp_flag = %d, wb_golden_otp_flag = %d\n", module_info.module_id, module_info.lens_id, module_otp_flag, wb_otp_flag, wb_golden_otp_flag);

	if((module_info.module_id == 0x43) && (module_info.lens_id == 0xe6) && (module_otp_flag == 1) && (wb_otp_flag == 1) && (wb_golden_otp_flag == 1)) {
        printk("zyt,camera gc5035, GC5035A_OTP_Flag2 = 1\n");
        GC5035A_OTP_Flag2 = 1;
    } else {
        printk("zyt,camera gc5035, GC5035A_OTP_Flag2 = 0\n");
        GC5035A_OTP_Flag2 = 0;
    }
...
void gc5035_otp_function2(void)
{
