...
#define PFX "hi846odin5g_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = HI846_ODIN5G_SENSOR_ID,
	...
	.i2c_addr_table = {0x46, 0x42, 0x44, 0xff},
	.i2c_speed = 400,
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0F04, 0x3200);
...
extern struct HI846_otp_struct HI846_otp2;
...
{	
	...
	HI846_OTP_write_cmos_sensor(0x0D04, 0x00); //Fsync Output enable
...
int info_otp_flag = 0;
int awb_otp_flag = 0;
int hi846_af_otp_flag = 0;
int hi846_lsc_otp_flag = 0;
//int hi846afinf = 0;
//int hi846afmacro = 0;

static int HI846_otp_read(void)
{
	int i = 0, addr = 0, wb_start_addr = 0,af_start_addr = 0, lsc_start_addr = 0;
	int  checksum = 0;
	LOG_INF("HI846 HI846_otp_read \n");

	HI846_otp2.Base_Info_Flag = OTP_read_cmos_sensor(0x0201);
	if (HI846_otp2.Base_Info_Flag == 0x01)	//Base Info Group1 valid
		addr = 0x0202;
	else if (HI846_otp2.Base_Info_Flag == 0x13)	//Base Info Group2 valid
		addr = 0x0213;
	else if (HI846_otp2.Base_Info_Flag == 0x37)	//Base Info Group3 valid
		addr = 0x0224;
	else
		addr = 0;
LOG_INF("HI846 addr = 0x%x \n", addr);
	if (addr == 0) {
		HI846_otp2.module_integrator_id = 0;
		HI846_otp2.prodyction_year = 0;
		HI846_otp2.production_month = 0;
		HI846_otp2.production_day = 0;
		HI846_otp2.lens_id = 0;
		HI846_otp2.vcm_id = 0;
		HI846_otp2.Driver_ic_id = 0;
		HI846_otp2.infocheck = 0;
	} else {
		HI846_otp2.module_integrator_id = OTP_read_cmos_sensor(addr);
		HI846_otp2.AF_FLAG = OTP_read_cmos_sensor(addr + 1);
		HI846_otp2.prodyction_year = OTP_read_cmos_sensor(addr + 2);
		HI846_otp2.production_month = OTP_read_cmos_sensor(addr + 3);
		HI846_otp2.production_day = OTP_read_cmos_sensor(addr + 4);
		HI846_otp2.sensor_id = OTP_read_cmos_sensor(addr + 5);
		HI846_otp2.lens_id = OTP_read_cmos_sensor(addr + 6);
		HI846_otp2.vcm_id = OTP_read_cmos_sensor(addr + 7);
		HI846_otp2.Driver_ic_id = OTP_read_cmos_sensor(addr + 8);
		HI846_otp2.F_num_id = OTP_read_cmos_sensor(addr + 9);
		HI846_otp2.infocheck = OTP_read_cmos_sensor(addr + 16);
	}
	checksum = (HI846_otp2.module_integrator_id + HI846_otp2.AF_FLAG + HI846_otp2.prodyction_year + HI846_otp2.production_month + HI846_otp2.production_day + HI846_otp2.sensor_id + HI846_otp2.lens_id + HI846_otp2.vcm_id + HI846_otp2.Driver_ic_id + HI846_otp2.F_num_id) % 0xFF + 1;

       if (checksum == HI846_otp2.infocheck)
	{
		info_otp_flag = 1;
		//hi846_af_otp_flag = 1;
...
LOG_INF("HI846 module_integrator_id = 0x%x, AF_FLAG = 0x%x, prodyction_year = 0x%x, production_month = 0x%x, production_day = 0x%x, sensor_id = 0x%x, lens_id = 0x%x, vcm_id = 0x%x, Driver_ic_id = 0x%x, F_num_id, 0x%x, infocheck = 0x%x \n",\
		HI846_otp2.module_integrator_id,HI846_otp2.AF_FLAG, HI846_otp2.prodyction_year, HI846_otp2.production_month, HI846_otp2.production_day, HI846_otp2.sensor_id, HI846_otp2.lens_id,HI846_otp2.vcm_id, HI846_otp2.Driver_ic_id, HI846_otp2.F_num_id, HI846_otp2.infocheck);


        checksum = 0;
	HI846_otp2.WB_FLAG = OTP_read_cmos_sensor(0x0C5F);
	if (HI846_otp2.WB_FLAG == 0x01)
		wb_start_addr = 0x0C60;
	else if (HI846_otp2.WB_FLAG == 0x13)
		wb_start_addr = 0x0C7E;
	else if (HI846_otp2.WB_FLAG == 0x37)
		wb_start_addr = 0x0C9C;
	else
	{
                HI846_otp2.WB_FLAG = 0;
		wb_start_addr = 0;
...
LOG_INF("HI846 wb_start_addr = 0x%x \n", wb_start_addr);
	...
	if (wb_start_addr != 0) {
                HI846_otp2.WB_FLAG = 1;
		for (i = 0; i < 30; i++) {
			HI846_OTP_write_cmos_sensor(0x70a, ((wb_start_addr+i) >> 8) & 0xff);	//start addr H
			HI846_OTP_write_cmos_sensor(0x70b, (wb_start_addr+i) & 0xff);	//start addr L
			HI846_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
		
			HI846_otp2.wb_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI846 wb_data[%d] = 0x%x  ", i, HI846_otp2.wb_data[i]);
		}
	}
	for(i=0;i<29;i++)
	{
	checksum += HI846_otp2.wb_data[i];
	}
	checksum = checksum % 0xFF + 1;
	
        if (checksum == HI846_otp2.wb_data[29])
	{
	...
	HI846_otp2.AF_FLAG = OTP_read_cmos_sensor(0x0CBA);
	LOG_INF("HI846 AF_FLAG = 0x%x\n",HI846_otp2.AF_FLAG);
	if (HI846_otp2.AF_FLAG == 0x01)
		af_start_addr = 0x0CBC;
	else if (HI846_otp2.AF_FLAG == 0x13)
		af_start_addr = 0x0CC2;
	else if (HI846_otp2.AF_FLAG == 0x37)
		af_start_addr = 0x0CC8;
	else
	{
                HI846_otp2.AF_FLAG = 0;
		af_start_addr = 0;
		LOG_INF("HI846 AF data invalid \n");
	}
	if (af_start_addr != 0) {
                HI846_otp2.AF_FLAG = 1;
		for (i = 0; i < 5; i++) {
			HI846_OTP_write_cmos_sensor(0x70a, ((af_start_addr+i) >> 8) & 0xff);	//start addr H
			HI846_OTP_write_cmos_sensor(0x70b, (af_start_addr+i) & 0xff);	//start addr L
			HI846_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
			HI846_otp2.af_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI846 af_data[%d] = 0x%x  ", i, HI846_otp2.af_data[i]);
		}
	}
        checksum = 0;
	for(i=0;i<4;i++)
	{
	checksum += HI846_otp2.af_data[i];
	}
	checksum = checksum % 0xFF + 1;
	
        if (checksum == HI846_otp2.af_data[4])
	{
	...
	HI846_otp2.LSC_FLAG = OTP_read_cmos_sensor(0x0235);
	LOG_INF("HI846 LSC_FLAG = 0x%x\n",HI846_otp2.LSC_FLAG);
	if (HI846_otp2.LSC_FLAG == 0x01)
		lsc_start_addr = 0x0236;
	else if (HI846_otp2.LSC_FLAG == 0x13)
		lsc_start_addr = 0x0599;
	else if (HI846_otp2.LSC_FLAG == 0x37)
	...
	if (lsc_start_addr != 0) {
		for (i = 0; i < 867; i++) {
			HI846_OTP_write_cmos_sensor(0x70a, ((lsc_start_addr+i) >> 8) & 0xff);	//start addr H
			HI846_OTP_write_cmos_sensor(0x70b, (lsc_start_addr+i) & 0xff);	//start addr L
			HI846_OTP_write_cmos_sensor(0x702, 0x01);	//single mode
			HI846_otp2.lsc_data[i] = read_cmos_sensor(0x708);	//otp data read
			LOG_INF("HI846 lsc_data[%d] = 0x%x  ", i, HI846_otp2.lsc_data[i]);
		}
	}
        checksum = 0;
	for(i=0;i<866;i++)
	{
	checksum += HI846_otp2.lsc_data[i];
	}
	checksum = checksum % 0xFF + 1;
	
        if (checksum == HI846_otp2.lsc_data[866])
	{
		LOG_INF("HI846_Sensor: LSC checksum PASS\n ");
		hi846_lsc_otp_flag = 1;
	}
        else
	{
		LOG_INF("HI846_Sensor: LSC checksum Fail\n ");
		hi846_lsc_otp_flag = 0;
	}
...
static bool HI846_otp_cali(void)
{
	...
	HI846_OTPSetting();
        ...
        if(info_otp_flag && awb_otp_flag && hi846_af_otp_flag && hi846_lsc_otp_flag)
            return true;
        else
            return false;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
			     ...
			     /*   if(HI846_otp_cali())
			        {
			              LOG_INF("[odin5g]hi846,check OTP pass\n");
			...
			UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_CHECK_SENSOR_ID:
		printk("bo_liu SENSOR_FEATURE_CHECK_SENSOR_ID 1\n");
	    get_imgsensor_id(feature_return_para_32);
		printk("bo_liu SENSOR_FEATURE_CHECK_SENSOR_ID 2\n");
	break;
...
UINT32 HI846ODIN5G_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
