...
#include "cxtvc12852_gc02m1_mipi_raw_sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = CXTVC12852_GC02M1_SENSOR_ID,
...
{
	pr_err("[%s] init_start\n", __func__);
	gc02m1_table_write_cmos_sensor(gc02m1_init_setting,
...
static void preview_setting(void)
{
	...
	gc02m1_table_write_cmos_sensor(gc02m1_preview_setting,
		sizeof(gc02m1_preview_setting)/sizeof(kal_uint16));
...
static void slim_video_setting(void)
{
	...
	gc02m1_table_write_cmos_sensor(gc02m1_slim_video_setting,
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			*sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1) + 8;
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc02m1_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_err("[gc02m1_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = (read_cmos_sensor(0xf0) << 8) | read_cmos_sensor(0xf1) + 8;
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("[gc02m1_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_err("[gc02m1_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	"GC02M1:CXT:2M:ASA2001252C1",
	2,////main:0 front:1 main2:2 front2:3  main3:4
	{
...
UINT32 CXTVC12852_GC02M1_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
