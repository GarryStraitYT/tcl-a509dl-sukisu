...
/***************************************************************************/
static ssize_t
aw3641e_get_reg(struct device *cd, struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t 
aw3641e_set_reg(struct device *cd,struct device_attribute *attr, const char *buf, size_t len)
{
	unsigned int databuf[1];

	if (sscanf(buf, "%d", &databuf[0]) == 1)
		flashlevel = databuf[0];
		//aw36518_write_reg(aw36518_i2c_client, databuf[0], databuf[1]);
	return len;
}

static DEVICE_ATTR(aw3641ereg, 0660, aw3641e_get_reg, aw3641e_set_reg);

static int aw3641e_create_sysfs(struct device *dev)
{
	int err;
...
static int aw3641e_enable(void)
{
	...
	// 	pinctrl_select_state(aw3641e_pinctrl, aw3641e_en_low);
	// 	mdelay(1);
	// 	aw3641e_ctrl_pulse(8+g_flash_duty);
	// }
	if(g_flash_duty == 1)
	{
		pinctrl_select_state(aw3641e_pinctrl, aw3641e_flash_low);
