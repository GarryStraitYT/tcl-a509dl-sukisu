...
// #ifdef VENDOR_EDIT
// shu5.zhang@ARCH, 2021/04/12, add for UXexpress KTC freq
...
extern struct themal_cpufreq_limit themal_limit[2];
#endif
// #endif /* VENDOR_EDIT */
...
// shu5.zahng@ARCH, 2021/04/12, add for GT
#if defined(CONFIG_TCL_UXEXPRESS) && defined(CONFIG_GRAPHICS_GT)
					cpu = p->mt_policy->cpu;
					ktc_target = find_boost_cpufreq(cpu, freqs.new);

					if (ktc_target != 0) {
						ktc_idx = _search_available_freq_idx(p, ktc_target, 0);

						if (ktc_idx < themal_limit[i].max_cpufreq_idx)
							ktc_idx = themal_limit[i].max_cpufreq_idx;

						if (ktc_idx > themal_limit[i].min_cpufreq_idx)
							ktc_idx = themal_limit[i].min_cpufreq_idx;

						if (ktc_idx > 15 || ktc_idx < 0)
							ktc_idx = 0;

						freqs.new =  cpu_dvfs_get_freq_by_idx(p, ktc_idx);
					}

					// trace_printk("%s freqs.new:%u  ktc_target:%u\n", __func__, freqs.new, ktc_target);
					// p->idx_opp_tbl = j;
...
// shu.zhang@tcl.com 2021/08/23 add for GT
#ifdef CONFIG_GRAPHICS_GT
int cpuhvfs_set_freq_quick(void)
{
	int i;
//	int j;
	struct mt_cpu_dvfs *p;
	...
	unsigned int ktc_target = 0;
	int	ktc_idx;
	//static int last_limit_idx[2] = {0, 0};
	...
	for_each_cpu_dvfs_only(i, p) {
		...
		if (p->mt_policy && p->mt_policy->governor && (p->mt_policy->cpu < 10) && (p->mt_policy->cpu >= 0)) {
			if (themal_limit[i].max_cpufreq_idx > 15 ||	themal_limit[i].max_cpufreq_idx < 0)
				themal_limit[i].max_cpufreq_idx = 0;

			if (themal_limit[i].min_cpufreq_idx > 15 ||themal_limit[i].min_cpufreq_idx < 0)
				themal_limit[i].min_cpufreq_idx = 15;

			freqs.old = cpu_dvfs_get_cur_freq(p);
			freqs.new = cpu_dvfs_get_freq_by_idx(p, 0);

			ktc_target = find_boost_cpufreq(p->mt_policy->cpu, freqs.new);

			if (ktc_target != 0) {

				ktc_idx = _search_available_freq_idx(p, ktc_target, 0);
				trace_printk("%s  ktc_idx=%d\n",__func__, ktc_idx);
				...
				freqs.new = cpu_dvfs_get_freq_by_idx(p, ktc_idx);
				if (p->idx_opp_tbl != _search_available_freq_idx(p, freqs.new, 0)) {
					p->idx_opp_tbl = _search_available_freq_idx(p, freqs.new, 0);
					cpufreq_freq_transition_begin(p->mt_policy, &freqs);
					cpufreq_freq_transition_end(p->mt_policy, &freqs, 0);
				}
