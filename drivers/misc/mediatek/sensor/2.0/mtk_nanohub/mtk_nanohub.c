...
// Begin modified by jianchuan.hu for add sensor info on 2022/07/13
...
static void save_sensorinfo(const int sensorType, const char *sensorName)
{
    switch (sensorType)
    {
    case SENSOR_TYPE_LIGHT:
        if(!strcmp(sensorName,"stk3a5x_l"))
            sprintf(lsensor_module_name, "STK33562-175:SENSORTEK:NA:AMY0001878C1");
        else
            snprintf(lsensor_module_name, 256, "%s", sensorName);
        break;
    case SENSOR_TYPE_PROXIMITY:
        if(!strcmp(sensorName,"stk3a5x_p"))
            sprintf(psensor_module_name, "STK33562-175:SENSORTEK:NA:AMY0001878C1");
        else
            snprintf(psensor_module_name, 256, "%s", sensorName);
        break;
    case SENSOR_TYPE_ACCELEROMETER:
        if(!strcmp(sensorName,"lsm6dso_acc"))
            sprintf(gsensor_module_name, "LSM6DSOETR3:ST:NA:AMY0001889C1");
        else if(!strcmp(sensorName,"icm4n607_acc"))
            sprintf(gsensor_module_name, "ICM-42607-P:TDK:NA:AMY0001985C1");
        else
            snprintf(gsensor_module_name, 256, "%s", sensorName);
        break;
    case SENSOR_TYPE_GYROSCOPE:
        if(!strcmp(sensorName,"lsm6dso_gyro"))
            sprintf(gyroscope_module_name, "LSM6DSOETR3:ST:NA:AMY0001889C1");
        else if(!strcmp(sensorName,"icm4n607_gyro"))
            sprintf(gyroscope_module_name, "ICM-42607-P:TDK:NA:AMY0001985C1");
        else
            snprintf(gyroscope_module_name, 256, "%s", sensorName);
        break;
    case SENSOR_TYPE_MAGNETIC_FIELD:
        if(!strcmp(sensorName,"mmc5603"))
            sprintf(compass_module_name, "MMC5603NJ:MEMSIC:NA:AMY0001890C1");
        else
            snprintf(compass_module_name, 256, "%s", sensorName);
        break;
    case SENSOR_TYPE_PRESSURE:
        if(!strcmp(sensorName,"spl07"))
            sprintf(barometer_module_name, "SPL07-001:GOER:NA:AMY0002102C1");
        else if(!strcmp(sensorName,"icp201xx"))
            sprintf(barometer_module_name, "ICP-20100-B:TDK:NA:AMY0002167C1");
        else if(!strcmp(sensorName,"lps22hh"))
            sprintf(barometer_module_name, "LPS22HHTR:ST:NA:AMY0001671C1");
        else
            snprintf(barometer_module_name, 256, "%s", sensorName);
        break;
    default:
        break;
    }
}
// End modified by jianchuan.hu for add sensor info on 2022/07/13
		...
		bool status)
{
	...
	if (!status)
		...
		// Begin modified by jianchuan.hu for add sensor info on 2022/07/13
		save_sensorinfo(p->sensorType, p->name);
		// End modified by jianchuan.hu for add sensor info on 2022/07/13
	}
}
