...
int port_dev_open(struct inode *inode, struct file *file)
{
	...
	CCCI_NORMAL_LOG(md_id, CHAR,
		...
		current->comm);
	if (port->name && !strcmp(port->name, "ccci_iqi")) {
		int md_state = ccci_fsm_get_md_state(md_id);
		CCCI_NORMAL_LOG(md_id, CHAR,
				"try open %s by %s with modem state: %d \n",
				port->name, current->comm, md_state);
		if (md_state < READY) {
			CCCI_NORMAL_LOG(md_id, CHAR,
					"Modem not ready for %s \n",
					port->name);
			return -EAGAIN;
		} else if (md_state > READY) {
			CCCI_NORMAL_LOG(md_id, CHAR,
					"Modem Crash Detected! for %s \n",
					port->name);
			return -ENODEV;
		}
	}
...
READ_START:
	...
	if (port->name && !strcmp(port->name, "ccci_iqi")) {
		...
		CCCI_NORMAL_LOG(
			md_id, CHAR,
			"Special process for IQI port:%s by %s with modem state: %d \n",
			port->name, current->comm, md_state);
		if (md_state < READY) {
			CCCI_NORMAL_LOG(md_id, CHAR,
					"Modem not ready for %s to read! \n",
					port->name);
			return -ENODEV;
		} else if (md_state > READY) {
			CCCI_NORMAL_LOG(md_id, CHAR,
