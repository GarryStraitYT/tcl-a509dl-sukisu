	...
	struct ccci_fsm_command *cmd, enum CCCI_EE_REASON reason)
{
...
#ifdef CONFIG_HELAEYE_BSP_MODEMCRASH_ON
		md_ccci_lasterrcode=EXCEPTION_MD_NO_RESPONSE;
#endif
	...
	case EXCEPTION_WDT:
		...
		CCCI_ERROR_LOG(ctl->md_id, FSM,
			"MD_WDT!\n");
#ifdef CONFIG_HELAEYE_BSP_MODEMCRASH_ON
		md_ccci_lasterrcode=EXCEPTION_WDT;
#endif
	...
	case EXCEPTION_EE:
		 ...
		 */
#ifdef CONFIG_HELAEYE_BSP_MODEMCRASH_ON
		md_ccci_lasterrcode=EXCEPTION_EE;
#endif
...
#ifdef CONFIG_HELAEYE_BSP_MODEMCRASH_ON
		md_ccci_lasterrcode=EXCEPTION_HS2_TIMEOUT;
#endif
	}
	else{
		fsm_routine_exception(ctl, NULL, EXCEPTION_HS1_TIMEOUT);
#ifdef CONFIG_HELAEYE_BSP_MODEMCRASH_ON
		md_ccci_lasterrcode=EXCEPTION_HS1_TIMEOUT;
#endif
  }
	fsm_finish_command(ctl, cmd, -1);
