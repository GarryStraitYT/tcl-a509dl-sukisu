...
static inline int AW8601xAF_SetAlgoMode(void)
{
	...
	if (strcmp(CamNameB_module_name, "S5K3L6:SHN:13M:ASA1300105C1") == 0 ){
		algo_cfg[0] = (unsigned char)(0x01);
		algo_cfg[1] = (unsigned char)(0x8a);
		ret = AW8601xAF_WriteRegs(AW_REG_CODE_H, algo_cfg,AW_DATA_BYTE_2);

		if (ret < 0) {
			AW_LOGE("Set init pos error, ret: %d", ret);
			return -EAGAIN;
		}
	}
	...
	if (*g_pAF_Opened == 2) {
		...
		if (g_motor_type == AW_BOTTOM_MOTOR){
			...
			while (g_u4CurrPosition > 50) {
				...
				AW8601xAF_SetPos(g_u4CurrPosition - af_step);
