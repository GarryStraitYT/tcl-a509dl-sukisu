...
static inline void AW86014AF_SetAdvance(void)
{
	...
	AW86014AF_WriteReg(0x06, 0x84);
	AW_LOG_INF("Set AW_ADVANCE_DIRECT\n");
#elif defined(AW_ADVANCE_VRC)
	AW86014AF_WriteReg(0x06, 0x84); //Set Ringing ON and VRC3
	AW86014AF_WriteReg(0x07, 0x01); // Set PRESC[1:0]=01
	AW86014AF_WriteReg(0x08, 0x15); // Set VRCT[6:0]=001 0101

	//Begin deleted by xiaoming-zhong for [Defect][11684657][lower af power on noise sound] on 2021/12/24
	/*AW86014AF_WriteReg(0x03, 0x01);
	AW86014AF_WriteReg(0x04, 0x90); //Set Ringing ON and VRC3*/
	//End deleted by xiaoming-zhong for [Defect][11684657][lower af power on noise sound] on 2021/12/24
...
static inline int
AW86014AF_GetVCMInfo(__user struct stAF_MotorInfo *pstMotorInfo)
{
...
int AW86014AF_Release(struct inode *a_pInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
		...
		if (g_u4CurrPosition > g_u4AF_INF &&
		    g_u4CurrPosition <= g_u4AF_MACRO) {
