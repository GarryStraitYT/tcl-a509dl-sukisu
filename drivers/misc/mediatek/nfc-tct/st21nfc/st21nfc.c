...
/* added by weiqiang for check I2C is ok begin*/
static int st21nfc_hw_detect(struct st21nfc_device *pDev)
{
	int  ret;
	size_t count;
	char tmp[255];

	if (pDev == NULL || pDev->gpiod_reset == 0)
	{
		pr_err("%s error(-EINVAL)\n", __func__);
		return -EINVAL;
	}

	gpiod_set_value(pDev->gpiod_reset, 0);
	msleep(20);
	gpiod_set_value(pDev->gpiod_reset, 1);
	msleep(20);
	gpiod_set_value(pDev->gpiod_reset, 0);
	msleep(20);
	gpiod_set_value(pDev->gpiod_reset, 1);
	...
	ret = i2c_master_recv(pDev->client, tmp, 3);
    if (ret < 0)
    {
		pr_err("%s: i2c_master_recv 3 returned %d\n", __func__, ret);
		goto out;
	}
	if (ret > 3)
	{
	...
	pr_info("%s: success(%d) -> %x %x %x\n", __func__, ret, tmp[0], tmp[1], tmp[2]);

	count = tmp[2];
	ret = i2c_master_recv(pDev->client, tmp, count);
    if (ret < 0)
    {
		pr_err("%s: i2c_master_recv %zu returned %d\n", __func__, count, ret);
		goto out;
	}
	if (ret > count)
	{
...
/* added by weiqiang for check I2C is ok end*/

//Begin modefied by weiqiang.zhou for T on 2020.10.29
int st21nfc_check_support()
{
    int retval = 0;
    int boardid_value = 0;
    int boardid_gpio = 0;
    ...
    if (!node) {
        pr_err("unable to find boardid node from dts");
        return -1;
    }
    retval = of_get_named_gpio(node, "st21nfc,boardid", 0);
    if ( !gpio_is_valid(retval)) {
        pr_err("unable to find boardid gpio from dts");
        return -1;
    }
    boardid_gpio = retval;
    pr_info("NFC-BOARDID GPIO=%d\n", boardid_gpio);

    retval = gpio_request( boardid_gpio, "st21nfc-boardid-gpio");
    if( retval) {
        pr_err("unable to request boardid gpio");
        return -1;
    }
    retval = gpio_direction_input(boardid_gpio);
    boardid_value = gpio_get_value(boardid_gpio);
    pr_info("boardid, gpio=%d, value=%d\n", boardid_gpio, boardid_value);

    gpio_free(boardid_gpio);
    return boardid_value;
}
//End modefied by weiqiang.zhou for T on 2020.10.29
/*
...
// QCOM and MTK54 use standard GPIO definition
    ...
    /* added by weiqiang for check I2C is ok begin*/
    if (st21nfc_hw_detect(st21nfc_dev) != 0)
    {
	    pr_info("%s hardware detect failed\n", __func__);
	    goto err_hw_detect;
    }
    /* added by weiqiang for check I2C is ok end*/
...
//modefied nfc module by weiqiang.zhou for Task 10693895 on 2021-01-25 for remove NFC chip check
#ifndef CONFIG_NFC_ST21NFCD_TCT
/*Added by weiqiang.zhou for T on 2020.10.29 Begin*/
  if ( st21nfc_check_support() == 1) {
      pr_info("There is st21nfc chip at present\n");
  } else {
      pr_info("There is no st21nfc chip at present\n");
      return 0;
  }
/*Added by weiqiang.zhou for T on 2020.10.29 Begin*/
#endif
