...
/* Begin add by jin.wang for task 2064 on 2021-10-26 */
...
#define PHY_MODE_RG_DP_PULLDOWN_SET 3
#define PHY_MODE_RG_DP_PULLDOWN_CLR 4
#define PHY_MODE_RG_DM_PULLDOWN_SET 5
#define PHY_MODE_RG_DM_PULLDOWN_CLR 6
...
void Charger_Detect_Release_Pulldown(void)
{
	...
	phy_set_mode_ext(glue->phy, PHY_MODE_USB_DEVICE, PHY_MODE_RG_DP_PULLDOWN_SET);
	phy_set_mode_ext(glue->phy, PHY_MODE_USB_DEVICE, PHY_MODE_RG_DM_PULLDOWN_SET);
	udelay(1);
	usb_prepare_enable_clock(false);
	DBG(0, "%s\n", __func__);
}
...
/* Begin del by jin.wang for jira 2064 at 2021-10-26 */
#if !IS_ENABLED(CONFIG_MUSB_OLD_ARCH)
...
/* End del by jin.wang */
	...
	DBG(0, "begin <%d,%d>,<%d,%d,%d,%d>\n",
			...
			real_enable, real_disable);
/* Begin mod by jin.wang for task 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	if (musb->power == true) {
		pr_err("%s: skip for power true\n", __func__);
		return;
	}
#else
	if (musb->power == true)
...
/* Begin modified by jin.wang for task 2064 on 2021-10-26 */
#if IS_ENABLED(CONFIG_MUSB_OLD_ARCH)
...
/* Begin added by jin.wang for task 11356632 on 2021-07-22 */
#if 0
...
#endif
/* End added by jin.wang for task 11356632 on 2021-07-22 */
...
/* Begin mod by jin.wang for task 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_MUSB_OLD_ARCH) || IS_ENABLED(CONFIG_TCT_CHARGER)
...
/* Begin mod by yan.gong for androidT on 2022-4-12 */
