...
static int musb_gadget_dequeue(struct usb_ep *ep, struct usb_request *request)
{
	...
	//Added by baiwei.peng to fix defect JTVZ-6290 by mtk patch on 2021/02/18
	...
	if (musb_ep->req_list.next != &req->list || musb_ep->busy)
		...
		/* Modify-start by baiwei.peng to fix defect JTVZ-6290 by mtk patch on 2021/02/18 */
		mtk_qmu_enable(musb,
				musb_ep->hw_ep->epnum,
				(musb_ep->is_in ? TXQ : RXQ));
		musb_g_giveback(musb_ep, request, -ECONNRESET);
		...
		//		musb_ep->hw_ep->epnum,
		//		(musb_ep->is_in ? TXQ : RXQ));
		/* Modify-end by baiwei.peng to fix defect JTVZ-6290 by mtk patch on 2021/02/18 */
	}
#else
