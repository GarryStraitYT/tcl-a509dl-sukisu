...
//Added by baiwei.peng to disable mt6360 long press power-key reset on 2021/09/09
#define MT6360_DISABLE_LONG_PRESS_REBOOT
...
{
	int i, ret = 0;
	const int max_wait_cnt = 2; //Modify by qiuguangliang for TITAN-4086 on 20220808
...
/* [BSP]Begin added by bitao.xiong for ENCORETF-8486 on 2022/11/16 */
static inline bool mt6360_is_kpoc(struct mt6360_pmu_chg_info *mpci)
{
	...
	return (boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT || boot_mode == LOW_POWER_OFF_CHARGING_BOOT);
}

static int mt6360_pull_up_dp(struct mt6360_pmu_chg_info *mpci)
{
	int ret = 0;
	/* Set Pull up D+ to 0.6V for CDP in kpoc */
	ret = mt6360_pmu_reg_update_bits(mpci->mpi,
					MT6360_PMU_DPDM_CTRL,
					0xff,
					0xf5);
...
/* [BSP]End added by bitao.xiong for ENCORETF-8486 on 2022/11/16 */
#endif
	...
	if (!attach) {
		...
		if (ret < 0)
			dev_notice(mpci->dev, "%s: disable chgdet fail\n",
				   __func__);
/* [BSP]Begin modified by bitao.xiong for ENCORETF-8486 on 2022/11/16 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	} else if (mpci->chg_type != STANDARD_CHARGER) {
		if (mt6360_is_kpoc(mpci) && mpci->chg_type == CHARGING_HOST) {
			dev_info(mpci->dev, "%s:Pull up D+ to 0.6V for CDP in KPOC\n", __func__);
			mt6360_pull_up_dp(mpci);
		} else {
		...
		mt6360_set_usbsw_state(mpci, MT6360_USBSW_USB);
#endif
/* [BSP]End modified by bitao.xiong for ENCORETF-8486 on 2022/11/16 */
	if (!inform_psy)
		return ret;
	ret = mt6360_psy_online_changed(mpci);
	if (ret < 0)
...
static int mt6360_dump_registers(struct charger_device *chg_dev)
{
	...
	ret = mt6360_pmu_reg_read(mpci->mpi, MT6360_PMU_DPDM_CTRL);
	if (ret < 0)
		return ret;
	dpdm_ctl = ret;
	...
	ret = mt6360_pmu_reg_read(mpci->mpi, MT6360_PMU_CHG_CTRL17);
	if (ret < 0)
		return ret;
	chg_ctrl17 = ret;
	...
	dev_info(mpci->dev, "%s: CHG_CTRL1 = 0x%02X, CHG_CTRL2 = 0x%02X, CHG_CTRL3 = 0x%02X, CHG_CTRL17 = 0x%02X, dpdm_ctl = 0x%02X\n",
		 __func__, chg_ctrl[0], chg_ctrl[1], chg_ctrl3, chg_ctrl17, dpdm_ctl);
	return 0;
}
...
/* Add-start by baiwei.peng to disable mt6360 long press power-key reset on 2021/09/09 */
#ifdef MT6360_DISABLE_LONG_PRESS_REBOOT
	ret = mt6360_pmu_reg_set_bits(mpci->mpi, MT6360_PMU_CHG_PUMP, 0xE0);
	if (ret < 0) {
		dev_err(mpci->dev, "%s: mt6360 disable long press reset failed\n", __func__);
	}
#endif
/* Add-end by baiwei.peng to disable mt6360 long press power-key reset on 2021/09/09 */
