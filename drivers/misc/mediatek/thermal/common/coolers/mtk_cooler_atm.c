...
// shu5.zhang@ARCH, 2021/04/12, add for GT
		...
		ktc_target = find_boost_cpufreq(0, 0);
		if (ktc_target != 0)
			adaptive_limit[print_cunt][1] = tscpu_get_curr_temp() - 5000;
		else
...
static int P_adaptive(int total_power, unsigned int gpu_loading)
{
	...
	pr_info("[tcl_thermal_cooler-atm] %s cpu_power=%d gpu_power=%d\n", __func__, cpu_power, gpu_power);
#endif
...
static int __phpb_dynamic_theta(int max_theta)
{
	...
	g_theta = theta;
#ifdef TCL_THERMAL_DEBUG
	pr_info("[tcl_thermal_cooler-atm] %s g_theta=%d tj_trip=%d TARGET_TJ=%d MAX_TARGET_TJ=%d max_theta=%d phpb_theta_min=%d\n",
		__func__, g_theta, tj_trip, TARGET_TJ, MAX_TARGET_TJ, max_theta, phpb_theta_min);
#endif
	return theta;
}
...
#ifdef TCL_THERMAL_DEBUG
	pr_info("[tcl_thermal_cooler-atm] %s delta_power=%d \n", __func__, delta_power);
#endif
	return delta_power;
}
...
#ifdef TCL_THERMAL_DEBUG
	pr_info("[tcl_thermal_cooler] %s thp max_cpu_pwr %d %d\n",
			__func__, thp_max_cpu_power, MAXIMUM_CPU_POWER);
#endif
#endif
...
#ifdef TCL_THERMAL_DEBUG
	pr_info("[tcl_thermal_cooler-atm] %s [%d] =%d, ttj=%d\n",
	 			__func__, (cdev->type[13] - '0'), state, ttj);
#endif
...
// shu5.zhang@ARCH, 2021/04/12, add for GT
	...
	if (ktc_target != 0)
		atm_curr_maxtj = tscpu_get_curr_temp() - 5000;
	else
