				...
				unsigned long state)
{
	...
	pr_info("[tcl_thermal_cooler-mutt] %s %s state=%d\n", __func__, cdev->type, state);
#endif
...
struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	if (ret != 0) {
		...
		if (target_lv < 0)
		{
			...
			__func__, clmutt_data.cooler_param[type].name);
		#ifdef TCL_THERMAL_DEBUG
			pr_info("[tcl_thermal_cooler-mutt] %s  %s:cancel reduce tx pwr!\n", __func__, clmutt_data.cooler_param[type].name);
		#endif
		}
		else
		{
			...
			clmutt_data.cooler_param[type].tx_pwr_db[target_lv]);
		#ifdef TCL_THERMAL_DEBUG
			pr_info("[tcl_thermal_cooler-mutt] %s  %s:reduce tx %d dBm\n",__func__,
				clmutt_data.cooler_param[type].name,
				clmutt_data.cooler_param[type].tx_pwr_db[target_lv]);
		#endif
		}
