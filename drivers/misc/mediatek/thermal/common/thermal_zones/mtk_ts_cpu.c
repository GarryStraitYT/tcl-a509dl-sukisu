...
static int fixed_cpu_tmp = -1;
module_param(fixed_cpu_tmp, int, 0644);
#endif
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	if(cnt % 5) {
		cnt = 0;
		pr_info("[tcl_thermal_zone-cpu] %s t_cpu=%d \n", __func__, curr_temp);
		pr_notice("[tcl_thermal_zone-cpu]%s fixed_cpu_tmp=%d\n", __func__, fixed_cpu_tmp);
	}
#endif
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	if (fixed_cpu_tmp >0) {
		curr_temp = fixed_cpu_tmp;
	}
#endif
