...
struct akm09911_i2c_data {
	...
	bool flush;
	bool enable;
	//Begin Added by weizhi.chen for Task 6377595 A5X on 2018/06/08
	atomic_t suspend;
	//End Added by weizhi.chen for Task 6377595 A5X on 2018/06/08
};
/*----------------------------------------------------------------------------*/
#ifdef CONFIG_OF
...
static ssize_t msensor_show(struct device* dev,struct device_attribute *attr, char *buf)
{
	char strbuf[AKM09911_BUFSIZE];
...
static DEVICE_ATTR(msensor, S_IWUSR | S_IRUGO, msensor_show, NULL);
static int create_chipinfo_node(void)
{
       ...
       err=device_create_file(chipinfo, &dev_attr_msensor);
       if (err){
               MAGN_PR_ERR("Failed to create device file(%s)!\n", dev_attr_msensor.attr.name);
               return -ENODEV;
       }
       return err;
}
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err) {
		MAGN_PR_ERR("create chip info  %d\n", err);
		goto exit_sysfs_create_group_failed;
	}
