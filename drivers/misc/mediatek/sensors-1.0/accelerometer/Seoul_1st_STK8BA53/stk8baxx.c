...
#define SENSOR_DIRECTION_DEBUG 1
...
#define ACC_LOG(fmt, args...)   pr_debug(KERN_ERR STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
...
#define STK_ACC_LOG(fmt, args...)   pr_debug(KERN_ERR STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
...
/*Add-Begin by zhikui.li@tcl.com,Change the smd threshold*/
/* STK8BAXX_REG_SLOPETHD */
#define STK8BAXX_SLOPETHD_DEF                0x88
/*Add-end by zhikui.li@tcl.com,Change the smd threshold*/
...
/*Add-Begin by zhikui.li@tcl.com,Remove the poll timer*/
#if 0
#ifndef INTERRUPT_MODE /* polling mode */
        hrtimer_start(&stk->accel_timer, stk->poll_delay, HRTIMER_MODE_REL);
#endif /* no INTERRUPT_MODE */
#endif
/*Add-end by zhikui.li@tcl.com,Remove the poll timer*/
#ifdef STK_CHECK_CODE
        stk->cc_count = 0;
...
/*Add-Begin by zhikui.li@tcl.com,Solve the smd interrupt problem*/
    u8 data = 0;
	if (enable) {
		stk->sig_motion_enable = 1;
        stk8baxx_reg_read(stk, STK8BAXX_REG_INTSTS1, 0, &data);
        stk_reset_latched_int(stk);
        msleep(30);
/*Add-Begin by zhikui.li@tcl.com,Solve the smd interrupt problem*/
		enable_irq(stk->irq1);
...
/*Add-Begin by zhikui.li@tcl.com,Solve the smd interrupt problem*/
       if(stk->sig_motion_enable != enable){
             STK_ACC_LOG("disable smd irq\n");
		disable_irq_nosync(stk->irq1);
        }
		stk->sig_motion_enable = 0;
/*Add-end by zhikui.li@tcl.com,Solve the smd interrupt problem*/
   ...
   stk = i2c_get_clientdata(client); 
    ...
    switch (cmd)
    {
        ...
        case GSENSOR_IOCTL_SET_CALI:
            ...
            error = 0;
#endif
/*Add-end by zhikui.li@tcl.com,Remove mtk gsensor calibration*/
            if (error)
            {
                STK_ACC_ERR("failed to stk_write_cali_to_file, error=%d", error);
                error = -EFAULT;
