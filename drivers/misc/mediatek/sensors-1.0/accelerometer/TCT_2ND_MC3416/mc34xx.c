...
#define STK_ACC_TAG                 "[MC3XXXX]"
#define ACC_PR_ERR(fmt, args...)    printk(KERN_ERR STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
//#define ACC_ERR(fmt, args...)   printk(KERN_INFO STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
...
static int MC3XXX_ValidateSensorIC(unsigned char *pbPCode, unsigned char *pbHwID)
{
	...
	else if ((0xA0 <= *pbHwID) && (*pbHwID <= 0xAF))
	    {
	        *pbPCode = (*pbPCode & 0xF1);
		*pbHwID = (*pbHwID & 0xF0);

	      if ((MC3XXX_PCODE_3416 == *pbPCode) || (MC3XXX_PCODE_3436 == *pbPCode))
			return (MC3XXX_RETCODE_SUCCESS);
	    }
...
static void MC3XXX_SetSampleRate(struct i2c_client *pt_i2c_client)
{
	...
	if (IS_MCFM12() || IS_MCFM3X()) {
		...
		ACC_LOG("[%s MENSA] REG(0x08) = 0x%02X\n", __func__, _baDataBuf[1]);			
	}
	else
		_baDataBuf[0] = 0x00;
...
static void MC3XXX_LowPassFilter(struct i2c_client *mc3XXX_client)
{
	...
	if (IS_MENSA())
	{
		res = MC3XXX_i2c_read_block(mc3XXX_client, MC3XXX_REG_RANGE_CONTROL, _baDataBuf,1);
		ACC_LOG("[%s MENSA] Read REG(0x20) = 0x%02X\n", __func__, _baDataBuf[0]);			
		_baDataBuf[0] = ((_baDataBuf[0] & 0xF0) | 0x09);// 128Hz @ 2048 Hz ODR	
		res = MC3XXX_i2c_write_block(mc3XXX_client, MC3XXX_REG_RANGE_CONTROL, _baDataBuf,1);
		if (res < 0)
			ACC_LOG("[%s] MENSA LPF failed.\n",__func__);
		else
			ACC_LOG("[%s] Write REG(0x20) = 0x%02X,MENSA LPF enable success.\n",__func__,_baDataBuf[0]);
	}
}
...
//Begin added by lanying.he for significant
static ssize_t regval_show(struct device_driver *ddri, char *buf)
{
    int ret = 0;
    u8 rbuf[0x13]={0};
    u8 cnt = 0;
...
static ssize_t regval_store(struct device_driver *ddri, const char *buf, size_t count)
{
     int reg;
     int values;
     int res;
...
static DRIVER_ATTR_RW(regval);//added by lanying.he for significant
#endif
...
static void MC3XXX_reset(struct i2c_client *client)
{
	...
	if((s_bPCODE==MC3XXX_PCODE_3416) || (s_bPCODE==MC3XXX_PCODE_3436))
		return;
...
//Begin modified by yan.gong for XR5854058 on 2018/1/8
        mc3xxx_mutex_unlock();
...
#endif
//End modified by yan.gong for XR5854058 on 2018/1/8
...
static int mc3xxx_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek,mc34xx");
    if(node!=NULL)
    {
   ...
   //Begin added by lanying.he for gsensor calibration
	...
	err = create_chipinfo_node();
	if (err) {
		ACC_PR_ERR("create chipinfo node = %d\n", err);
		goto exit_create_attr_failed;
	}
...
static void eintWork(struct work_struct *work)
{
	...
	if(IS_MENSA())	
	{
		if(MC3XXX_i2c_read_block(obj->client, 0x14, databuf, 0x01)<0)
	    {
	        ACC_PR_ERR("%s read MC3416 register err!\n", __func__);
	        goto eint_work_exit;
	    }
	}
	else
	{
	    ...
	    {
	        ACC_PR_ERR("%s read mc3413 register err!\n", __func__);
	        goto eint_work_exit;
	    }
	}
...
static int  mc3xxx_local_init(void)
{
	...
	if (s_nInitFlag == MC3XXX_INIT_FAIL)
		return -1;
#ifdef CONFIG_MTK_ROBUST      
    sprintf(gsensor_module_name,"MC3416-P:mcube:NA");//add by lanying.he for gsensor deviceinfo
#endif    
	return 0;
}
