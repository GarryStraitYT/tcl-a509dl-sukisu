...
#define STK_ACC_DRIVER_VERSION "404.1.5" /* kernel-version:major-number:minor-number */
...
#define STK_ACC_FUN(f)              pr_debug(KERN_INFO STK_ACC_TAG" %s\n", __FUNCTION__)
#define ACC_LOG(fmt, args...)       pr_debug(KERN_INFO STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
...
#define STK_ACC_LOG(fmt, args...)   pr_debug(KERN_INFO STK_ACC_TAG" %s %d: "fmt"\n", __FUNCTION__, __LINE__, ##args)
...
#define STK8BAXX_SLOPETHD_DEF                0x85
 ...
 * @return: ssize_t
 */
static ssize_t stk_layout_store(struct device_driver *ddri, const char *buf, size_t count)
{
    int direction = 0;
    int ret = 0;
 ...
 * @return: ssize_t
 */
static ssize_t stk_layout_show(struct device_driver *ddri, char *buf)
{
    struct stk8baxx_data *stk = stk_data;
   ...
   stk = i2c_get_clientdata(client); 
    ...
    switch (cmd)
    {
        ...
        case GSENSOR_IOCTL_SET_CALI:
            ...
            xyz[stk->cvt.map[STK_AXIS_X]] = (u8)(accelOffset[STK_AXIS_X] / stk->cvt.sign[STK_AXIS_X]);
            xyz[stk->cvt.map[STK_AXIS_Y]] = (u8)(accelOffset[STK_AXIS_Y] / stk->cvt.sign[STK_AXIS_Y]);
            xyz[stk->cvt.map[STK_AXIS_Z]] = (u8)(accelOffset[STK_AXIS_Z] / stk->cvt.sign[STK_AXIS_Z]);
