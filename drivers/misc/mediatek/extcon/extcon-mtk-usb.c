...
static int mtk_usb_extcon_probe(struct platform_device *pdev)
{
	...
	/* vbus */
/* Begin modified by bitao.xiong for BORATF-3336 on 2022/10/14 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	#if IS_ENABLED(CONFIG_MT6370_PMU_CHARGER) || IS_ENABLED(CONFIG_MT6360_PMU_CHARGER)
	extcon->vbus = devm_regulator_get(dev, "vbus");
	#else
	extcon->vbus = regulator_get(NULL, "usb-otg-vbus");
	#endif
#else
	extcon->vbus = devm_regulator_get(dev, "vbus");
#endif
/* End modified by bitao.xiong for BORATF-3336 on 2022/10/14 */
