...
//    int ret = 0;
	...
	if (fsa_priv->plug_state) {
		...
		dev_info(dev, "%s: aa reg[0x%x]=0x%x.\n", __func__, FSA4480_SWITCH_SETTINGS, switch_status);
		        regmap_read(fsa_priv->regmap, FSA4480_SWITCH_CONTROL, &switch_status);
		dev_info(dev, "%s: bb reg[0x%x]=0x%x.\n", __func__, FSA4480_SWITCH_CONTROL, switch_status);
		...
		if (gpio_is_valid(fsa_priv->hs_det_pin)) {
           ...
           /* if ((20630 == get_project()) || (20631 == get_project()) || (20632 == get_project())) {
			...
			//gpio_direction_output(fsa_priv->hs_det_pin, 0);
			state = gpio_get_value(fsa_priv->hs_det_pin);
			dev_info(dev, "%s: hly after hs_det_pin state = %d.\n", __func__, state);
		}
       #endif
		/* deactivate switches */
 ...
 *
 * @nb - notifier block of fsa4480fi
