...
static int bq2589x_inform_charger_type(struct bq2589x *bq)
{
	...
	if (bq->chg_type != POWER_SUPPLY_TYPE_UNKNOWN)
		propval.intval = 1;
	else
		propval.intval = 0;
...
static irqreturn_t bq2589x_irq_handler(int irq, void *data)
{
	...
	enum power_supply_type prev_chg_type;
...
static int bq2589x_enable_pumpx(struct bq2589x *bq)
{
	const u8 val = BQ2589X_PUMPX_ENABLE << BQ2589X_EN_PUMPX_SHIFT;

	return bq2589x_update_bits(bq, BQ2589X_REG_04, BQ2589X_EN_PUMPX_MASK,
				   val);
}

/*
static int bq2589x_disable_pumpx(struct bq2589x *bq)
{
	const u8 val = BQ2589X_PUMPX_DISABLE << BQ2589X_EN_PUMPX_SHIFT;

	return bq2589x_update_bits(bq, BQ2589X_REG_04, BQ2589X_EN_PUMPX_MASK,
				   val);
}*/

static void bq2589x_pumpx_up(struct bq2589x *bq, bool is_increase)
{
	u8 val = 0;
	/* Input current limit = 500 mA, changes after PE+ detection */
	bq2589x_set_input_current_limit(bq, 500);
	/* CC mode current = 2048 mA */
	bq2589x_set_chargecurrent(bq, 2048);
	bq2589x_enable_charger(bq);
	bq2589x_enable_pumpx(bq);

	if (is_increase) {
		val = BQ2589X_PUMPX_UP << BQ2589X_PUMPX_UP_SHIFT;
		bq2589x_update_bits(bq, BQ2589X_REG_09, BQ2589X_PUMPX_UP_MASK,
				   val);
	} else {
		val = BQ2589X_PUMPX_DOWN << BQ2589X_PUMPX_DOWN_SHIFT;
		bq2589x_update_bits(bq, BQ2589X_REG_09, BQ2589X_PUMPX_DOWN_MASK,
				   val);		
	}
	/* Input current limit = 500 mA, changes after PE+ detection */
	bq2589x_set_input_current_limit(bq, 500);
...
static int bq2589x_send_ta_current_pattern(struct charger_device *chg_dev,
                                          bool is_increase)
{
	struct bq2589x *bq = dev_get_drvdata(&chg_dev->dev);

    bq2589x_pumpx_up(bq, is_increase);
...
#define PEOFFTIME 40
#define PEONTIME 90
static int bq2589x_set_ta20_current_pattern(struct charger_device *chg_dev,
                                            u32 chr_vol)
{
	int value;
    int i, j = 0;
    int flag;
	struct bq2589x *bq = dev_get_drvdata(&chg_dev->dev);
	const u8 val = BQ2589X_FORCE_VINDPM_ENABLE << BQ2589X_FORCE_VINDPM_SHIFT;
...
static int bq2589x_reset_ta(struct charger_device *chg_dev)
{
	struct bq2589x *bq = dev_get_drvdata(&chg_dev->dev);
	const u8 val = BQ2589X_FORCE_VINDPM_ENABLE << BQ2589X_FORCE_VINDPM_SHIFT;
	bq2589x_update_bits(bq, BQ2589X_REG_0D, BQ2589X_FORCE_VINDPM_MASK,
...
static int bq2589x_enable_cable_drop_comp(struct charger_device *chg_dev, bool en)
{
    pr_err("%s: en = %d\n", __func__, en);
    return bq2589x_set_ta20_current_pattern(chg_dev, 21000000);
}
...
static struct charger_ops bq2589x_chg_ops = {
	...
	/* PE+/PE+20 */
	.send_ta_current_pattern = bq2589x_send_ta_current_pattern,
	.set_pe20_efficiency_table = bq2589x_set_pep20_efficiency_table,
	.send_ta20_current_pattern = bq2589x_set_ta20_current_pattern,
	.reset_ta = bq2589x_reset_ta,
	.enable_cable_drop_comp = bq2589x_enable_cable_drop_comp,
