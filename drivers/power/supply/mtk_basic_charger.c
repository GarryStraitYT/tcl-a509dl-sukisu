	...
	struct chg_limit_setting *setting)
{
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			if (info->sw_jeita.sm == TEMP_T0_TO_T1 || info->sw_jeita.sm == TEMP_BELOW_T0 ||
				info->sw_jeita.sm ==TEMP_ABOVE_T4) {
				pdata->charging_current_limit = 0;
			} else if(info->sw_jeita.sm == TEMP_T1_TO_T2) {
				pdata->charging_current_limit = min(pdata->charging_current_limit, info->data.jeita_temp_t1_to_t2_cc);
			} else if (info->sw_jeita.sm == TEMP_T3_TO_T4) {
				pdata->charging_current_limit = min(pdata->charging_current_limit, info->data.jeita_temp_t3_to_t4_cc);
			}
			chr_err("[%s]sw_jeita.sm:%d  input_curren_limit:%d  charging_current_limit:%d\n", __func__,
