...
#endif
/* End del by jin.wang */
>>>>>>> buffalo_s0.mp1.rc-V1_hz_rom_202110271526
...
static void mtk_charger_check_nonstandard_charger(struct mtk_charger *info, bool is_charger_on)
{
	...
	chr_err("%s, chg_type=%d, %d, %d, %d, %d\n", __func__, get_charger_type(info),
	chg_data->thermal_input_current_limit, chg_data->thermal_charging_current_limit, info->enable_sw_jeita,info->sw_jeita.sm);

	if (get_charger_type(info) == POWER_SUPPLY_TYPE_USB_FLOAT) {
		adapter_type = UN_COMPLICAN_BC1P2;
		info->nonstand_chg_type = adapter_type;
		...
		if (get_charger_type(info) == POWER_SUPPLY_TYPE_USB_DCP) {
			...
			if (chg_data->thermal_input_current_limit == -1 &&
					...
					chr_err("%s, time_begin=%d, input_avg_current=%d, threshold=%d \n", __func__, time_begin,
					info->input_avg_current,info->data.ac_charger_current * 85 / 100);
					if((time_begin < 30)&&(time_begin >= 5)) {
						#if IS_ENABLED(CONFIG_CHARGER_BQ24158_V1) || IS_ENABLED(CONFIG_CHARGER_BQ24158)
						if (info->input_avg_current < info->data.ac_charger_current * 85 / 100) {
						#else
						if (info->input_avg_current < info->data.ac_charger_current * 70 / 100) {
						#endif
							adapter_type |= LOW_RATE_ADAPTER;
							info->nonstand_chg_type = adapter_type;
							chr_err("%s, nonstand_chg_type=0x%x", __func__,  info->nonstand_chg_type);
							if (!IS_ERR_OR_NULL(info->battery_psy))
								power_supply_changed(info->battery_psy);
						}
					}
...
/* Begin added by bitao.xiong for task-9878355 on 2020-09-05 */
	...
	if (fg_cust_data->embedded_sel) {
		if (get_uisoc(info) >= 62) {
			chr_err("mini sw stop charging,uisoc >=62\n");
				mini_chg_disabled = true;
		} else if (get_uisoc(info) <= 45) {
			chr_err("mini sw start charging,uisoc <=45\n");
				mini_chg_disabled = false;
			}
...
#if defined(CONFIG_TCT_CHARGER)
#if defined(TARGET_BUILD_MMITEST)
/* Begin modified by hailong.chen for task 10204962 on 2020-11-13 */
//Begin added by linsheng.liu for wrightpro charge porting task 10587810 on 2020/12/28
#ifndef CONFIG_MTK_WRIGHTPRO_CHARGE
	if (get_uisoc(info) >= 62) {
		chr_err("mini sw stop charging,uisoc >=62\n");
		charging = false;
		goto stop_charging;
	}
//End added by linsheng.liu for wrightpro charge porting task 10587810 on 2020/12/28
#endif
/* End modified by hailong.chen for task 10204962 on 2020-11-13 */
#endif
	if (gm->chg_disable) {
...
/* Begin add by jin.wang task 2064 on 2021.10.8 */
#if defined(CONFIG_TCT_CHARGER)
...
#if defined(CONFIG_TCT_CHARGER) && defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_20_SUPPORT)
	if (alg == NULL) {
		chr_err("Error: get pe2 fail, delay\n");
		return false;
	}
#else
	if (alg == NULL)
...
/* End mod by jin.wang */
	...
	info->alg[idx] = alg;
/* Begin mod by jin.wang task 2064 on 2021.10.8 */
#if defined(CONFIG_TCT_CHARGER) && defined(CONFIG_MTK_PUMP_EXPRESS_PLUS_SUPPORT)
	if (alg == NULL) {
