...
/*Begin:optimize dual charging algo by wanglin.chen on 2022.08.01*/
...
//	}
	...
	if (mtk_pe40_get_is_connect(info)) {
		...
		if (adapter_dev_get_property(info->pd_adapter, TYPEC_RP_LEVEL)
			== 3000) {
			/*start to add by weijun for charging current*/
			#if defined(JRD_PROJECT_FULL_BUFFALO_TMO) || defined(JRD_PROJECT_VND_BUFFALO_TMO)
			...
			#endif
			/*end to add by weijun for charging current*/
        ...
        /*start to add by weijun for pd charging current*/
        #if defined(JRD_PROJECT_FULL_BUFFALO_TMO) || defined(JRD_PROJECT_VND_BUFFALO_TMO)
        if (pdata->input_current_limit > 2000000) {
        pdata->input_current_limit = 2000000;
        }
        #endif
        /*end to add by weijun for pd charging current*/
	...
	if ((mtk_pe20_get_is_enable(info) &&
		...
		/* Slave charger may not have input current control */
		pdata2->input_current_limit
				= info->data.ac_charger_input_current/2;
		pdata->input_current_limit
				= info->data.ac_charger_input_current/2;
...
static void dual_swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if ((mtk_pe_get_is_connect(info) && mtk_pe_get_is_enable(info)) ||
	(mtk_pe20_get_is_connect(info) && mtk_pe20_get_is_enable(info)) ||
	(mtk_pdc_check_charger(info) && !dual_swchg_check_pd_leave(info))) {
		pdata->input_current_limit = info->data.ac_charger_input_current;
		pdata->charging_current_limit = info->data.ta_ac_charger_current;
	}

	/* Add-start by baiwei.peng for usb if certification current limit on 2022/10/24 */
        #if IS_ENABLED(TARGET_BUILD_USBIF_COMPLIANCE)
        /* usbif certification limit */
        if (info->chr_type == STANDARD_HOST) {
                if (info->usb_state == USB_SUSPEND) {
                        pdata->input_current_limit =
                                info->data.usb_charger_current_suspend; /* 0mA */
                } else if (info->usb_state == USB_UNCONFIGURED) {
                        pdata->input_current_limit =
                                info->data.usb_charger_current_unconfigured; /* 70mA */
                } else if (info->usb_state == USB_CONFIGURED) {
                        pdata->input_current_limit =
                                info->data.usb_charger_current_configured; /* 500mA */
                } else {
                        pdata->input_current_limit =
                                info->data.usb_charger_current_unconfigured; /* 70mA */
                }
                pdata->charging_current_limit = pdata->input_current_limit;
                chr_err("usb_state=%d current:%d\n", info->usb_state, pdata->input_current_limit);
                if (info->usb_state != USB_CONFIGURED) {
                        charger_dev_enable_hz(info->chg1_dev, true);
			charger_dev_enable_hz(info->chg2_dev, true);
                } else {
                        charger_dev_enable_hz(info->chg1_dev, false);
                        charger_dev_enable_hz(info->chg2_dev, false);
                }
        }
        #endif
	/* Add-end by baiwei.peng for usb if certification current limit on 2022/10/24 */
	...
	if (pdata->thermal_input_current_limit != -1) {
		...
		ret = charger_dev_get_min_input_current(info->chg1_dev, &aicr1_min);
		if (ret != -ENOTSUPP) {
			if (pdata->thermal_input_current_limit < aicr1_min) {
				pdata->input_current_limit = 0;
			}
		}
	}
	...
	if (pdata->thermal_charging_current_limit != -1) {
		...
		ret = charger_dev_get_min_charging_current(info->chg1_dev, &ichg1_min);
		if (ret != -ENOTSUPP) {
			if(pdata->thermal_charging_current_limit < ichg1_min) {
				pdata->charging_current_limit = 0;
			}
		}
	}
	...
	if(pdata->input_current_limit_by_aicl != -1) {
		...
		!mtk_pe20_get_is_connect(info) &&
		!mtk_is_TA_support_pd_pps(info)) {
			chr_err("input_current_limit_by_aicl=%d\n", pdata->input_current_limit_by_aicl);
	...
	pr_notice("type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d atm:%d parallel:%d\n",
		info->chr_type, info->usb_unlimited,
		IS_ENABLED(TARGET_BUILD_USBIF_COMPLIANCE),
		info->usb_state,
		_uA_to_mA(pdata->input_current_limit_by_aicl),
		info->atm_enabled, info->data.parallel_vbus);
	...
	if ((mtk_pe_get_is_connect(info) && mtk_pe_get_is_enable(info)) ||
		...
		switch (swchgalg->state) {
		case CHR_CC:
			pdata2->charging_current_limit = pdata->charging_current_limit/2;
			pdata->charging_current_limit = pdata->charging_current_limit/2;
			break;
		case CHR_TUNING:
			pdata2->charging_current_limit = pdata2->charging_current_limit;
	...
	pr_notice("force:%d thermal:(%d %d)setting:(%d %d)(%d %d)",
		_uA_to_mA(pdata->force_charging_current),
		_uA_to_mA(pdata->thermal_input_current_limit),
		_uA_to_mA(pdata->thermal_charging_current_limit),
		_uA_to_mA(pdata->input_current_limit),
...
#endif
/*End:optimize dual charging algo by wanglin.chen on 2022.08.01*/
...
static void swchg_select_cv(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita)
		if (info->sw_jeita.cv != 0) {
			charger_dev_set_constant_voltage(info->chg1_dev,
							info->sw_jeita.cv);
			/* [BSP]Begin added by bitao.xiong for ENCORETF-7812 on 2022/11/03 */
			#if IS_ENABLED(CONFIG_TCT_CHARGER)
			charger_dev_set_constant_voltage(info->chg2_dev,
							info->sw_jeita.cv);
			#endif
			/* [BSP]End added by bitao.xiong for ENCORETF-7812 on 2022/11/03 */
			return;
		}
	...
	/* Begin added by weijun for charging current on 2021-08-13 */
	#if !IS_ENABLED(CONFIG_TCT_CHARGER)
	if (chg2_chip_enabled)
		charger_dev_set_constant_voltage(info->chg2_dev,
		...
		charger_dev_set_constant_voltage(info->chg2_dev,
			constant_voltage);
	#endif
	/* End added by weijun for charging current on 2021-08-13 */
}

/*Begin:optimize dual charging algo by wanglin.chen on 2022.08.01*/
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static void check_charging_port(struct charger_manager *info)
{
	...
	if (chg2_enable == true) {
		if ((mtk_pe20_get_is_enable(info) &&
			...
			if (info->sw_jeita.sm == TEMP_T2_TO_T3) {
				charger_dev_enable_termination(info->chg2_dev, false);
			} else {
				charger_dev_enable_termination(info->chg2_dev, true);
			}
...
static void dual_swchg_turn_on_charging(struct charger_manager *info)
{
	...
	if (swchgalg->state == CHR_ERROR) {
		...
		if ((mtk_pe_get_is_enable(info) && mtk_pe_get_is_connect(info)) ||
			(mtk_pe20_get_is_enable(info) && mtk_pe20_get_is_connect(info)) ||
			(mtk_pdc_check_charger(info) && !dual_swchg_check_pd_leave(info))) {
			if (info->chg2_data.input_current_limit == 0 ||
				info->chg2_data.charging_current_limit == 0) {
				chg2_enable = false;
	...
	if (chg2_enable == true) {
		if ((mtk_pe_get_is_enable(info) && mtk_pe_get_is_connect(info)) ||
			...
			if (swchgalg->state != CHR_POSTCC) {
				charger_dev_enable(info->chg2_dev, true);
				charger_dev_set_eoc_current(info->chg1_dev, info->data.dual_polling_ieoc);
...
int mtk_dual_switch_chr_full(struct charger_manager *info)
{
	...
	charger_dev_is_charging_done(info->chg1_dev, &chg_done);
	/*start to add by weijun for charging status pr 11661180 */
	#if defined(JRD_PROJECT_FULL_BUFFALO_TMO) || defined(JRD_PROJECT_VND_BUFFALO_TMO)
	if (!chg_done && (mt_get_charger_type() != CHARGER_UNKNOWN)) {
		swchgalg->state = CHR_CC;
	...
	#endif
	/*end to add by weijun for charging status pr 11661180 */
				...
				void *v)
{
	...
	struct dual_switch_charging_alg_data *swchgalg = info->algorithm_data;
	u32 ichg2, ichg2_min;
	/*Begin: add by wanglin.chen for on 2023.01.31*/
	int ichg2_t, ichg2_min_t;
	/*End: add by wanglin.chen for on 2023.01.31*/
	bool chg_en = false;
	bool chg2_chip_enabled = false;
	...
	if (event == CHARGER_DEV_NOTIFY_EOC) {
		...
		if (!chg_en || !chg2_chip_enabled) {
			...
			ichg2_t = (int)ichg2;
			ichg2_min_t = (int)ichg2_min;
			/*Begin: add by wanglin.chen for on 2023.01.31*/
			#if 0
			if (ichg2 - 500000 < ichg2_min)
			#else
			if (ichg2_t - 500000 < ichg2_min_t)
			/*End: add by wanglin.chen for on 2023.01.31*/
			#endif
			{
