...
bool mtk_pdc_check_charger(struct charger_manager *info)
{
	if (mtk_is_pdc_ready(info) == false) {
		chr_err("[%s] pd is not ready\n", __func__);
		return false;
	}

	chr_err("[%s] pd is ready\n", __func__);
	return true;
}
...
/* Begin modified by bitao.xiong for ENCORETF-42 on 2022-07-07 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
int mtk_pdc_get_setting(struct charger_manager *info, int *newvbus, int *newcur, int *newidx)
{
	int ret = -1;
	unsigned int ibus = 0, vbus = 0;
	bool chg1_mivr = false, chg2_mivr = false;
	unsigned int mivr1 = 0, mivr2 = 0;
	bool chg2_enable = false;
	int chg1_curr = 0, chg2_curr = 0;
	int vbat = 0;
	int idx = -1, selected_idx =-1;
	struct mtk_pdc *pd = NULL;
	struct adapter_power_cap *cap = NULL;
	unsigned int now_max_watt = 0, pd_max_watt = 0, pd_min_watt = 0;
	...
	chr_err("[%s %d]cap_nr=%d hv_en=%d\n", __func__, __LINE__, cap->nr, info->enable_hv_charging);
	...
	ibus = ibus / 1000;
	chr_err("[%s %d] ibus=%d \n", __func__, __LINE__, ibus);
	if (ret < 0) {
		chr_err("[%s %d] get ibus fail, keep default voltage\n", __func__, __LINE__);
		return -1;
	}
	...
	if (ret < 0) {
		chr_err("[%s %d] get vbus fail, keep default voltage\n", __func__, __LINE__);
		return -1;
	}

	if (info->enable_hv_charging == false) {
		chr_err("[%s %d] go to reset\n", __func__, __LINE__);
		goto reset;
	}
	...
	if ((chg1_mivr && (vbus < mivr1 - 500))
		|| (chg2_mivr && (vbus < mivr2 - 500))) {
		chr_err("[%s %d] go to reset\n", __func__, __LINE__);
		goto reset;
	}
	...
	vbat = battery_get_bat_voltage(); //mV
	charger_dev_get_charging_current(info->chg1_dev, &chg1_curr);  //chg1 conf
	chg1_curr = chg1_curr / 1000;
	if (info->data.parallel_vbus)
	{
		charger_dev_get_charging_current(info->chg2_dev, &chg2_curr);  //chg2 conf
		chg2_curr = chg2_curr / 1000;
	}

	chr_err("[%s %d] chg1_curr=%d chg2_curr=%d vbat=%d\n", __func__, __LINE__, chg1_curr, chg2_curr, vbat);
	now_max_watt = vbat * (chg1_curr + chg2_curr) / 1000; //mW
	chr_err("[%s %d] now_max_watt=%d\n", __func__, __LINE__, now_max_watt);

	pd_max_watt = (cap->max_mv[idx] * cap->ma[idx]) / 1000 * (100 - info->data.ibus_err) / 100;
	chr_err("[%s %d] pd_max_watt=%d\n", __func__, __LINE__, pd_max_watt);

	pd_min_watt = (cap->max_mv[pd->pd_buck_idx] * cap->ma[pd->pd_buck_idx]) / 1000
				* (100 - info->data.ibus_err) / 100
				- info->data.vsys_watt / 1000;
	if (pd_min_watt <= 5000)
		pd_min_watt = 5000;//mW
	chr_err("[%s %d] pd_min_watt=%d\n", __func__, __LINE__, pd_min_watt);


	if ((now_max_watt >= pd_max_watt) || chg1_mivr || chg2_mivr) {
		*newidx = pd->pd_boost_idx;
		boost = true;
		chr_err("[%s %d] newidx=pd_boost_idx=%d\n", __func__, __LINE__, *newidx);
	} else if (now_max_watt <= pd_min_watt) {
		*newidx = pd->pd_buck_idx;
		buck = true;
		chr_err("[%s %d] newidx=pd_buck_idx=%d\n", __func__, __LINE__, *newidx);
	} else {
		*newidx = selected_idx;
		boost = false;
		buck = false;
		chr_err("[%s %d] newidx=selected_idx=%d\n", __func__, __LINE__, *newidx);
	}

	*newvbus = cap->max_mv[*newidx];
	*newcur = cap->ma[*newidx];
	chr_err("[%s %d] newvbus=%d newcur=%d\n", __func__, __LINE__, *newvbus, *newcur);
...
reset:
	...
	*newvbus = cap->max_mv[*newidx];
	*newcur = cap->ma[*newidx];
	chr_err("[%s %d] newidx=pd_reset_idx=%d\n", __func__, __LINE__, *newidx);
	chr_err("[%s %d] newvbus=%d newcur=%d\n", __func__, __LINE__, *newvbus, *newcur);
	return 0;
}
#else
...
#endif
/* End modified by bitao.xiong for ENCORETF-42 on 2022-07-07 */
...
void mtk_pdc_init_table(struct charger_manager *info)
{
	...
	/*Begin:add for limit input current 2000mA*/
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	for (i = 0; i < pd->cap.nr; i++)
	{
		if(pd->cap.ma[i] > info->data.ac_charger_input_current)
		{
			pd->cap.ma[i] = info->data.ac_charger_input_current;
		}
	}
#endif
	/*End:add for limit input current 2000mA*/
