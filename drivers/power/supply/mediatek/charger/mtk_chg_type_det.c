...
//Begin Modified by qiuguangliang for 11139316 on 2021-05-21
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	struct power_supply_desc real_type_desc;
	struct power_supply_config real_type_cfg;
	struct power_supply *real_type_psy;
#endif
//End Modified by qiuguangliang for 11139316 on 2021-05-21
	#ifdef CONFIG_EXTCON_USB_CHG
...
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static bool flag_support_fast_charging;
#endif
...
/* [TCTCD][BSP]Begin added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
	...
	struct mtk_pdc *pd = NULL;
	bool is_pd = false;


	if (!mtk_chg || !mtk_chg->cti || !mtk_chg->cti->chg_consumer || !mtk_chg->cti->chg_consumer->cm) {
		pr_notice("%s: no mtk chg data\n", __func__);
		return -EINVAL;
	}
	cti = mtk_chg->cti;
	...
	if (is_typec_adapter(info)) {
		...
		is_pd = true;
		vbus = pd->cap.max_mv[pd->pd_idx] * 1000;
		ibus = pd->cap.ma[pd->pd_idx] * 1000;
	}
#endif
/* [TCTCD][BSP]End added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
	...
	enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
	/* [TCTCD][BSP]Begin modified by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
	#if !IS_ENABLED(CONFIG_TCT_CHARGER)
		val->intval = 500000;
	#else
		if (mtk_chg->chg_type == CHARGING_HOST)
			val->intval = 1200000; // popup "charging"
		else
			val->intval = 500000; // popup "slow charging"
	#endif
	/* [TCTCD][BSP]End modified by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		val->intval = 5000000;
		break;
...
{
	/*start to add by weijun for workarount xiaomi adapter */
	...
	if (vbus > 4400)
		return;
	#endif
	/*end to add by weijun for workarount xiaomi adapter*/
...
struct mtk_battery_sysfs_field_info {
	...
	int (*set)(struct power_supply *psy,
		struct mtk_battery_sysfs_field_info *attr, int val);
	int (*get)(struct power_supply *psy,
		struct mtk_battery_sysfs_field_info *attr, char *val);
};
static enum power_supply_property real_type_properties[] = {
	POWER_SUPPLY_PROP_ONLINE,
};
static int mt_real_type_get_property(struct power_supply *psy,
	enum power_supply_property psp, union power_supply_propval *val)
{
	struct mt_charger *mtk_chg = power_supply_get_drvdata(psy);

	printk("\033[44m qiuguang  %s %d \033[0m \n", __func__, __LINE__);
	switch (psp) {
	case POWER_SUPPLY_PROP_ONLINE:
		val->intval = 0;
		/* Force to 1 in all charger type */
...
static int real_type_set(struct power_supply *psy, struct mtk_battery_sysfs_field_info *attr,int val)
{
	printk("%s %d \n",__func__, __LINE__);
	return 0;
}
static	int real_type_get(struct power_supply *psy, struct mtk_battery_sysfs_field_info *attr, char *val)
{
	struct mt_charger *mtk_chg = power_supply_get_drvdata(psy);
	char *type[] =  {
	"CHARGER_UNKNOWN",
	...
	sprintf(val, "%s", type[mtk_chg->chg_type]);
	printk ("%s %s \n", __func__, type[mtk_chg->chg_type]);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (battery_attr->set != NULL)
		battery_attr->set(psy, battery_attr, val);
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (battery_attr->get != NULL)
		battery_attr->get(psy, battery_attr, val);

	count = scnprintf(buf, PAGE_SIZE, "%s", val);
	return count;
}
#define BAT_SYSFS_FIELD_RW(_name, _prop, _set, _get)	\
 {									 \
...
static struct mtk_battery_sysfs_field_info battery_sysfs_field_tbl[] = {
	BAT_SYSFS_FIELD_RW(real_type, BAT_PROP_REAL_TYPE, real_type_set, real_type_get),
};
...
static int __maybe_unused battery_sysfs_create_group(struct power_supply *psy)
{
...
//Begin Modified by qiuguangliang for 11139316 on 2021-05-21
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	mt_chg->real_type_desc.name = "mtk_charger_type";
	mt_chg->real_type_desc.type = POWER_SUPPLY_TYPE_UNKNOWN;
	mt_chg->real_type_desc.properties = real_type_properties;
	mt_chg->real_type_desc.num_properties = ARRAY_SIZE(real_type_properties);
	mt_chg->real_type_desc.get_property = mt_real_type_get_property;
	mt_chg->real_type_cfg.drv_data = mt_chg;
#endif
//End Modified by qiuguangliang for 11139316 on 2021-05-21
...
//Begin Modified by qiuguangliang for 11139316 on 2021-05-21
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	mt_chg->real_type_psy = power_supply_register(&pdev->dev,
		&mt_chg->real_type_desc, &mt_chg->real_type_cfg);
	if (IS_ERR(mt_chg->real_type_psy)) {
		dev_notice(&pdev->dev, "Failed to register power supply: %ld\n",
			PTR_ERR(mt_chg->real_type_psy));
		ret = PTR_ERR(mt_chg->real_type_psy);
		return ret;
	}
	battery_sysfs_create_group(mt_chg->real_type_psy);
#endif
//End Modified by qiuguangliang for 11139316 on 2021-05-21
...
/* [TCTCD][BSP]Begin added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
int ac_status_changed(bool fast_support)
{
	struct power_supply *ac_psy = power_supply_get_by_name("ac");
	...
	if (IS_ERR_OR_NULL(ac_psy)) {
		...
		if (!IS_ERR_OR_NULL(ac_psy)) {
			pr_info("%s: get ac power supply failed\n", __func__);
			return -EINVAL;
		}
	}

	flag_support_fast_charging = fast_support;
	power_supply_changed(ac_psy);
	power_supply_put(ac_psy);
	return 0;
}
#endif
/* [TCTCD][BSP]End added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
