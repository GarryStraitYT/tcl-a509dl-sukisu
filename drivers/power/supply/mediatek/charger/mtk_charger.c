...
#include <mtk_battery_internal.h> //add by tangshan.bai for task 11036631 macau on 2021/04/26
...
/* Begin add by bitao.xiong for ENCORECKT-2669 on 2022-08-21 */
...
#endif
/* End add by bitao.xiong for ENCORECKT-2669 on 2022-08-21 */

/* Begin added by bitao.xiong for ENCORETF-42 on 2022-08-28 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static int health = POWER_SUPPLY_HEALTH_GOOD;
void battery_update_health(int health);
#endif
/* End added by bitao.xiong for ENCORETF-42 on 2022-08-28 */
...
{
	struct charger_manager *info = consumer->cm;
	/* Begin added by weijun for overheat power off on 2021-08-05 */
	#if defined(TARGET_BUILD_MMITEST) || defined(DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY)
	chr_err("%s: dev:%s idx:%d current:%d mini or disable temp not limit current\n", __func__,
		dev_name(consumer->dev), idx, charging_current);
	return 0;
	#endif
	/* End added by weijun for overheat power off on 2021-08-05 */
...
//Begin modify by qiuguangliang for 11260953 on 20210623
	...
	if (battery_psy && 0 == power_supply_get_property(battery_psy,	POWER_SUPPLY_PROP_TEMP,&propval)) { 
...
//End modify by qiuguangliang for 11260953 on 20210623
...
/* Begin modified by hailong.chen for task 9337002 on 2020-04-28 */
...
void do_sw_jeita_state_machine(struct charger_manager *info)
{
    ...
    /* Begin modified by hailong.chen for task 9486270 on 2020-06-01 */
    if ((info->enable_step_chg) && (sw_jeita->sm == TEMP_T2_TO_T3)) {
        vbat_now = battery_get_bat_voltage() * 1000;
        if (info->chg1_data.charging_current_limit_by_vbat == -1) {
            if(vbat_now > info->data.step_chg_vbat) {
                info->chg1_data.charging_current_limit_by_vbat = info->data.step_chg_current;
            }
        } else {
            if (vbat_now < (info->data.step_chg_vbat - info->data.step_chg_vbat_hysteresis)) {
                info->chg1_data.charging_current_limit_by_vbat = -1;
            }
        }
    } else {
        info->chg1_data.charging_current_limit_by_vbat = -1;
    ...
    printk("[SW_JEITA]preState:%d newState:%d tmp:%d cv:%d chg:(%d) vbat_now:%d enable_step_chg:%d\n",
            sw_jeita->pre_sm, sw_jeita->sm, info->battery_temp,
            sw_jeita->cv, info->chg1_data.charging_current_limit_by_vbat,
            vbat_now, info->enable_step_chg);
    /* End modified by hailong.chen for task 9486270 on 2020-06-01 */
...
#endif
/* End modified by hailong.chen for task 9337002 on 2020-04-28 */
...
int charger_psy_event(struct notifier_block *nb, unsigned long event, void *v)
{
	...
	if (strcmp(psy->desc->name, "battery") == 0) {
		...
		if (!ret) {
			/*Begin: mdoify by wangling.chen for GPUTURBO-47 on 2023.02.01*/
			#if 0
			tmp = battery_get_bat_temperature() * 10;//Modify by qiuguangliang for 11260953 on 20210623
			#else
			tmp = val.intval;
			#endif
			/*End: mdoify by wangling.chen for GPUTURBO-47 on 2023.02.01*/
...
void mtk_charger_int_handler(void)
{
	...
	/* [BSP]Begin modified by bitao.xiong for ENCORETF-7804 on 2022/11/03 */
	#if IS_ENABLED(CONFIG_TCT_CHARGER)
	} else if (pinfo->enable_sw_jeita
			&& ((pinfo->sw_jeita.sm == TEMP_ABOVE_T4)
			|| (pinfo->sw_jeita.sm == TEMP_BELOW_T0))) {
		chr_err("%s: battery temperature is tool cold or hot,jeita.sm=%d\n", __func__, pinfo->sw_jeita.sm);
		charger_manager_notifier(pinfo, CHARGER_NOTIFY_ERROR);
	#endif
	/* [BSP]End modified by bitao.xiong for ENCORETF-7804 on 2022/11/03 */
	} else
...
/*Begin:add for charging current not up to 2500ma by wanglin.chen on 2023.07.25*/
#if IS_ENABLED(CONFIG_TCT_CHARGER)
...
#endif
/*End:add for charging current not up to 2500ma by wanglin.chen on 2023.07.25*/
...
/*Begin:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
	pdata1->nonstand_chg_type = 0xff;
	pdata1->input_current_limit_by_aicl = -1;
/*End:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
...
/* End add by bitao.xiong for ENCORECKT-2669 on 2022-08-21 */
	...
	/* Begin added by weijun for nonstand charge type */
	get_monotonic_boottime(&info->charging_begin_time);
	/* End added by weijun for nonstand charge type */
...
/* [TCTCD][BSP]Begin added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	ac_status_changed(0);
#endif
/* [TCTCD][BSP]End added by bitao.xiong for ENCORETF-6462 on 2022/09/29 */
	return 0;
}
...
//Begin added by tangshan.bai for defect 11043976 on 2020-05-07
...
//End added by tangshan.bai for defect 11043976 on 2020-05-07

/* Begin added by bitao.xiong for ENCORETF-42 on 2022-08-28 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
int battery_do_health_update(struct charger_consumer *consumer)
{
	struct charger_manager *info = consumer->cm;
	if (info != NULL && info->init_done 
			&&(!mutex_is_locked(&info->charger_lock))) {
...
static bool mtk_chg_check_vbus(struct charger_manager *info)
{
	...
	if (vchr > info->data.max_charger_voltage) {
		...
		info->data.stop_by_ovp = true; //Modify by qiuguangliang for 10985708 on 20210617
		return false;
	}
//Begin modify by qiuguangliang for 10985708 on 20210617
	if (info->data.stop_by_ovp) {
		if (vchr <= info->data.ovp_restore_voltage) {
			chr_err("%s: stop_by_ovp vbus(%d mV) <= %d mV \n", __func__, vchr / 1000,
						info->data.ovp_restore_voltage / 1000);
			info->data.stop_by_ovp = false;
			return true;
		} else {
			chr_err("%s:  vbus(%d mV) > %d mV ovp mode stop\n", __func__, vchr / 1000,
						info->data.ovp_restore_voltage / 1000);
			return false;
		}
	}
//End modify by qiuguangliang for 10985708 on 20210617
...
//Begin modified  by tangshan.bai for task 11008424 MMI SW ignore battery check on 20210410
...
//End modified  by tangshan.bai for task 11008424 MMI SW ignore battery check on 20210410
...
/* Begin added by weijun for nonstand charge type */
#if 0//!defined(TARGET_BUILD_MMITEST)
static void mtk_charger_check_input_avg_current(struct charger_manager *info, bool is_charger_on)
{
	int chg_current = 0;
	int i = 0, ichg = 0;
	...
	if (is_charger_on == true) {
		for (i = 0; i < 5; i++) {
			msleep(200);
			//ichg = pmic_get_charging_current(info);
			charger_dev_get_ibus(info->chg1_dev, &ichg);
			pr_err("%s: ibus=%d\n", __func__, ichg);
			if (ichg <= 0)
				return;
			chg_current += ichg;
                                  ...
                                  struct device_attribute *attr, char *buf)
{
	...
	chg_data = &info->chg1_data;

	if (mt_get_charger_type() == NONSTANDARD_CHARGER)
	...
	if (mt_get_charger_type() == STANDARD_CHARGER) {
		if (chg_data->thermal_input_current_limit == -1 &&
			...
			battery_get_uisoc() < 80 && battery_get_uisoc() > 1) {
				struct timespec time_now, charging_time;
				int time_begin =-1;
				get_monotonic_boottime(&time_now);
				charging_time = timespec_sub(time_now, info->charging_begin_time);
				time_begin = charging_time.tv_sec;
				if((time_begin<70)&&(time_begin>=40)) {
					is_running = mtk_is_pep_series_connect(info) | mtk_pdc_check_charger(info);

					if (!is_running &&
						(info->input_avg_current>0) &&
						(info->input_avg_current < info->data.ac_charger_input_current * 70 / 100) )
						nonstand_chg_type = 0x2;
				}
		}
	}

	chr_err("%s: nonstand_chg_type:0x%x, input_avg_current=%d, is_running=%d\n", __func__, nonstand_chg_type, info->input_avg_current, is_running);
...
/* End added by weijun for nonstand charge type */
#else
/* [BSP]Begin modified by bitao.xiong for ENCORETF-9421 on 2023/02/01 */
#if 0
/*Begin:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
static ssize_t nonstand_charge_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct charger_manager *info = dev->driver_data;
	const int weak_chg_input_current = info->data.weak_ac_charger_input_current;

	if (info->chg1_data.nonstand_chg_type != 0xFF)
	{
		return sprintf(buf, "0x%x\n", info->chg1_data.nonstand_chg_type);
	}

	if (mt_get_charger_type() == NONSTANDARD_CHARGER)
	{
		info->chg1_data.nonstand_chg_type = 0x1;
	}
	else if(mt_get_charger_type() == STANDARD_CHARGER)
	{
		if (battery_get_uisoc() < 80)
		{
			if (info->chg1_data.input_current_limit_by_aicl > 0)
			{
				if (info->chg1_data.input_current_limit_by_aicl <= weak_chg_input_current)
				{
					info->chg1_data.nonstand_chg_type =0x2;
				}
				else
				{
					info->chg1_data.nonstand_chg_type =0x0;
				}
			}
		}
		else
	...
	chr_err("[cwldebug %s] input_current_limit_by_aicl=%d nonstand_chg_type=%d\n", __func__, info->chg1_data.input_current_limit_by_aicl, info->chg1_data.nonstand_chg_type);
	return sprintf(buf, "0x%x\n", info->chg1_data.nonstand_chg_type);
}
DEVICE_ATTR_RO(nonstand_charge_type);
/*End:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
...
static ssize_t nonstand_charge_type_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	chr_err("[cwldebug %s] nonstand_chg_type=%d\n", __func__, info->chg1_data.nonstand_chg_type);
	return sprintf(buf, "0x%x\n", info->chg1_data.nonstand_chg_type);
}
DEVICE_ATTR_RO(nonstand_charge_type);
#endif
/* [BSP]End modified by bitao.xiong for ENCORETF-9421 on 2023/02/01 */
#endif

extern int chg_enable; //Added by tangshan.bai for task 10669361 on 20210413
//Begin added by tangshan.bai for task 10973847 third party battery cannot charge on 2021-04-07
extern const char *battery_name ;
//End added by tangshan.bai for task 10973847 third party battery cannot charge on 2021-04-07
...
/* Begin add by bitao.xiong for ENCORECKT-2674 on 2022-08-22 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
...
#endif
/* End add by bitao.xiong for ENCORECKT-2674 on 2022-08-22 */

	temperature = info->battery_temp;
	thermal = &info->thermal;

//begin add by tangshan.bai for task 10669361 on 20210413
	if(chg_enable == false){
		charging = false;
		printk("cmd to disable charging\n");
		goto stop_charging;
	}

	#ifndef TARGET_BUILD_MMITEST
	if (strcmp("NA:NA:NA:NA", battery_name) == 0)
		{
			charging = false;
			goto stop_charging;
		}
	#endif
//end add by tangshan.bai for task 10669361 on 20210413
	if (info->enable_sw_jeita == true) {
		do_sw_jeita_state_machine(info);
...
/* Begin modifed by bitao.xiong for ENCORECKT-2669 on 2022-08-21 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	if (is_mmitest || is_inproductionflag) {
		chr_err("%s  is_mmitest=%d, is_inproductionflag=%d, soc_limited = %d\n",
				__func__, is_mmitest, is_inproductionflag, soc_limited);
		if (soc_limited) {
...
#endif
/* End modifed by bitao.xiong for ENCORECKT-2669 on 2022-08-21 */
...
/* Begin: add charging_enabled node by wanglin.chen on 2022.08.10*/
...
/* End: add charging_enabled node by wanglin.chen on 2022.08.10*/
/*Begin:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
	#if 0
	/* Begin added by weijun for nonstand charge type */
	#if !defined(TARGET_BUILD_MMITEST)
	mtk_charger_check_input_avg_current(info, charging);
	#endif
	/* End added by weijun for nonstand charge type */
	#endif
/*End:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
...
// workaround for mt6768 
	...
	/*Begin: modify for ENCORETF-10841 by wanglin.chen on 2023.05.10*/
		...
		#else
		do {
			if (vbus >= 0 && vbus < 2500 && !mt_charger_plugin() && !info->pd_reset) {
				msleep(100);
				vbus_absent_cnt++;
				vbus_online = false;
			} else {
...
/*Begin:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
void sw_dev_run_aicl(struct charger_manager *info, int *sw_aicl)
{
	...
	if ((!info->enable_sw_jeita && (bat_temp < 0 || bat_temp > 50))
	|| (info->enable_sw_jeita && (info->sw_jeita.sm == TEMP_BELOW_T0 || info->sw_jeita.sm == TEMP_ABOVE_T4)))
	{
		return;
	}
...
static void check_dev_aicl(struct charger_manager *info)
{
	struct charger_data *pdata;
	...
	&&!mtk_pdc_check_charger(info))
	{
		if(battery_get_uisoc() < 80)
		{
			...
			if (hw_aicl > 0) {
				pdata->input_current_limit_by_aicl = hw_aicl;
			}
			/*Begin: modify by wanglin.chen for ENCOREVZW-10110 on 2023.02.09*/
			#if 0
			sw_dev_run_aicl(info, &sw_aicl);
			if (sw_aicl > 0) {
				pdata->input_current_limit_by_aicl = sw_aicl;
			}
			#endif
			/*End: modify by wanglin.chen for ENCOREVZW-10110 on 2023.02.09*/
		}
		chr_err("[cwldebug %s] ui_soc=%d sw_aicl=%d hw_aicl=%d\n", __func__, battery_get_uisoc(), sw_aicl, hw_aicl);
	}
}
/*End:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
...
//Begin add by tangshan.bai for task 11036631 macau on 2021/04/26
...
static void read_charger_para(struct charger_manager *info,
                              struct device_node *np)
{
    ...
    CHARGER_PROP_READ(info->enable_compatible_adapter,
                      "enable_compatible_adapter",
                      ENABLE_COMPATIBLE_ADAPTER);
	printk("tangshan [charger_para] read end");
    /* Begin added by hailong.chen for task 9486270 on 2020-06-01 */
    ...
    CHARGER_PROP_READ(info->data.step_chg_vbat_hysteresis,
                      ...
                      STEP_CHG_VBAT_HYSTERESIS);
    CHARGER_PROP_READ(info->data.step_chg_current,
                      "step_chg_current",
                      STEP_CHG_CURRENT);
    /* End added by hailong.chen for task 9486270 on 2020-06-01 */

/* Begin added by tangshan.bai for task10282332on 2020-11-27 */
    CHARGER_PROP_READ(info->data.max_charging_time_ac,
                      "max_charging_time_ac",
                       MAX_CHARGING_TIME_AC);
/* End added by tangshan.bai for task10282332on 2020-11-27 */
}

static void override_charger_para(struct charger_manager *info,
                                  struct device_node *np)
{
    char prop_name[128];
    u32 val;
    ...
    if (info->chg1_dev &&
        ...
        CHARGER_PROP_READ_SUFFIX(fg_cust_data.car_tune_value,
                                 "car_tune_value",
                                 info->chg1_dev->props.alias_name);
    } else {
    ...
    /* always last,add code to above*/
}
//End add by tangshan.bai for task 11036631 macau on 2021/04/26
				...
				struct device *dev)
{
	...
	info->enable_dynamic_mivr &= !of_property_read_bool(np, "disable_dynamic_mivr");
...
//Begin modify by qiuguangliang for 10985708 on 20210617
	if (of_property_read_u32(np, "ovp_restore_voltage", &val) >= 0)
		info->data.ovp_restore_voltage = val;
	else {
		chr_err("use default V_OVP_RESTORE:%d\n", V_OVP_RESTORE);
		info->data.ovp_restore_voltage = V_OVP_RESTORE;
	}
//End modify by qiuguangliang for 10985708 on 20210617
...
/*End:modify by wanglin.chen on 2022.11.18 for slow chager detection of ENCOREVZW-8327 */
	...
	info->data.ta_12v_support &= !of_property_read_bool(np, "disable_ta_12_support");
...
//Begin add by tangshan.bai for task 11036631 on 2021/04/26
    read_charger_para(info, np);
    override_charger_para(info, np);
//End add by tangshan.bai for task 11036631 on 2021/04/26
...
//Begin added by tangshan.bai for task 10669361 on 20210413
...
void input_suspend_enable(void)
{
	chr_err("%s: starts\n", __func__);
...
void input_suspend_disable(void)
{
	chr_err("%s: starts\n", __func__);
...
//End added by tangshan.bai for task 10669361 on 20210413
...
/* Begin added by tangshan.bai for task 11036631 2021-04-26  */
    info->chg1_data.charging_current_limit_by_jeita = -1;
    info->chg1_data.charging_current_limit_by_vbat = -1;
/* End added by tangshan.bai for task 11036631 2021-04-26  */
...
/* Begin added by hailong.chen for task 10195880 on 2020-11-12 */
#if defined(TARGET_BUILD_MMITEST)
	info->enable_sw_safety_timer = false;
	charger_dev_enable_safety_timer(info->chg1_dev, false);
#endif
/* End added by hailong.chen for task 10195880 on 2020-11-12 */
