...
/* Begin added by qiuguangliang for task 10985708 on 2021-05-11 */
...
/* ============================================================ */
/* Begin added by qiuguangliang for task 10985708 on 2021-05-11 */
const char *battery_name={"NA:NA:NA:NA"};
...
int battery_get_batt_id(void)
{
	...
	if (ret < 0) {
		bm_err("iio_read_channel_processed fail");
		return ret;
	}
...
void fgauge_get_profile_id(void)
{
	...
	for (index = 0; index < 4; index++) {
		batid_max = 0;
		batid_min = 0;
		sprintf(node_name, "bat_id_max%d", index);
		of_property_read_u32(np, node_name, &batid_max);
		sprintf(node_name, "bat_id_min%d", index);
		of_property_read_u32(np, node_name, &batid_min);
		bm_err("batt_id_max=%d,batt_id_min=%d\n", batid_max, batid_min);
		if (batid_max == 0 || batid_min == 0) {
			gm.battery_id = 0;
			bm_err("not found compatible battery id,bat_id use default 0\n");
			return;
		}
		if (rawdata > batid_min && rawdata < batid_max) {
			sprintf(node_name, "battery_name%d", index);
			of_property_read_string(np, node_name, &battery_name);
			bm_err("battery name is %s\n", battery_name);
            // Added by bin.song.hz for task:9148940 on 2020-03-31 begin
            #ifdef CONFIG_TCT_DEVICEINFO
            sprintf(battery_info_module_name, "%s", battery_name);
            #endif
            // Added by bin.song.hz for task:9148940 on 2020-03-31 end
			gm.battery_id = index;
			return;
		}
	}
...
#endif
/* End added by qiuguangliang for task 10985708 on 2021-05-11 */
...
status:%d,type:%d,vbat:%d,capacity:%d,current:%s%d,temperature:%d,Vbus:%d\n",
            tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday,
            tm.tm_hour, tm.tm_min, tm.tm_sec, ts.tv_nsec,
            battery_main.BAT_STATUS, mt_get_charger_type(), battery_main.BAT_batt_vol,
            battery_get_uisoc(), fg_current_state ? "" : "-",
            fg_current, gm.tbat_precise, g_chr_vol);
...
#else
    printk("tangshan It is MINI sw ,kernel ignore bat check\n");
    return;
#endif
...
static void mt_battery_thermal_check(int tmp)
{
	//do nothing
	bm_err("[%s]temperature=%d,defined TARGET_BUILD_MMITEST or Disable thermal\n", __func__, tmp);
}
#else
#define BATTERY_SHUTDOWN_HOT_TEMPERATURE 60
#define BATTERY_SHUTDOWN_COLD_TEMPERATURE (-20)
static struct timespec pre_temp_fault_time;
static void mt_battery_thermal_check(int tmp)
{
	struct timespec now_time, diff;

	if (tmp >= BATTERY_SHUTDOWN_HOT_TEMPERATURE ||
			tmp <= BATTERY_SHUTDOWN_COLD_TEMPERATURE) {
		battery_update(&battery_main);//notify android power off
		get_monotonic_boottime(&now_time);

		if (pre_temp_fault_time.tv_sec == 0) {
			pre_temp_fault_time = now_time;
		}

		diff = timespec_sub(now_time, pre_temp_fault_time);
		bm_debug("[%s]diff time:%ld,battery temperature:%d\n", __func__, diff.tv_sec, tmp);
		if (diff.tv_sec >= 60 || is_kernel_power_off_charging()) {
			bm_err("[Battery] Tbat(%d)>=60 or <=-20, system need power down.\n", tmp);
			kernel_power_off();
		}
	} else {
		pre_temp_fault_time.tv_sec = 0;
	}
