...
/*start to add by weijun for battery temp fixed*/
static int fixtemp_val = -65536;
module_param_named(
	fixtemp_val, fixtemp_val,
	int, S_IRUSR | S_IWUSR
);
/*end to add by weijun for battery temp fixed*/
...
int input_enable = true;
extern void input_suspend_enable(void);
extern void input_suspend_disable(void);

static ssize_t show_input_enable(struct device *dev, struct device_attribute *attr, char *buf)
{
	bm_err("show_input_enable\n");
	return sprintf(buf, "%u\n", input_enable);
}

static ssize_t store_input_enable(struct device *dev, struct device_attribute *attr,
                                  const char *buf, size_t size)
{
    bm_err("store_input_enable\n");
    if (kstrtouint(buf, 10, &input_enable) == 0)
    {
		bm_err("chg_enable = %d\n", input_enable);
		if(input_enable){
		    input_suspend_disable();
		}else{
		    input_suspend_enable();
		}
		return size;
    }
        return size;
}
static DEVICE_ATTR(input_enable, 0664, show_input_enable, store_input_enable);
...
signed int battery_meter_get_ISense(void)
{
	...
	if (is_charging == false)
			fg_current_temp = 0 - fg_current_temp;
	return fg_current_temp/10;
#endif
}
    /*End Added By tangshan.bai for defect 10923172 on 2021-03-15 */
...
/* Add-start by baiwei.peng for BMS uart log support on 2021/09/23 */
#ifdef TCT_BMS_SW_SUPPORT
extern signed int g_chr_vol;
#endif
/* Add-end by baiwei.peng for BMS uart log support on 2021/09/23 */
...
void battery_update_psd(struct battery_data *bat_data)
{
    ...
    /*Begin Added By tangshan.bai for defect 10923172 on 2021-03-15 */
    bat_data->BAT_ISense_Current = battery_meter_get_ISense();
...
{
	battery_main.BAT_HEALTH = health;
}
EXPORT_SYMBOL(battery_update_health);
#endif
...
/*Begin Added By tangshan.bai for defect 10923172 on 2021-03-15 */
    ...
    case POWER_SUPPLY_PROP_BATTERY_TYPE:
        val->strval = battery_name;		//sprintf(battery_info_module_name, "%s", battery_name);
        break;
/*end Added By tangshan.bai for defect 10923172 on 2021-03-15 */
/*Add begin by tangshan.bai for task 10669361 on 20210413*/
	case POWER_SUPPLY_PROP_COULOMB_COUNT:
		val->intval = gauge_get_coulomb();
		break;
/* Begin: add charging_enabled node by wanglin.chen on 2022.08.10*/
...
/* End: add charging_enabled node by wanglin.chen on 2022.08.10*/
#endif
/*Add end by tangshan.bai for task 10669361 on 20210413*/
	...
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		...
		if ((ret == POWER_SUPPLY_CAPACITY_LEVEL_FULL) ||
			...
			if (fgcurrent != 0)
			{
				/*Begin:modify by wanglin.chen for ENCOREVZW-8120 on 2022.11.24*/
				if(fgcurrent < 5000)
				{
					fgcurrent = 5000;
				}
				/*End:modify by wanglin.chen for ENCOREVZW-8120 on 2022.11.24*/
				time_to_full = remain_mah * 3600 / fgcurrent;
			}

			bm_err("time_to_full:%d, remain:ui:%d mah:%d, fgcurrent:%d, qmax:%d\n",
...
/*Begin Added By tangshan.bai for defect 10923172 on 2021-03-15 */
    battery_main.BAT_ISense_Current = 0;
    battery_main.BAT_batt_id = 0;
/*End Added By tangshan.bai for defect 10923172 on 2021-03-15 */
}
...
void battery_update(struct battery_data *bat_data)
{
 ...
 //begin added by tangshan.bai for task 10949798 on 2021-03-24
#ifdef DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY
    if((gm.ui_soc <=4)&&(battery_main.BAT_batt_vol >= 3700 )) {
        gm.ui_soc = 4;
        bat_data->BAT_CAPACITY = 4;
        printk("Is DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY ,hold soc = 4 \n");
    }
#endif
//end added by tangshan.bai for task 10949798 on 2021-03-24

	battery_update_psd(&battery_main);
	bat_data->BAT_TECHNOLOGY = POWER_SUPPLY_TECHNOLOGY_LION;
/* Begin modified by bitao.xiong for ENCORETF-42 on 2022-08-28 */
#if !IS_ENABLED(CONFIG_TCT_CHARGER)
	bat_data->BAT_HEALTH = POWER_SUPPLY_HEALTH_GOOD;
#else
	if (gm.pbat_consumer != NULL) {
		battery_do_health_update(gm.pbat_consumer);
	}
#endif
/* End modified by bitao.xiong for ENCORETF-42 on 2022-08-28 */

/* Begin modified by bitao.xiong for ENCORETF-42 on 2022-07-26 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	bat_data->BAT_PRESENT = pmic_is_battery_exist();
#else
	bat_data->BAT_PRESENT = 1;
#endif
/* End modified by bitao.xiong for ENCORETF-42 on 2022-07-26 */
...
/* [BSP]Begin modified by bitao.xiong for ENCORETF-7823 on 2022/11/15 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	bm_err(
		"[%s] %d %d %d %d %lld %d\n",
		__func__,
...
/* [BSP]End modified by bitao.xiong for ENCORETF-7823 on 2022/11/15 */
...
int force_get_tbat_internal(bool update)
{
	...
	if (update == true || pre_bat_temperature_val == -1) {
		...
		if (bat_temperature_volt != 0) {
			...
			/* [BSP]Begin modified by bitao.xiong for ENCORETF-7823 on 2022/11/15 */
			#if !IS_ENABLED(CONFIG_TCT_PROJECT_ENCORE)
				bat_temperature_volt =
				bat_temperature_volt -
				((fg_current_temp *
				...
				vol_cali =
					...
					(fg_meter_res_value + fg_r_value))
						/ 10000);
			#endif /*CONFIG_TCT_PROJECT_ENCORE*/
			/* [BSP]End modified by bitao.xiong for ENCORETF-7823 on 2022/11/15 */
			} else {
				bat_temperature_volt_temp =
					bat_temperature_volt;
...
/* [BSP]Begin modified by bitao.xiong for ENCORETF-7823 on 2022/11/15 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
		bm_err("[force_get_tbat] %d,%d,%d,%d,%d,%d r:%d %d %d\n",
...
/* Modify-start by baiwei.peng for FR-11264548 on 2021/08/05 */
    /* Begin added by tangshan.bai for task 10949798 on 2021-03-24 */
#if IS_ENABLED(DISABLE_TEMPERATURE_DETECTION_AND_THERMAL_POLICY) || \
	IS_ENABLED(TARGET_BUILD_CERTIFICATION) || IS_ENABLED(TARGET_BUILD_GCF) || \
	IS_ENABLED(TARGET_BUILD_MMITEST)
    gm.tbat_precise = 250;
    return 25;
#endif
    /* End added by tangshan.bai for task 10949798 on 2021-03-24 */
/* Modify-end by baiwei.peng for FR-11264548 on 2021/08/05 */
...
#else
	/*start to add by weijun for battery temp fixed*/
	if (fixtemp_val != -65536) {
		bm_err("[%s] fixed TBAT=25 t\n", __func__);
		gm.tbat_precise = fixtemp_val;
		return fixtemp_val/10;
	}
	/*end to add by weijun for battery temp fixed*/
...
/*****************************/
	...
	ret_device_file = device_create_file(&(dev->dev),
		&dev_attr_chg_enable);//added by tangshan.bai for task 10669361 on 20210413
	ret_device_file = device_create_file(&(dev->dev),
		&dev_attr_input_enable);//added by weijun for on 20210825
