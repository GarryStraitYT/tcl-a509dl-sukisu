...
/* Begin add by jin.wang to support typec interface at 2021-11-29 */
#if IS_ENABLED(CONFIG_TYPEC) && IS_ENABLED(CONFIG_MUSB_OLD_ARCH)
#include <linux/usb/typec.h>
#endif
/* End add by jin.wang */

/* History 1.0: modified by hailong.chen for task 9168065 on 2020-04-14 */
/* History 2.0: modified by hailong.chen for task 9329786 on 2020-04-28 */
/* History 3.0: modified by hailong.chen for task 9329786 on 2020-05-06 */
...
enum {
    DUAL_ROLE_PROP_PR_SRC = 0, DUAL_ROLE_PROP_PR_SNK, DUAL_ROLE_PROP_PR_NONE,
    /*The following should be the last element*/
...
enum {
    DUAL_ROLE_PROP_VCONN_SUPPLY_NO = 0, DUAL_ROLE_PROP_VCONN_SUPPLY_YES,
    /*The following should be the last element*/
...
static int wusb3801_read_device_id(struct wusb3801_chip *chip)
{
    ...
    chip->dev_id = rc;
    dev_info(cdev,
             "Vendor id: 0x%02x, Version id: 0x%02x\n",
...
static int wusb3801_set_dfp_power(struct wusb3801_chip *chip, uint8_t hcurrent)
{
    ...
    rc = wusb3801_write_masked_byte(chip->client,
    WUSB3801_REG_CONTROL0,
                                    WUSB3801_HOST_CUR_MASK, hcurrent);
    if (IS_ERR_VALUE_64(rc)) {
 ...
 *
 ************************************************************************/
static int wusb3801_set_toggle_time(struct wusb3801_chip *chip,
                                    uint8_t toggle_time)
{
    struct device *cdev = &chip->client->dev;
    int rc = 0;
...
static int wusb3801_reset_device(struct wusb3801_chip *chip)
{
    ...
    dev_info(cdev,
             "mode[0x%02x], host_cur[0x%02x], dttime[0x%02x]\n",
             chip->mode,
             chip->dfp_power,
             chip->dttime);
                               ...
                               char *buf)
{
    ...
    if (rc == 0 && chip->cc_sts != 0xFF)
        ...
        rc -= 1;
//begin modify by zhiquan.wen.hz for typec mini for xr11299376 on 20210701
//    ret = snprintf(buf, PAGE_SIZE, "cc_sts (%d)\n", rc);
    ret = snprintf(buf, PAGE_SIZE, "%u\n", rc+1);
//end modify by zhiquan.wen.hz for typec mini for xr11299376 on 20210701
    return ret;
}
                                 ...
                                 size_t size)
{
    ...
    if (sscanf(buff, "%d", &state) == 1) {
        ...
        if (((state == WUSB3801_STATE_UNATTACHED_SNK)
                        && ((chip->mode & WUSB3801_ROLE_MASK)
                                        == BIT_REG_CTRL0_RLE_SRC))
                        || ((state == WUSB3801_STATE_UNATTACHED_SRC)
                                        && ((chip->mode & WUSB3801_ROLE_MASK)
                                                        == BIT_REG_CTRL0_RLE_SNK))) {
            mutex_unlock(&chip->mlock);
            return -EINVAL;
        }
...
err8: device_remove_file(cdev, &dev_attr_fregdump);
#endif /* __TEST_CC_PATCH__ */

err7: device_remove_file(cdev, &dev_attr_fclientcur);
err6: device_remove_file(cdev, &dev_attr_fhostcur);
err5: device_remove_file(cdev, &dev_attr_fdttime);
err4: device_remove_file(cdev, &dev_attr_freset);
err3: device_remove_file(cdev, &dev_attr_fmode);
err2: device_remove_file(cdev, &dev_attr_ftype);
err1: device_remove_file(cdev, &dev_attr_fchip_state);
err0: return ret;
}
    ...
    if (type == WUSB3801_TYPE_SNK) {
        ...
        limit =
                        (chip->bc_lvl == WUSB3801_SNK_3000MA ?
                                        3000 :
                                        (chip->bc_lvl == WUSB3801_SNK_1500MA ?
                                                        1500 : 0));
        dev_dbg(cdev, "%s: limit = %d\n", __func__, limit);
        wusb3801_power_set_icurrent_max(chip, limit);
...
static int typec_partner_register(struct wusb3801_chip *chip)
{
	...
	if (!chip->typec_partner) {
		if (chip->type == WUSB3801_TYPE_AUD_ACC)
			chip->typec_partner_desc.accessory =
					TYPEC_ACCESSORY_AUDIO;
		else if (chip->type == WUSB3801_TYPE_DBG_ACC)
			chip->typec_partner_desc.accessory =
...
#endif /* __RESET_PATCH__ */
    dev_err(cdev,
            "%s: type[0x%02x] chipstate[0x%02x]\n",
            __func__,
            chip->type,
            chip->state);
...
static int test_cc_patch(struct wusb3801_chip *chip)
{
    ...
    i2c_smbus_write_byte_data(chip->client, WUSB3801_REG_TEST_02, 0x82);
    msleep(100);
    i2c_smbus_write_byte_data(chip->client, WUSB3801_REG_TEST_09, 0xC0);
    msleep(100);
    ...
    do {
        ...
        rc_reg_0f = i2c_smbus_read_byte_data(chip->client, WUSB3801_REG_TEST_09);
        if (IS_ERR_VALUE_64(rc_reg_0f)) {
            dev_err(cdev, "%s: WUSB3801_REG_TEST_09 failed to read\n", __func__);
        }
        i++;
        dev_err(cdev, "%s: WUSB3801_REG_TEST_09=0x%02x,i=%d\n", __func__, rc_reg_0f, i);
    } while (rc_reg_0f != 0 && i < 5);

    msleep(10);
    i2c_smbus_write_byte_data(chip->client, WUSB3801_REG_TEST_02, 0x80);
    ...
    do {
        ...
        i++;
        dev_err(cdev, "%s: WUSB3801_REG_TEST_02=0x%02x,i=%d\n", __func__, rc_reg_08, i);
...
work_unlock: enable_irq(chip->irq_gpio);
}
...
static int wusb3801_parse_dt(struct wusb3801_chip *chip)
{
    ...
    rc = of_property_read_u32(dev_node, "wusb3801,init-mode", &val);
    ...
    rc = of_property_read_u32(dev_node, "wusb3801,host-current", &val);
    ...
    rc = of_property_read_u32(dev_node, "wusb3801,drp-toggle-time", &val);
    data->dttime = (u8) val;
    ...
    dev_dbg(cdev,
            "init_mode:%d dfp_power:%d toggle_time:%d\n",
            data->init_mode,
            data->dfp_power,
            data->dttime);
...
/* Begin del by jin.wang to avoid build conflict err at 2021-11-29 */
...
err5:
    ...
    wusb3801_cancel_timer(&chip->current_timer);
#endif /* __DEBOUNCE_TIMER__ */
err4: wusb3801_destory_device(cdev);
err3: destroy_workqueue(chip->cc_wq);
...
err2: if (&client->dev.of_node != NULL)
        devm_kfree(cdev, chip->pdata);
err1: i2c_set_clientdata(client, NULL);
