...
#ifdef CONFIG_LONG_VOLUPKEY_LOG_STORE
...
struct mutex capturelog_lock;
atomic_t vol_up_long_press_dump_flag = ATOMIC_INIT(0);
struct timer_list Long_press_volupkey_timer;
void vol_up_long_press_timefunction(struct timer_list *t);
#endif
...
void vol_up_long_press_timefunction(struct timer_list *t)
{
	...
	atomic_set(&vol_up_long_press_dump_flag, 1);
	printk("vol_up_long_press_dump_flag=%d\n",vol_up_long_press_dump_flag);
	dump_thread = false;
	wake_up_interruptible(&dump_log_wait);
	printk("wake_up_interruptible dump_log_wait vol_up_long_press_dump_flag=%d \n",vol_up_long_press_dump_flag);
...
#ifdef CONFIG_LONG_VOLUPKEY_LOG_STORE
void _dump_last_log_lock(void)
{
  	mutex_lock(&capturelog_lock);
}
void _dump_last_log_unlock(void)
{
  	mutex_unlock(&capturelog_lock);
}
void keypad_display_err(void)
{
	  	...
	  	DAL_Printf("\n");
	  	DAL_Printf("\n");
	  	DAL_Printf("hang_detect:Keypad:current->comm=[%s] kernel_printk_tofb_ctrl=[0x%x] get_boot_mode=[%d] current_version=%d\n",
	  	current->comm,kernel_printk_tofb_ctrl,get_boot_mode(),current_version);
	  	print_ver_info();
			DAL_Printf("Keypad:logcat_kernel()=%d\n",logcat_to_kernel());
			DAL_Printf("--Android LOG analysis End------------------------------------\n");
	  	klog_bootprof_to_ui();
	  	klog_check_to_ui();
			_dump_last_log_lock();
			log_store_to_emmc();
			_dump_last_log_unlock();
	  	DAL_Printf("\n");
	  	...
	  	DAL_Printf("\n BOOT Diagnostic System. BUG please send to zhoujinggao@tcl.com \n");
}
static int dump_last_log(void *arg)
{
	int ret;
	...
	while (1) {
    msleep(500);
    ret = wait_event_interruptible(dump_log_wait,
    atomic_read(&vol_up_long_press_dump_flag)==1);
    printk("hang_detect log_store wait_event_interruptible ret=%d vol_up_long_press_dump_flag =%d \n",ret,vol_up_long_press_dump_flag);

		if (atomic_read(&vol_up_long_press_dump_flag)==1)
		{
				kpd_print("hang_detect vol_up_long_press_flag_timer start log_store_to_emmc start\n");
				atomic_set(&vol_up_long_press_dump_flag, 0);
		#ifdef CONFIG_TCL_BOOT_DETECT
#ifdef CONFIG_TCL_FORCE_DAILY_KEY_DETECT
		    // normal boot mode trigger! daily version will triger
				if ( get_boot_mode() == 0 && current_version == 0 && kernel_printk_tofb_ctrl != 0x28)
				{
					printk("hang_detect vol_up_long_press_flag_timer start log_store_to_emmc start daily\n");
		      keypad_display_err();
					printk("hang_detect vol_up_long_press_flag_timer start log_store_to_emmc stop\n");
				}
#else
				if ((kernel_printk_tofb_ctrl & 0x10) == 0x10 && get_boot_mode() == 0)
				{
		      	printk("hang_detect vol_up_long_press_flag_timer start log_store_to_emmc start current_version=%d \n",current_version);
		      	keypad_display_err();
						printk("hang_detect vol_up_long_press_flag_timer start log_store_to_emmc stop\n");
			  }
#endif			  
		...
		{
		printk("hang_detect log_store vol_down_key release. ret=%d\n",ret);
	  }
	}
	return 0;
}
...
#ifdef CONFIG_LONG_VOLUPKEY_LOG_STORE
			if (pressed) {
				printk("Long_press_key_timer start linux_keycode=%d\n",linux_keycode);
				timer_setup(&Long_press_volupkey_timer,vol_up_long_press_timefunction,TIMER_DEFERRABLE);
				mod_timer(&Long_press_volupkey_timer, jiffies + 5*HZ);

			} else {
				kpd_print("Long_press_key_timer stop \n");
				del_timer_sync(&Long_press_volupkey_timer);
			}
			if (!pressed &&
				atomic_read(&vol_up_long_press_dump_flag)) {
				atomic_set(&vol_up_long_press_dump_flag, 0);
			}
#endif
		}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	mt_kpd_debugfs();
#ifdef CONFIG_LONG_VOLUPKEY_LOG_STORE
  mutex_init(&capturelog_lock);
  hd_thread = kthread_create(dump_last_log,
				NULL, "dumplogtoexpdb");
  if (IS_ERR(hd_thread))
  		 printk("Failed to create dump_kmsg_expdb_log\n");
  else
  		 printk(" Create dump_kmsg_expdb_log thread : done\n");
  if (hd_thread != NULL)
