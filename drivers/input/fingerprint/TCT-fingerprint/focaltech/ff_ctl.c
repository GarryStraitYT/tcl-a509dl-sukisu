...
extern int ff_ctl_reset_device(int on);
...
//    /* Initialize the PWR/SPI/RST/INT pins resource. */
    ...
    do {
        ...
        if (err) {
			FF_LOGE("request_irq(..) = %d. irq_num %d set to 0", err, g_context->irq_num);
			g_context->irq_num = 0;
            break;
        }
...
extern const char *focaltech_info;
...
static long ff_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    #ifdef CONFIG_TCT_DEVICEINFO
        sprintf(fp_module_name,focaltech_info);
    #endif
        break;
