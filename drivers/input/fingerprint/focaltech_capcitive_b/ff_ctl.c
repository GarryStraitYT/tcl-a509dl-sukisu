...
#else
unsigned char fp_module_name[256];
static ssize_t  fpinfo_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	printk("Dump  %s \n",fp_module_name);
	if (!dev ) {
		return 0;
...
static ssize_t fpinfo_store(struct device* dev, struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}
extern struct device* get_deviceinfo_dev(void);
static DEVICE_ATTR(fp, 0444, fpinfo_show, fpinfo_store);
static void Create_fpinfo_node_ForMMI(void)
{
	struct device * fp;
...
static int ff_ctl_enable_irq(bool on)
{
    ...
    do {   
		static bool is_irq_en = true;
		FF_LOGI("is_irq_en = %d.", is_irq_en);
		/* Control the clock source. */
		if (on && !is_irq_en) {
			enable_irq(g_context->irq_num);
			is_irq_en= true;
		} else if (!on && is_irq_en) {
...
static int ff_ctl_init_driver(void)
{
    ...
    #if defined CONFIG_TCT_DEVICEINFO
    sprintf(fp_module_name,"FT9371:FOCAL:Sunwin:AYB0000219C1");
    #else
    sprintf(fp_module_name,"FT9371:FOCAL:Sunwin:AYB0000219C1");
    Create_fpinfo_node_ForMMI();
    #endif
    FF_LOGV("'%s' leave.", __func__);
    return err;
}
