...
//Add-begin by guang.xiao , add fp node for factory mode
...
#endif
//Add-end by guang.xiao , add fp node for factory mode

#if SF_SPI_RW_EN
static struct of_device_id  sf_of_match[] = {
   // { .compatible = COMPATIBLE_SW_FP, },
    { .compatible = "mediatek,fingerprint-fpsensor", },
};
...
static int sf_ctl_report_key_event(struct input_dev *input, sf_key_event_t *kevent)
{
    ...
    switch (kevent->key) {
        ...
        case SF_KEY_F11:
//add by qiaozhen.li for fp touch key report (camera take photo) begin
            key_code = 255; //FP_CLICK
//add by qiaozhen.li for fp touch key report (camera take photo) end
            break;
    ...
    xprintk(SF_LOG_LEVEL, "%s(...) %d leave.\n", __FUNCTION__, key_code);
    return err;
}
...
#endif
    #if defined CONFIG_TCT_DEVICEINFO
	sprintf(fp_module_name,"SW9006C:Sunwave:Kerr:AYB0000221C1");
    #endif
            break;
        }
...
//Add-end by guang.xiao , add fp node for factory mode 
//add by qiaozhen.li for fp touch key report (camera take photo) begin
    input_set_capability(sf_ctl_dev.input, EV_KEY, 255);
//add by qiaozhen.li for fp touch key report (camera take photo) end
...
static int sf_ctl_init_input(void)
{
	...
	__set_bit(FP_POWER_EVENT, sf_ctl_dev.input->keybit);
...
static int sf_ctl_get_fp_hw_id(void)
{
	...
	if (pn)
	{
            ...
            rst_gpio = of_get_named_gpio(pn, "gpio-rst-std", 0);
            if (rst_gpio < 0)
            {
                xprintk(KERN_INFO,"%s get rst gpio fail\n", __FUNCTION__);
                return -EIO;
            }
	 #else
			of_property_read_u32_array(np, "gpio-id", &id_gpio, 1);
			of_property_read_u32_array(np, "gpio-power", &power_gpio, 1);
            of_property_read_u32_array(np, "gpio-rst", &rst_gpio, 1);
     #endif
        ...
        ret = gpio_request(rst_gpio, "gpio-rst");
        if (ret)
        {
            xprintk(KERN_INFO,"%s rst gpio request fail\n", __FUNCTION__);
            return -EIO;
        }
		...
		xprintk(KERN_INFO,"%s id-std value = %d\n", __FUNCTION__, id_value);
...
#endif
    xprintk(KERN_INFO, "'%s'  spi register driver enter 1\n", __FUNCTION__);
#if SF_SPI_RW_EN
    /**register SPI device、driver***/
    //spi_register_board_info(spi_board_devs, ARRAY_SIZE(spi_board_devs));
    xprintk(KERN_INFO, "'%s'  spi register driver enter\n", __FUNCTION__);
