...
static void ff_ctl_device_event(struct work_struct *ws)
{
    ...
    //__pm_wakeup_event(&g_context->wakelock, 2000); // 2 seconds.
    if (g_context->wakelock!=NULL)
	    __pm_wakeup_event(g_context->wakelock, 2000);
...
static irqreturn_t ff_ctl_device_irq(int irq, void *dev_id)
{
    ...
    //__pm_wakeup_event(&g_context->wakelock, 5000);
    if (g_context->wakelock!=NULL)
	    __pm_wakeup_event(g_context->wakelock, 5000);
...
static int ff_ctl_report_key_event(struct input_dev *input, ff_key_event_t *kevent)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
    printk("'%s' enter. %d", __func__, kevent->code);
...
static int ff_ctl_register_input(void)
{
    ...
    if (g_config) {
        ...
        input_set_capability(g_context->input, EV_KEY, 250);
    }
...
static int ff_ctl_init_driver(void)
{
    ...
    do {
        ...
        if (err) {
            FF_LOGI("'%s' request_irq err=%d", __func__,err);
            break;
        }
...
#define FOCAL_HW_ID 1
static int ff_ctl_get_fp_hw_id(void)
{
	int ret;
	...
	if (pn)
	{
		...
		if (ret)
		{
			FF_LOGE("%s id gpio request fail\n", __FUNCTION__);
			return -EIO;
		}
		...
		if (ret)
		{
			FF_LOGE("%s power gpio request fail\n", __FUNCTION__);
			return -EIO;
		}
...
//static char *FocalFpName = "Focal";
//extern char *g_pFpName;
//extern bool bIsFpProbe;

//Begin Added by yuduan.xie for Task 7029322 on 2018.09.20
#if 0
static int iFpHalInitSucceed = 0;
static ssize_t fp_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	bool bIsInitSucceed = 0;
...
char fp_module_name[32]="NA:NA:NA:0";
...
DEVICE_ATTR(fp, 0664, fp_show, fp_store);

extern struct device* get_deviceinfo_dev(void);
static void Create_FP_info_node_ForMMI(void)
{
	struct device * fp;
...
#endif
//End added by qian.xue for XR5859733 on 2018/01/10
...
/*	if (bIsFpProbe)
	{
		FF_LOGI("'%s' fingerprint has probe", __func__);
		return -EINVAL;
	}
*/
    if (ff_ctl_get_fp_hw_id() != FOCAL_HW_ID)
    {
		FF_LOGE("'%s' hw id error", __func__);
		return -EINVAL;
	}

    #if 0
    Create_FP_info_node_ForMMI();
    ...
    //wakeup_source_init(&ff_ctl_context.wakelock, "focaltech_wakelock");
    ff_ctl_context.wakelock = wakeup_source_register(NULL, "focaltech_wakelock");
    if (ff_ctl_context.wakelock == NULL) {
         printk(KERN_ERR"fail to request focal fingerprint_wakelock\n");
    }
    /* Assign the context instance. */
    g_context  = &ff_ctl_context;
	//g_pFpName  = FocalFpName;
    //bIsFpProbe = 1;

    //Begin add by yuduan.xie for task 8177729 on 2019/08/07
    #if defined CONFIG_TCT_DEVICEINFO
    sprintf(fp_module_name,"FT9362:FOCAL:EACHOPTO");
    #endif
    //End add by yuduan.xie for task 8177729 on 2019/08/07
...
static void __exit ff_ctl_driver_exit(void)
{
    ...
    //wakeup_source_trash(&g_context->wakelock);
    wakeup_source_unregister(g_context->wakelock);
