...
//Begin Added by qianbo.pan for deviceinfo task:11475486 on 2021.08.26
#if defined CONFIG_TCT_DEVICEINFO
extern char fp_module_name[256];
#endif
//End Added by qianbo.pan for deviceinfo task:11475486 on 2021.08.26
...
static bool isFree = false;    //Added by qianbo.pan for task:11391646 on 2021.099.26
...
{
    int err = 0;
//Begin Added by qianbo.pan for task:11391646 on 2021.09.26
    xprintk(KERN_ERR, "%s(..) enter, isFree = %d.\n", __FUNCTION__, isFree);
    if (isFree) {
        return err;
    }
//End Added by qianbo.pan for task:11391646 on 2021.09.26
#if (SF_POWER_MODE_SEL == PWR_MODE_GPIO)
...
static int sf_ctl_device_free_gpio(struct sf_ctl_device *ctl_dev)
{
    ...
    isFree = true;    // Added by qianbo.pan for task:11391646 on 2021.09.26
#if SF_MTK_CPU
...
static int sf_ctl_device_init_gpio_pins(struct sf_ctl_device *ctl_dev)
{
    ...
    struct platform_device *p_irq_dev  = NULL;
    
    sf_ctl_dev = ctl_dev;
    xprintk(SF_LOG_LEVEL, "%s(..) enter.\n", __FUNCTION__);
    ctl_dev->pdev->dev.of_node = of_find_compatible_node(NULL, NULL, "mediatek,fingerprint-pinctrl");
