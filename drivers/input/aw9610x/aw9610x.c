...
#define  TCL_SAR_EVENT_REPORT 1
#ifdef TCL_SAR_EVENT_REPORT
//begin modify by xiakang.chen for task ODIN5G-3333 on 2021-12-31
#include <linux/miscdevice.h>
//end modify by xiakang.chen for task ODIN5G-3333 on 2021-12-31
#endif
...
#ifdef TCL_SAR_EVENT_REPORT
//begin  modify by xiakang.chen for task ODIN5G-3333 on 2021-12-31
void send_aw9610_uevent(enum AW9610_STATUS val,char counter,uint32_t sar_num);
static struct miscdevice aw9610_swtich_dev[2] = {
	{
...
static struct device_attribute qis_attr[2] = {
	{
         .attr = {
		.name = "status",
		.mode = 0444,
...
aw9610x_cfg_all_loaded(const struct firmware *cont, void *context)
{
	...
	ret = awsar_parsing_bin_file(aw_bin);
	if (ret < 0) {
...
static ssize_t aw9610x_aot_cali_get(struct device *dev,
				struct device_attribute *attr,
					char *buf)
{
	uint32_t state;
...
/*modified by zhikui.li begin,Add sar sensor plug and unplug usb calibration function*/
#include <linux/power_supply.h>
static int sar_notifier_call(struct notifier_block *nb, unsigned long ev, void *v)
{
	 struct aw9610x *aw9610x = container_of(nb, struct aw9610x, nb);
     struct power_supply *psy = v;
     union power_supply_propval val;
     int attach = 0;
     uint32_t data_en = 0;
     static int num0_old_attach=-1,num1_old_attach=-1;
     if ((strcmp(psy->desc->name, "charger") == 0) || (strcmp(psy->desc->name, "mtk_charger_type") == 0)) {//Receive USB plug-in notification
         power_supply_get_property(psy, POWER_SUPPLY_PROP_ONLINE, &val);
         attach = val.intval;

         if((aw9610x->sar_num==0) &&(num0_old_attach != attach)){
             pr_err("%s:will sar sensor calibration sar_num=%d\n", __func__,aw9610x->sar_num);
             //sensor_calibration_to_hub(ID_SAR);
             aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
             aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
							(data_en & 0x3f) << 8);
         }
         if((aw9610x->sar_num==1) &&(num1_old_attach != attach)){
              pr_err("%s:will sar sensor calibration sar_num=%d\n", __func__,aw9610x->sar_num);
              //sensor_calibration_to_hub(ID_SAR);
              aw9610x_i2c_read(aw9610x, REG_SCANCTRL0, &data_en);
              aw9610x_i2c_write_bits(aw9610x, REG_SCANCTRL0, ~(0x3f << 8),
         ...
         if(aw9610x->sar_num==0) num0_old_attach = val.intval;
         if(aw9610x->sar_num==1) num1_old_attach = val.intval;
     }
     return NOTIFY_OK;
}
static int sar_register_psy_listener(struct aw9610x *aw9610x)
{
     int rc = 0;
     aw9610x->nb.notifier_call = sar_notifier_call;
     rc = power_supply_reg_notifier(&aw9610x->nb);
     if (rc < 0) {
...
/*modified by zhikui.li end,Add sar sensor plug and unplug usb calibration function*/
...
static DEVICE_ATTR(aot_cali, 0664, aw9610x_aot_cali_get, aw9610x_aot_cali_set);
...
#ifdef TCL_SAR_EVENT_REPORT
//begin  modify by xiakang.chen for task ODIN5G-3333 on 2021-12-31
void send_aw9610_uevent(enum AW9610_STATUS val,char channel,uint32_t sar_num)
{
    char *envp[2];
    char *psensor = NULL;
    if (channel == 0)
    ...
    if(val == 0){   //near
        if (channel == 0){
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_ADD, envp);
        }else if(channel == 1){
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_ONLINE, envp);
        }else{
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_CHANGE, envp);
        }
    }else{                    //far
        if (channel == 0){
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_REMOVE, envp);
        }else if(channel == 1){
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_OFFLINE, envp);
        }else{
        kobject_uevent_env(&aw9610_swtich_dev[sar_num].this_device->kobj, KOBJ_MOVE, envp);
        }
    }
}
//end  modify by xiakang.chen for task ODIN5G-3333 on 2021-12-31
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		...
		if (aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state !=
			...
			case FAR:
			    AWLOGE(aw9610x->dev, "lzk_debug FAR channel=%d sar_num=%d i=%d\n",i,j);
				send_aw9610_uevent(AW9610_STATUS_FAR,i,j);
				break;
			case TRIGGER_TH0:
			    AWLOGE(aw9610x->dev, "lzk_debug NEAR channel=%d sar_num=%d i=%d\n",i,j);
				send_aw9610_uevent(AW9610_STATUS_NEAR,i,j);
				break;
			case TRIGGER_TH1:
				break;
			case TRIGGER_TH2:
...
static void aw9610x_irq_handle(struct aw9610x *aw9610x)
{
	...
	//AWLOGD(aw9610x->dev, "pad = 0x%08x", curr_status);
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
        if(aw9610x->sar_num==0)//lzk_debug
             report_value = 227;
        else
             report_value = 231;
		if(aw9610x->channel_func & (1 << i)){
			...
			if (aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state !=
				...
				switch (aw9610x->aw_pad[j * AW_CHANNEL_MAX + i].curr_state) {
				    case FAR:
				        report_value =  report_value + i*2 + 1;
					    input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, report_value, 1);
					    input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
					    input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, report_value, 0);
					    input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
					    AWLOGE(aw9610x->dev, "lzk_debug0 pad = 0x%08x report_value=%d sar_num=%d i=%d\n",curr_status,report_value,aw9610x->sar_num,i);
					    break;
				    case TRIGGER_TH0:
				        report_value =  report_value + i*2;
					    input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, report_value, 1);
					    input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
					    input_report_key(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, report_value, 0);
					    input_sync(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
					    AWLOGE(aw9610x->dev, "lzk_debug1 pad = 0x%08x report_value=%d ,sar_num=%d i=%d\n",curr_status,report_value,aw9610x->sar_num,i);
					    break;
				    case TRIGGER_TH1:
...
static int32_t aw9610x_interrupt_init(struct aw9610x *aw9610x)
{
	...
	AWLOGE(aw9610x->dev,"enter sar_num=%d",aw9610x->sar_num);
	...
	if (gpio_is_valid(aw9610x->irq_gpio)) {
		...
		if (ret) {
			...
			irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;//lzk_debug
			if(aw9610x->sar_num==0){
			    ret = devm_request_threaded_irq(&aw9610x->i2c->dev,
							    aw9610x->to_irq, NULL,
							    aw9610x_irq, irq_flags,
							    "aw9610x_irq0", aw9610x);
...
aw9610x_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
...
#ifdef TCL_SAR_EVENT_REPORT
    //Begin Modified by xiakang.chen for task ODIN5G-3333 on 2021-12-31
    j = aw9610x->sar_num;
	ret = misc_register(&aw9610_swtich_dev[j]);
    if (ret)
    {
		AWLOGE(&i2c->dev,"accel_factory_device register failed\n");
		goto switch_dev_register_failed;
    }
    send_aw9610_uevent(AW9610_STATUS_FAR,0,j);
    ret=device_create_file(aw9610_swtich_dev[j].this_device, &qis_attr[j]);
    if (ret)
    {
		AWLOGE(&i2c->dev,"sx932 register attr failed\n");
		goto switch_dev_register_failed;
    }
    //End Modified by xiakang.chen for task ODIN5G-3333 on 2021-12-31
#endif
	aw9610x_sw_reset(aw9610x);
	...
	/* input device */
	j = aw9610x->sar_num;
	aw9610x->aw_pad[j * AW_CHANNEL_MAX].input = input_allocate_device();
	if (!(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input)) {
		err_num = i;
		goto exit_input_alloc_failed;
	}
	aw9610x->aw_pad[j * AW_CHANNEL_MAX].input->name =
					pad_event[j * AW_CHANNEL_MAX].name;
	for (i = 0; i < AW_CHANNEL_MAX; i++) {
		if(aw9610x->channel_func & (1 << i)){							
			input_set_capability(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, EV_KEY, 227 + 2*i);
			input_set_capability(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, EV_KEY, 227 + 2*i +1);
			input_set_capability(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, EV_KEY, 231 + 2*i);
			input_set_capability(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input, EV_KEY, 231 + 2*i +1);
		}
	}
	ret = input_register_device(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
	if (ret) {
		AWLOGE(aw9610x->dev, "failed to register input device");
		input_free_device(aw9610x->aw_pad[j * AW_CHANNEL_MAX].input);
		err_num = i;
...
/*modified by zhikui.li begin,Add sar sensor plug and unplug usb calibration function*/
	sar_register_psy_listener(aw9610x);
/*modified by zhikui.li end,Add sar sensor plug and unplug usb calibration function*/
	return AW_SAR_SUCCESS;
...
static int aw9610x_resume(struct device *dev)
{
	...
	aw9610x_i2c_write(aw9610x, REG_CMD, AW9610X_ACTIVE_MODE);
	enable_irq(aw9610x->to_irq);
	//aw9610x_i2c_write(aw9610x, 0x0000, 0x00001f1f);
    aw9610x_interrupt_clear(aw9610x);//lzk_debug
	return 0;
}
