...
#endif
/* Begin add by gaoxiang.zou for ctp info */
extern struct device* get_deviceinfo_dev(void);
static ssize_t  ctp_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	if ((fwver == 0xFF) || (fwver == 0x00))
		num_read_chars = snprintf(buf, PAGE_SIZE, "get tp fw version fail! please turn on lcd!\n");
	else
		num_read_chars = snprintf(buf, PAGE_SIZE, "FT8756_1st:CDOT:%02x\n", fwver);
		//num_read_chars = snprintf(buf, PAGE_SIZE, "FT8756_1st:CDOT\n");

	mutex_unlock(&input_dev->mutex);
	return num_read_chars;
}

static ssize_t ctp_store(struct device* dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return count;
}

static DEVICE_ATTR(ctp, 0444, ctp_show, ctp_store);

static void create_ctp_node_forMMI(void)
{
	struct device * ctp;
	...
	if (device_create_file(ctp, &dev_attr_ctp) < 0) {
		pr_err("Failed to create ctp device file(%s)!\n", dev_attr_ctp.attr.name);
	}

	return;
}
/* End add by gaoxiang.zou for ctp info */
...
#endif
    /* Begin add by gaoxiang.zou for ctp info */
    printk("Create CTP device info node for MMI \n");
