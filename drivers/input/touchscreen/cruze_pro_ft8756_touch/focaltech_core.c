...
#ifdef CONFIG_TCT_PROJECT_CRUZE_PRO
...
/* Begin modify for TP process */
#ifdef CONFIG_TCT_PROJECT_CRUZE_PRO
extern void (*tct_grip_mode_set)(u8 screen_mode, u8 level);
//0x8C register
void fts_edge_process(u8 screen_mode, u8 level)
{
	int ret = 0;
	u8 writebuf[2] = { 0 };

    if (fts_data->pm_suspend) {
        FTS_ERROR("Dont write tp register when tp suspend");
        return;
    }

	if (VERTICAL == screen_mode) {
		writebuf[0] = 0x8C;
		writebuf[1] = 0; //portrait
	} else if (HORIZONTAL_LEFT == screen_mode || HORIZONTAL_RIGHT == screen_mode) {
		writebuf[0] = 0x8C;
		writebuf[1] = 1; //landscape
	} else {
		FTS_INFO("error param %d", screen_mode);
		return;
	}

	ret = fts_write(writebuf, 2);
	if(0 != ret){
		FTS_ERROR("write err %d", ret);
	}
...
static unsigned int charge_mode_status = 0; // for resume restore

//0x8B register
void fts_charge_process(unsigned int enable)
{
	int ret = 0;
	u8 writebuf[2] = { 0 };

	//record status for resume restore.
	charge_mode_status = enable;
	...
	if (0 == enable) {
		writebuf[0] = 0x8B;
		writebuf[1] = 0; //plug-out
	} else if (1 == enable) {
		writebuf[0] = 0x8B;
		writebuf[1] = 1; //plug-in
	} else {
		FTS_INFO("error param %d", enable);
		return;
	}
...
static int fts_power_source_ctrl(struct fts_ts_data *ts_data, int enable)
{
    ...
    if (enable) {
        if (ts_data->power_disabled) {
            ...
            gpio_direction_output(ts_data->pdata->reset_gpio, 1); //iovcc On, reset pin High
            msleep(1);
...
/* Begin meng.zhang power LCD bias gpio on 20210719 */

    pdata->biasneg_gpio = of_get_named_gpio_flags(np, "focaltech,bias-neg-gpio",
                      0, &pdata->biasneg_gpio_flags);
    if (pdata->biasneg_gpio < 0)
        FTS_ERROR("Unable to get biasneg_gpio");

    pdata->biaspos_gpio = of_get_named_gpio_flags(np, "focaltech,bias-pos-gpio",
                      0, &pdata->biaspos_gpio_flags);
    if (pdata->biaspos_gpio < 0)
        FTS_ERROR("Unable to get biaspos_gpio");

    FTS_INFO("biasneg_gpio:%d, biaspos_gpio:%d", pdata->biasneg_gpio, pdata->biaspos_gpio);

/* End meng.zhang power LCD bias gpio on 20210719 */
...
static void fts_lcd_bias_off()
{
	int ret = 0;
	struct fts_ts_data *ts_data = fts_data;

	//delay more than 4 frame to make sure cmd take effect.
	msleep(80);

	//power LCD bias gpio on 20210719
	if (gpio_is_valid(ts_data->pdata->biasneg_gpio)) {
		ret = gpio_request(ts_data->pdata->biasneg_gpio, "fts_biasneg_gpio");
		if (ret) {
			FTS_ERROR("[biasneg_gpio]gpio request failed");
		}

		ret = gpio_direction_output(ts_data->pdata->biasneg_gpio, 0);
		if (ret) {
			FTS_ERROR("[biasneg_gpio]set_direction failed");
		}

		FTS_INFO("free biasneg_gpio=%d", ts_data->pdata->biasneg_gpio);
		gpio_free(ts_data->pdata->biasneg_gpio);
	}

	if (gpio_is_valid(ts_data->pdata->biaspos_gpio)) {
		ret = gpio_request(ts_data->pdata->biaspos_gpio, "fts_biaspos_gpio");
		if (ret) {
			FTS_ERROR("[biaspos_gpio]gpio request failed");
		}

		ret = gpio_direction_output(ts_data->pdata->biaspos_gpio, 0);
		if (ret) {
			FTS_ERROR("[biaspos_gpio]set_direction failed");
		}

		FTS_INFO("free biaspos_gpio=%d", ts_data->pdata->biaspos_gpio);
		gpio_free(ts_data->pdata->biaspos_gpio);
	}
}
...
//Begin add by dingting.meng for T-11008328 on 2021.4.1
#if FTS_TEST_EN
...
#endif
//End add by dingting.meng for T-11008328 on 2021.4.1
...
/* Begin meng.zhang make IC deep standby 20210619 */
#ifdef CONFIG_TCT_PROJECT_CRUZE_PRO
        FTS_INFO("make TP enter deep standby");
        ret = fts_write_reg(0xFD, 0x5A);
        if (ret < 0)
            FTS_ERROR("set TP to deep standby fail, ret=%d", ret);
#endif
/* End meng.zhang make IC deep standby 20210619 */
...
/* Begin modify for TP charge process restore */
#ifdef CONFIG_TCT_PROJECT_CRUZE_PRO
	if(1 == charge_mode_status){
		fts_write_reg(0x8B, 0x01);
	}
    if (tct_ts_data != NULL)
        fts_edge_process(tct_ts_data->tct_grip_mode, tct_ts_data->tct_grip_level);
#endif
/* End modify for TP charge process restore */
...
/* Begin modify for TP edge process */
#ifdef CONFIG_TCT_PROJECT_CRUZE_PRO
	tct_grip_mode_set = fts_edge_process;
	tct_set_charge_process_en = fts_charge_process;
#endif
/* End modify for TP edge process */
