...
#else
const struct cts_firmware *cts_request_newer_firmware_from_fs(
        struct cts_device *cts_dev, const char *filepath, u16 curr_version)
{
    struct cts_firmware *firmware;
    ...
    ret = request_firmware(&fw_entry, filepath, &cts_dev->pdata->spi_client->dev);
    if (ret < 0) {
        cts_info("request firmware faild %d", ret);
        return NULL;
    }
    ...
    pos = FIRMWARE_VERSION_OFFSET;
    buff[0] = *(fw_entry->data + pos);
    buff[1] = *(fw_entry->data + pos + 1);
    read_size = buff[0] | buff[1] << 8;
    cts_err("pos:%08x, read_size:0x%04x", pos, read_size);
    ...
    firmware->data = (u8 *)vmalloc(firmware->size);
    memcpy(firmware->data, fw_entry->data, fw_entry->size);
    if (firmware->data == NULL) {
...
const struct cts_firmware *cts_request_firmware_from_fs(struct cts_device *cts_dev,
            const char *filepath)
{
        ...
        const char *filepath, bool to_flash)
{
    ...
    cts_info("Update from file '%s' to %s", CFG_CTS_FIRMWARE_FILEPATH,
        to_flash ? "flash" : "sram");
...
const struct cts_firmware *cts_request_firmware(struct cts_device *cts_dev,
