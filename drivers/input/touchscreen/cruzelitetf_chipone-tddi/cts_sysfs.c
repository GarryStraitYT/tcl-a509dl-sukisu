        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    cts_info("Update firmware from file '%s'", CFG_CTS_FIRMWARE_FILEPATH);

    firmware = cts_request_firmware_from_fs(cts_dev, CFG_CTS_FIRMWARE_FILEPATH);
    if (firmware == NULL) {
...
#ifdef CFG_TCL_EDGE_INHIBITION
static ssize_t edge_inhibition_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (argc > 2) {
        cts_err("Invalid num args %d\n"
                "  echo enable [0/1] > edge_inhibition\n", argc);
        return -EFAULT;
    }

    ret = kstrtobool(argv[0], &cts_dev->rtdata.edge_enable);
    if (ret) {
        cts_err("Invalid param of enable");
        return ret;
    }

    cts_info("Set edge inhibition mode %d", cts_dev->rtdata.edge_enable);

    if (cts_dev->rtdata.program_mode) {
        cts_warn("Send command %d while chip in program mode",
            cts_dev->rtdata.edge_enable);
        return -ENODEV;
    }
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_EDGE_INHIBITION,
        cts_dev->rtdata.edge_enable ? 1 : 0, 3, 0);
    if (ret) {
        cts_err("Send EDGE_INHIBITION %d failed %d",
            cts_dev->rtdata.edge_enable, ret);
    }

    return count;
}
static DEVICE_ATTR(edge_inhibition, S_IWUSR,
    NULL, edge_inhibition_store);
#endif

#ifdef CFG_TCL_LANDSCAPE_MODE
/**
 * mode:
 * 0:normal
 * 1:fringe on the left
 * 2:fringe on the right
 */
static ssize_t landscape_mode_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (argc > 2) {
        cts_err("Invalid num args %d\n"
                "  echo mode [0/1/2] > landscape_mode\n", argc);
        return -EFAULT;
    }
    ret = kstrtou8(argv[0], 0, &cts_dev->rtdata.landscape_mode);
    if (ret) {
        cts_err("Invalid param of enable");
        return ret;
    }
    cts_info("Set landscape mode %d", cts_dev->rtdata.landscape_mode);

    if (cts_dev->rtdata.program_mode) {
        cts_warn("Send command %d while chip in program mode", cts_dev->rtdata.landscape_mode);
        return -ENODEV;
    }
    ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_LANDSCAPE_MODE,
        cts_dev->rtdata.landscape_mode, 3, 0);
    if (ret) {
        cts_err("Send LANDSCAPE_MODE %d failed %d",
            cts_dev->rtdata.landscape_mode, ret);
    }

    return count;
}
static DEVICE_ATTR(landscape_mode, S_IWUSR,
    NULL, landscape_mode_store);


extern void (*tct_grip_mode_set)(u8 screen_mode, u8 level);
void chipone_grip_mode_set(u8 screen_mode, u8 level)
{
	int ret;
	//struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    	struct cts_device *cts_dev = &cts_data->cts_dev;
	cts_dev->rtdata.landscape_mode = screen_mode - 1;
	cts_info("tct Set landscape mode %d", cts_dev->rtdata.landscape_mode);
	if (cts_dev->rtdata.program_mode) {
       		cts_warn("tct Send command %d while chip in program mode", cts_dev->rtdata.landscape_mode);
        	return;
    	}
    	ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_LANDSCAPE_MODE,
        cts_dev->rtdata.landscape_mode, 3, 0);
    	if (ret) {
        	cts_err("tct Send LANDSCAPE_MODE %d failed %d",
            	cts_dev->rtdata.landscape_mode, ret);
    	}
}
#endif

#ifdef CFG_TCL_CHARGER_MODE
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    if (argc > 2) {
        cts_err("Invalid num args %d\n"
                "  echo 0/1 [disable/enable] > charger_mode\n", argc);
        return -EFAULT;
    }

    ret = kstrtobool(argv[0], &cts_dev->rtdata.charger_enable);
    if (ret) {
        cts_err("Invalid param of enable");
        return ret;
    }

    cts_info("Set dev charger %s", cts_dev->rtdata.charger_enable ? "Enable" : "Disable");
    ret = cts_send_command(cts_dev, !!cts_dev->rtdata.charger_enable ?
            CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
    if (ret) {
        cts_err("Send CMD_CHARGER_%s failed %d",
            cts_dev->rtdata.charger_enable ? "ATTACHED" : "DETACHED",
            ret);
    }

    return count;
}
static DEVICE_ATTR(charger_mode, S_IWUSR,
    NULL, charger_mode_store);


extern void (*tct_set_charge_process_en)(unsigned int enable);
void chipone_set_charge_en(unsigned int enable)
{
	int ret;
	//struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    	struct cts_device *cts_dev = &cts_data->cts_dev;
	if (enable == 1){
		cts_dev->rtdata.charger_enable = true;
	}else{
		cts_dev->rtdata.charger_enable = false;
	}
	cts_info("tct Set dev charger %s", cts_dev->rtdata.charger_enable ? "Enable" : "Disable");
    	ret = cts_send_command(cts_dev, !!cts_dev->rtdata.charger_enable ?
            CTS_CMD_CHARGER_ATTACHED : CTS_CMD_CHARGER_DETACHED);
    	if (ret) {
        	cts_err("tct Send CMD_CHARGER_%s failed %d",
            	cts_dev->rtdata.charger_enable ? "ATTACHED" : "DETACHED",
            	ret);
    	}
...
#ifdef CFG_TCL_PALM
static ssize_t palm_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
    struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    ...
    if (argc > 2) {
        cts_err("Invalid num args %d\n"
                "  echo 0/1 [disable/enable] > palm\n", argc);
        return -EFAULT;
    }

    ret = kstrtobool(argv[0], &cts_dev->rtdata.palm_enable);
    if (ret) {
        cts_err("Invalid param of enable");
        return ret;
    }

    cts_info("Set palm %s", cts_dev->rtdata.palm_enable ? "Enable" : "Disable");
    ret = cts_send_command(cts_dev, !!cts_dev->rtdata.palm_enable ?
        CTS_CMD_PALM_ATTACHED : CTS_CMD_PALM_DETACHED);
    if (ret) {
        cts_err("Send CMD_PALM failed %d", ret);
    }

    return count;
}
static DEVICE_ATTR(palm, 0664,
    NULL, palm_store);
#endif

#ifdef CFG_TCL_EARJACK
static ssize_t earjack_store(struct device *dev,
        struct device_attribute *attr, const char *buf, size_t count)
{
	struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
	struct cts_device *cts_dev = &cts_data->cts_dev;
	...
	if (argc > 2) {
        	cts_err("Invalid num args %d\n"
                	"  echo 0/1 [disable/enable] > earjack\n", argc);
        	return -EFAULT;
   	}

	ret = kstrtobool(argv[0], &cts_dev->rtdata.earjack_enable);
    	if (ret) {
        	cts_err("Invalid earjack of enable");
        	return ret;
    	}

    	cts_info("Set dev earjack %s", cts_dev->rtdata.earjack_enable ? "ATTACHED" : "DETATCHED");

    	ret = cts_send_command(cts_dev,
        	!!cts_dev->rtdata.earjack_enable ? CTS_CMD_EARJACK_ATTACHED : CTS_CMD_EARJACK_DETACHED);
    	if (ret) {
        	cts_err("Send CMD_EARJACK_%s failed %d(%s)",
            	cts_dev->rtdata.earjack_enable ? "ATTACHED" : "DETACHED",
            	ret, cts_strerror(ret));
    	}
...
static DEVICE_ATTR(earjack, 0664,
    NULL, earjack_store);
#endif
...
#ifdef CFG_TCL_EDGE_INHIBITION
    &dev_attr_edge_inhibition.attr,
#endif
#ifdef CFG_TCL_LANDSCAPE_MODE
    &dev_attr_landscape_mode.attr,
#endif
#ifdef CFG_TCL_CHARGER_MODE
...
#define COMP_CAP_TEST_DATA_FILENAME     "comp-cap-test-data.txt"
    ...
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Reset-Pin", reset_pin_test_result, 0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Int-Pin", int_pin_test_result,0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Rawdata", rawdata_test_result,0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Noise", noise_test_result,0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Open", open_test_result,0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Short", short_test_result,0);
    count += cts_print_test_result_to_buffer(buf + count, PAGE_SIZE,
        "Comp-Cap", comp_cap_test_result,0);
...
#ifdef CFG_TCL_LANDSCAPE_MODE
	tct_grip_mode_set = chipone_grip_mode_set;
#endif
#ifdef CFG_TCL_CHARGER_MODE
	tct_set_charge_process_en = chipone_set_charge_en;
#endif
