...
int cts_resume_device(struct cts_device *cts_dev)
{
    ...
    if (cts_dev->rtdata.edge_enable) {
        cts_info("Set edge enable");
        ret = cts_fw_reg_writeb_retry(cts_dev, CTS_DEVICE_FW_REG_EDGE_INHIBITION,
                1, 3, 0);
        if (ret)
            cts_err("Send EDGE_INHIBITION Enable failed %d", ret);
    }
#endif
...
static inline void cts_init_rtdata_with_normal_mode(struct cts_device *cts_dev)
{
...
#ifdef CFG_TCL_EDGE_INHIBITION
    cts_dev->rtdata.edge_enable = false;
#endif
#ifdef CFG_TCL_LANDSCAPE_MODE
    ...
    cts_dev->rtdata.earjack_enable = false;
#endif
}
...
static void cts_update_firmware_work(struct work_struct *work)
{
    struct chipone_ts_data *cts_data = container_of(to_delayed_work(work),
        struct chipone_ts_data, update_work);
    struct cts_device *cts_dev = &cts_data->cts_dev;

    int  ret, retries = 0;
    const struct cts_firmware *firmware = NULL;

    cts_info("cts_update_firmware_work start!");
    cts_info("hwid: %06x fwid: %04x fw_ver: %04x", cts_dev->hwdata->hwid,
            cts_dev->hwdata->fwid, cts_dev->fwdata.version);
...
update_firmware:
    if (firmware) {
        ...
        if (cts_dev->hwdata->fwid == CTS_DEV_FWID_INVALID) {
...
init_hwdata:
    ...
    cts_dev->fwdata.version = device_fw_ver;


    cts_data->update_workqueue =
        create_singlethread_workqueue(CFG_CTS_DEVICE_NAME "-update_workqueue");
    if (cts_data->update_workqueue == NULL) {
        cts_err("Create update workqueue failed");
        return -ENOMEM;
    }
    INIT_DELAYED_WORK(&cts_data->update_work, cts_update_firmware_work);
    queue_delayed_work(cts_data->update_workqueue,
            &cts_data->update_work, msecs_to_jiffies(14000));

    cts_info("Start update firmware after 14 seconds!");
