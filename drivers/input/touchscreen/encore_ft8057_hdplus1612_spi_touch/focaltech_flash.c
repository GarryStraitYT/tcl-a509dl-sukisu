...
/* Begin add for TP device info */
#ifdef CONFIG_TCT_DEVICEINFO
extern char ctp_module_name[256];
static void deviceinfo_module_name_copy(void){
	int ret = 0;
	unsigned char ver = 0;
	...
	ret = fts_read_reg(FTS_REG_FW_VER, &ver); //0xA6-->0x87 0x56
	if(ret < 0){
		FTS_ERROR("read firmware version failed,set version to 0!\n");
		ver = 0;
	}
	if(fts_data->ic_info.ids.chip_idh == 0x80 && fts_data->ic_info.ids.chip_idl == 0x57){
		sprintf(ctp_module_name, "FT8057:TDT:0x%x:AUC0660119C1", ver);
	} else {
		sprintf(ctp_module_name, "FT8057:TDT:NA:AUC0660119C1");
	}
	mutex_unlock(&input_dev->mutex);

	printk("[LCD] ctp_module_name = %s\n", ctp_module_name);

	return;
}
#endif
/* End add for TP device info */
