...
void fts_charger_detect_mode(unsigned int value)
{
    int ret = -1;
    u8 read_data = 0;
    ...
    if (ret < 0) {
        FTS_ERROR("write FTS_REG_CHARGER_MODE_EN to %d fail", value);
    }else {
        FTS_ERROR("write FTS_REG_CHARGER_MODE_EN to %d successfully", value);
    }

    ret = fts_read_reg(FTS_REG_CHARGER_MODE_EN, &read_data);
    if (ret < 0) {
        FTS_ERROR("read FTS_REG_CHARGER_MODE_EN fail, ret = %d", ret);
    }else {
        FTS_ERROR("read FTS_REG_CHARGER_MODE_EN successfully, read_data = 0x%x", read_data);
    }
}
EXPORT_SYMBOL(fts_charger_detect_mode);

void fts_grip_mode_set(u8 screen_mode, u8 level)
{
	int ret = -1;
	u8 read_data = 0;
	...
	switch(screen_mode) {
		case 1:
			ret = fts_write_reg(FTS_REG_EDGE_MODE_EN, 0);//vertical
			if (ret < 0) {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN fail, ret = %d", ret);
			}else {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN successfully, ret = %d", ret);
				ret = fts_read_reg(FTS_REG_EDGE_MODE_EN, &read_data);
				if (ret < 0) {
					FTS_ERROR("read FTS_REG_EDGE_MODE_EN fail, ret = %d", ret);
				}else {
					FTS_ERROR("read FTS_REG_EDGE_MODE_EN successfully, read_data = %d", read_data);
				}
			}
			break;
		case 2:
			ret = fts_write_reg(FTS_REG_EDGE_MODE_EN, 2);//left rotation
			if (ret < 0) {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN fail, ret = %d", ret);
			}else {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN successfully, ret = %d", ret);
			...
			break;
		case 3:
			ret = fts_write_reg(FTS_REG_EDGE_MODE_EN, 1);//right rotation
			if (ret < 0) {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN fail, ret = %d", ret);
			}else {
				FTS_ERROR("write FTS_REG_EDGE_MODE_EN successfully, ret = %d", ret);
...
EXPORT_SYMBOL(fts_grip_mode_set);
