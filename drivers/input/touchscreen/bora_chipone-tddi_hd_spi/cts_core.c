...
void cts_set_prox_en(unsigned int enable)
{
	...
	ret = cts_send_command(cts_dev,
		enable ? CTS_CMD_ENABLE_PROXIMITY : CTS_CMD_DISABLE_PROXIMITY);
	if (ret)
...
#ifdef CONFIG_CTS_TP_PROXIMITY
		u8 ps_status = 0;
		ret = cts_fw_reg_readsb(cts_dev, CTS_DEVICE_FW_REG_TOUCH_INFO, &ps_status, 1);
		if (ret)
			cts_err("Get 0x1000 info failed %d in suspend mode", ret);

		if(open_near || (ps_status & CTS_CMD_PROXIMITY_STATUS)) {
			cts_dbg("4.Get ps status data = %x from TP, pre_status: %d, open_near status: %d(%s)",
				ps_status, pre_status, open_near, open_near ? "near" : "far");
				
			if (ps_status & CTS_CMD_PROXIMITY_STATUS) {
				prox_cur_status = 1;
				open_near = 1 ;
			} else if (((ps_status & CTS_CMD_PROXIMITY_STATUS) == 0) && open_near) {
				prox_cur_status = 2;
				open_near = 0 ;
