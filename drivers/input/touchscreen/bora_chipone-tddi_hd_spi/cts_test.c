...
unlock_device:
    ...
    r = cts_start_device(cts_dev);//bo_liu 未检查的返回值
	if (r){
		cts_err("cts_start_device error");
	}
    ...
    struct cts_test_param *param)
{
    ...
    if (cts_dev == NULL || param == NULL ||
        ...
        /*cts_err("Rawdata test with invalid param: priv param: %p size: %d",
            param->priv_param, param->priv_param_size);*/ // bo_liu, 在 null 检查后解引用
    ...
    struct cts_test_param *param)
{
    ...
    if (cts_dev == NULL || param == NULL ||
        ...
        /*cts_err("Gesture rawdata test with invalid priv param: %p size: %d",
            param->priv_param, param->priv_param_size);*/ //bo_liu 在 null 检查后解引用
		cts_err("Gesture rawdata test with invalid priv param");
        return -EINVAL;
    }
