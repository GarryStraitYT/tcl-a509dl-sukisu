...
#if defined(CONFIG_TCT_BSP_FEATURE)
...
int8_t nvt_customizeCmd(uint8_t u8Cmd)
{
	...
	#if defined(NVTFLASH_WORK_PROTECT)		
...
void nvt_xxx_set_charger_bit(unsigned int state)
{
	int32_t i32ret = 0;
	NVT_LOG("nvt_xxx_set_charger_bit enter, state=%d\n", state);
	i32ret = tpd_usb_plugin(state);
	ts->charge_mode = state;
	NVT_LOG("i32ret=[%d]\n",i32ret);///jx
}

int nvt_xxx_read_charger_bit(void)
{
	uint8_t buf[8] = {0};
	//---set xdata index to EVENT BUF ADDR---
	nvt_set_page(ts->mmap->EVENT_BUF_ADDR | EVENT_MAP_HOST_CMD);
...
#define	CMD_EDGE_REJECT_VERTICAL	(0x7A)
#define	CMD_EDGE_REJECT_LEFT_Up  	(0x7B)
#define	CMD_EDGE_REJECT_RIGHT_Up 	(0x7C)
...
void tpd_set_EdgeRestain_Switch(unsigned int i32Switch)
{
	int32_t i32ret = 0;
	...
	ts->edge_mode = i32Switch;
	NVT_LOG("para=[%d],i32ret=[%d]\n", i32Switch,i32ret);///jx
}

#define	CMD_PLAY_GAME_ON_MODE	(0x7D)
#define	CMD_PLAY_GAME_OFF_MODE  	(0x7E)

#define PLAY_GAME_ON (1)
#define PLAY_GAME_OFF (0)


int tpd_set_PlayGame_Switch(unsigned int i32Switch)
{
	int32_t i32ret = 0;
	if(i32Switch == PLAY_GAME_ON)
		i32ret = nvt_customizeCmd(CMD_PLAY_GAME_ON_MODE);
	else if(i32Switch == PLAY_GAME_OFF)
		i32ret = nvt_customizeCmd(CMD_PLAY_GAME_OFF_MODE);
	else{
		i32ret = (-1);
	}
	ts->game_mode = i32Switch;
	NVT_LOG("para=[%d],i32ret=[%d]\n",i32Switch,i32ret);///jx
	return	i32ret;
}

void nvt_ext_mode_resume(void)
{
	int ret = 0;
	...
	if (ts->game_mode)
		ret = nvt_customizeCmd(CMD_PLAY_GAME_ON_MODE);
	else
		ret = nvt_customizeCmd(CMD_PLAY_GAME_OFF_MODE);
	if (ret < 0)
        NVT_LOG("tpd_usb_plugin 0x%02X cmd failed.\n", CMD_PLAY_GAME_ON_MODE);

	tpd_set_EdgeRestain_Switch(ts->edge_mode);
}
...
void nvt_grip_mode_set(u8 screen_mode, u8 level)
{
	if (screen_mode == VERTICAL)
		tpd_set_EdgeRestain_Switch(EDGE_REJECT_VERTICAL);
	else if (screen_mode == HORIZONTAL_LEFT)
		tpd_set_EdgeRestain_Switch(EDGE_REJECT_LEFT_Up);
	else if (screen_mode == HORIZONTAL_RIGHT)
		tpd_set_EdgeRestain_Switch(EDGE_REJECT_RIGHT_Up);
	else
		NVT_ERR("wrong parameters, screen_mode=%d\n", screen_mode);
...
#if defined(CONFIG_TCT_BSP_FEATURE)
	tct_set_charge_process_en = nvt_xxx_set_charger_bit;
	tct_grip_mode_set = nvt_grip_mode_set;
#endif
	ts->charge_mode = 0;
	ts->game_mode = 0;
	ts->edge_mode = EDGE_REJECT_VERTICAL;
