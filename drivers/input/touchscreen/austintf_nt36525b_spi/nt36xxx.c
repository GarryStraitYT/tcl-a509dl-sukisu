...
const uint16_t gesture_key_array[] = {
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_C
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_W
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_V
	KEY_DOUBLEWAKEUP,  //GESTURE_DOUBLE_CLICK
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_Z
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_M
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_O
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_e
	KEY_DOUBLEWAKEUP,  //GESTURE_WORD_S
	KEY_DOUBLEWAKEUP,  //GESTURE_SLIDE_UP
	KEY_DOUBLEWAKEUP,  //GESTURE_SLIDE_DOWN
	KEY_DOUBLEWAKEUP,  //GESTURE_SLIDE_LEFT
	KEY_DOUBLEWAKEUP,  //GESTURE_SLIDE_RIGHT
};
#endif
...
#ifdef CONFIG_TCT_DEVICEINFO
	snprintf(ctp_module_name, 256, "NT36525B:TDT:0x01:NA");
#endif
...
//Begin add by bing-zhang for 11698878 on 2022/01/10
#if defined(CONFIG_TCT_PROJECT_AUSTINTF)
extern void (*tct_set_double_wakeup_en)(unsigned int enable);

void nt36xxx_double_wakeup_process(unsigned int enable)
{

	struct nvt_ts_data *ts_data = ts;

    if (ts->pm_suspend) {
        NVT_LOG("Dont write tp register when tp suspend");
        return;
    }

 	mutex_lock(&ts_data->input_dev->mutex);
    if (!enable) {
        NVT_LOG("nvt_ts disable gesture");
        ts_data->gesture_mode = NVT_GESTURE_DIS;
    } else if (1 == enable) {
        NVT_LOG("nvt_ts enable gesture");
        ts_data->gesture_mode = NVT_GESTURE_EN;
    }
...
#endif
//End add by bing-zhang for 11698878 on 2022/01/10
...
static int32_t nvt_parse_dt(struct device *dev)
{
	...
	if (2 == androidboot_dtbo_idx) {
		ts->reset_gpio = of_get_named_gpio_flags(np, "novatek,reset-gpio-v1_0", 0, &ts->reset_flags);
	}else {
...
*/
   ...
   if (POINT_DATA_LEN >= POINT_DATA_CHECKSUM_LEN) {
	...
	if((point_data[1]==0xF0) && (point_data[2]==0x04) && (point_data[3]==0x01)){
		...
		input_report_key(ts->input_dev, KEY_GESTURE_PALMIN, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_GESTURE_PALMIN, 0);
		input_sync(ts->input_dev);
	}else if((point_data[1]==0xF0) && (point_data[2]==0x04) && (point_data[3]==0x02)){
		NVT_LOG ("[NVT-ts] got PalmLeave\n");
		input_report_key(ts->input_dev, KEY_GESTURE_PALMOUT, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_GESTURE_PALMOUT, 0);
		input_sync(ts->input_dev);
	}
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	//Begin add by bing-zhang for 11698878 on 2022/01/10
	#if defined(CONFIG_TCT_PROJECT_AUSTINTF)
	tct_set_double_wakeup_en = nt36xxx_double_wakeup_process;
	#endif
	//End add by bing-zhang for 11698878 on 2022/01/10
	...
	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_PALMIN);
