...
extern void fts_charger_detect_mode(unsigned int value);
#endif

#ifdef TCT_FEATURE_TOUCH_EDGE
extern void (*tct_grip_mode_set)(u8 screen_mode, u8 level);
extern void fts_grip_mode_set(u8 screen_mode, u8 level);
#endif
...
static int fts_read_touchdata(struct fts_ts_data *ts_data, u8 *buf)
{
	...
	#ifdef CFG_TCL_PALM
	if (fts_palm_reopen) {
		tct_fts_palm_report(ts_data, buf[0]);
	}else {
		fts_palm_reopen = true ;
	}
	#endif
...
*/
    ...
    #ifdef TCT_FEATURE_TOUCH_EDGE
    tct_grip_mode_set = fts_grip_mode_set;
    #endif

    #ifdef TCT_FEATURE_TOUCH_CHARGE
    tct_set_charge_process_en = fts_charger_detect_mode;
    #endif
