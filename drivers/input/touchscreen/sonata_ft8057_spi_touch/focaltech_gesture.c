...
void tct_fts_palm_report(struct fts_ts_data *ts_data, u8 data)
{
	...
	palm_state = (data >> 6) & 0x01;

	if (palm_state) {
		input_report_key(input_dev, TCL_PALM_IN, 1);
		input_sync(input_dev);
		msleep(100);
		input_report_key(input_dev, TCL_PALM_IN, 0);
		input_sync(input_dev);
		printk("fts palm_state = %d",palm_state);
		fts_palm_reopen = false ;
	}
}
#endif
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
	...
	#ifdef CFG_TCL_PALM
	__set_bit(TCL_PALM_IN, input_dev->keybit);
	#endif
