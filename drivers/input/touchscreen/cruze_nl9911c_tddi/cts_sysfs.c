...
extern void (*tct_grip_mode_set)(unsigned int screen_mode, unsigned int level);
void chipone_grip_mode_set(unsigned int screen_mode, unsigned int level)
{
	int ret;
	//struct chipone_ts_data *cts_data = dev_get_drvdata(dev);
    struct cts_device *cts_dev = &cts_data->cts_dev;
    if (screen_mode >= 1 && screen_mode<=3)
        ...
        cts_err("error screen_mode:%d", screen_mode);
        return;
    }
...
static DEVICE_ATTR(palm, S_IWUSR,
    NULL, palm_store);
#endif
