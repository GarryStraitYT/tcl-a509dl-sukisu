...
#else
extern unsigned char ctp_module_name_temp[128];
#endif
...
//Begin add by bing-zhang rawdata for 11492114 on 20210923
static ssize_t  ctpinfo_show(struct device *dev,
	                           struct device_attribute *attr, char *buf)
{
     printk("Dump  %s \n",ctp_module_name_temp);
...
static DEVICE_ATTR(ctp,     0444, ctpinfo_show, ctpinfo_store);
...
//End add by bing-zhang rawdata for 11492114 on 20210923
...
#ifdef CONFIG_TCT_DEVICEINFO
  	snprintf(ctp_module_name, 128, "NT36525B:TDT:0x%02X:NA", ts->fw_ver);
#else
	snprintf(ctp_module_name_temp, 128, "NT36525B:TDT:0x%02X:NA", ts->fw_ver);
#endif
