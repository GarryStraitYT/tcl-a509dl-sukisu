...
//Begin add by bing-zhang for RATF-1256 on 2022/01/24
unsigned char ctp_module_name[128];
...
////End add by bing-zhang for RATF-1256 on 2022/01/24
...
static int is_sm_fb_notify_register = 0;//bo_liu add temp
...
int semi_touch_work_done(void)
{
    ...
    check_return_if_fail(ret, NULL);
	is_sm_fb_notify_register = 1;//bo_liu add temp
    return ret;
}
int semi_touch_resource_release(void)
{
	//bo_liu modify temp
	if (is_sm_fb_notify_register == 1){
		printk("bo_liu, [%s][%d], \n", __func__, __LINE__);
...
static int semi_touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    //Begin add by bing-zhang for RATF-1256 on 2022/01/24
    snprintf(ctp_module_name, 128, "CHSC5448:TWS");
