...
extern struct chipone_ts_data *cts_data_gl;
...
{
	int ret = 0;
	//Begin modify by bing-zhang for ENCORETF-8650 on 2022/12/09
	int retries = -1;
	//End modify by bing-zhang for ENCORETF-8650 on 2022/12/09
	u8 tdata[4] = { 'R', 'S', 'T', '!' };
...
{
	int ret;
	struct chipone_ts_data *cts_data = cts_data_gl;
...
void cts_firmware_upgrade_work(struct work_struct *work)
{
	...
	//Begin add by bing-zhang for ENCORETF-5830 on 2022/08/11
	cts_lock_device(cts_dev);
	retries = 0;
	do {
		#ifdef CONFIG_CTS_I2C_HOST
	...
	cts_unlock_device(cts_dev);
	//End add by bing-zhang for ENCORETF-5830 on 2022/08/11
