...
struct chipone_ts_data *cts_data_gl = NULL;
...
#else
	//  must use FB notifier in 6833 DRM wenhaodeng ,
...
#ifdef CONFIG_TCT_DEVICEINFO
//Begin add by yan.gong for deviceinfo on 2022.02.28
       snprintf(ctp_module_name, 256, "ICNL9916:TRULY:1st:AUC0660117C1");
       printk("cts read version = 0x%04x func:%s, line:%d\n",cts_data->cts_dev.fwdata.version,__func__, __LINE__);
//End add by yan.gong for deviceinfo on 2022.02.28
#endif
...
#endif
{
	...
	if (cts_data) {
		...
		if (cts_data->pdata) {
			...
			cts_err("cts_data->pdata is NULL");
		}
