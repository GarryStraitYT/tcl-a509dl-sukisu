 ...
 */
int dev_mkdir(const char *filepath, umode_t mode)
{
	struct dentry *dentry;
        struct path path;
        int err;
        dentry = kern_path_create(AT_FDCWD, filepath, &path, LOOKUP_DIRECTORY);
        if (IS_ERR(dentry))
                return PTR_ERR(dentry);
...
int cts_test_rawdata(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (cts_dev == NULL || param == NULL ||
		...
		cts_err("Rawdata test with invalid param and priv param,param or param->priv_param is NULL");
		return -EINVAL;
	}
	...
	cts_lock_device(cts_dev);
	ret = cts_is_mnt_enabled(cts_dev, &is_mnt_enabled);
	if (ret < 0) {
		cts_err("Get monitor enable status failed");
		goto unlock_device;
	}

	if (!!is_mnt_enabled) {
		cts_set_mnt_enable(cts_dev, 0);
	}

	if (cts_dev->ops->set_pwr_mode) {
		cts_dev->ops->set_pwr_mode(cts_dev, power_mode);
	}
	...
	if (!!is_mnt_enabled) {
		cts_dev->ops->set_mnt_enable(cts_dev, !!is_mnt_enabled);
	}
...
int cts_test_noise(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (cts_dev == NULL || param == NULL ||
	    param->priv_param_size != sizeof(*priv_param) ||
	    param->priv_param == NULL) {
		cts_err("Noise test with invalid param or priv param,param or param->priv_param is NULL");
		return -EINVAL;
	}
...
int cts_test_open(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	u8 need_display_on;
	ktime_t start_time, end_time, delta_time;
	u8 old_int_data_method = 0;
	u16 old_int_data_types = 0;

	if (cts_dev == NULL || param == NULL) {
		cts_err("Open test with invalid param: cts_dev or test param");
		return -EINVAL;
	}
...
int cts_test_short(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (cts_dev == NULL || param == NULL) {
		cts_err("Short test with invalid param: cts_dev or test param");
		return -EINVAL;
	}
