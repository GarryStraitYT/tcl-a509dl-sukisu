...
static void deviceinfo_module_name_copy(void){
	...
	if(fts_data->ic_info.ids.chip_idh == 0x87 && fts_data->ic_info.ids.chip_idl == 0x56){
		sprintf(ctp_module_name, "FT8756:%s:0x%x:AUC0650123C1", "TDT", ver);
	} else {
		sprintf(ctp_module_name, "Focaltech:NA:NA:NA");
	}
