        ...
        { 
            ...
            if(ppt->rp.id != 0x0f)
            {
                ...
                else
                {
                    ...
                    if(pointed)
                    {
                        input_report_abs(st_dev.input, ABS_MT_POSITION_X,SEMI_TOUCH_SOLUTION_X-((unsigned int)(ppt->rp.x_h4 << 8) | ppt->rp.x_l8));
                        input_report_abs(st_dev.input, ABS_MT_POSITION_Y,SEMI_TOUCH_SOLUTION_Y-((unsigned int)(ppt->rp.y_h4 << 8) | ppt->rp.y_l8));
                        input_report_abs(st_dev.input, ABS_MT_TOUCH_MAJOR, ppt->rp.z);
                        input_report_abs(st_dev.input, ABS_MT_WIDTH_MAJOR, ppt->rp.z);
                        input_report_key(st_dev.input, BTN_TOUCH, 1);
                    }
