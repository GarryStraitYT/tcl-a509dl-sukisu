...
void cts_add_drv_proximity(void);
...
int  cts_get_prox_en(void)
{
	  return  prox_active;
}


void  cts_set_prox_en2(int en)
{
	   prox_active=en;
}
void cts_set_prox_en(unsigned int enable)
{
	...
	ret = cts_tcs_spi_write(&chipone_ts_data->cts_dev, TP_STD_CMD_PARA_PROXI_EN_RW,&prox_active, sizeof(prox_active));
...
#if 0 //def CONFIG_CTS_TP_PROXIMITY
		if((open_near == 1) || (touch_info->vkey_state & 0x80)) {
  ...
  {  
		...
		if (touch_info->vkey_state & 0x80) {
			...
			if (irq_number == 2) {
				//irq_number = 0;
                                value1[0] = 0;
				ps_report_interrupt_data(value1);  //ps_report_interrupt_data(prox_cur_status);
                                cts_info("[ TP_PS ] proximity status = near");
				return 0;
			}
		} else if (((touch_info->vkey_state & 0x80) == 0) && open_near == 1) {
			prox_cur_status = 2;
			...
			irq_number = 0;
                        value1[0] = 1;
		    ps_report_interrupt_data(value1);	//ps_report_interrupt_data(prox_cur_status);
                        cts_info("[ TP_PS ] proximity status = far");
			return 0;
		}else{
			prox_cur_status = 2;
			open_near = 0 ;
