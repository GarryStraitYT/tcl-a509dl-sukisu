...
static int tp_ps_work_skip_ctp_poweroff_flag=0; 
#endif
//guozhiliang add TP lcd sleep in PROXIMITY mode 


static int ctp_is_suspend_flag=0; 
int cts_is_suspend(void)
{
	return ctp_is_suspend_flag;
}
...
int cts_suspend(struct chipone_ts_data *cts_data)
{
  ...
  //guozhiliang add TP lcd sleep in PROXIMITY mode 
#if defined(CONFIG_TP_PROXIMITY_SLEEP_CONTROLL_IN_BSP)  
    tp_ps_work_skip_ctp_poweroff_flag=cts_get_prox_en();	
	if(tp_ps_work_skip_ctp_poweroff_flag)
	{
	    cts_info("Suspend skip1");
		return 0;
     }
 #endif
//guozhiliang add TP lcd sleep in PROXIMITY mode 

	 ctp_is_suspend_flag=1;
#endif
...
//guozhiliang add TP lcd sleep in PROXIMITY mode 
	...
	if(tp_ps_work_skip_ctp_poweroff_flag)
	{
	    cts_info("Resume skip1");
		return 0;
     }
#endif
//guozhiliang add TP lcd sleep in PROXIMITY mode 
	 ctp_is_suspend_flag=0;
#endif
	...
	//add for resume fail retry
		if (ret) 
		{
	      int i;
			  for(i=0;i<5;i++)
		    { 
		    	cts_warn("Resume device retry =%d", i);		
		    	msleep(500);
					cts_lock_device(&cts_data->cts_dev);
					ret = cts_resume_device(&cts_data->cts_dev);
					cts_unlock_device(&cts_data->cts_dev);	
...
//Begin added by wenhaodeng for task CIVICS-3392 on 20230105
//[FAQ23179] reference
...
#endif
//End added by wenhaodeng for task CIVICS-3392 on 20230105
...
#endif
	cts_info("cts_driver_probe done !!");
//End added by wenhaodeng for task CIVICS-3392 on 20230105
