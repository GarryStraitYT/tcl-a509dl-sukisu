...
//begin modified by xiongbo.huang compile pass on2020/0310
//#include "mt_boot_common.h"
#include "mtk_boot_common.h"
//end modified by xiongbo.huang compile pass on2020/0310
...
DEFINE_MUTEX(gtp_i2c_access);
//begin modified by mingbo.feng fix tp can not use sometimes on2020/0728
static atomic_t  irq_enabled = ATOMIC_INIT(1);
//end modified by mingbo.feng fix tp can not use sometimes on2020/0728
...
//begin end by xiongbo.huang compile pass on2020/0310
...
u8 driver_num = 0;
u8 sensor_num = 0;
//begin modified by xiongbo.huang on 2020/0310
u16 gtp_ref_retries = 0;//this value is sometimes possible larger than 255,so use u16
//end modified by xiongbo.huang on 2020/0310
...
#ifdef CONFIG_OF
static const struct of_device_id gt9xx_dt_match[] = { {.compatible = "mediatek,goodix_touch_mtk"}, {},};
#endif
...
//begin add modify by jiaxin.pi for task on 11425063
reset_proc:
    gtp_gpio_output(GTP_IRQ_GPIO, 0);
    gtp_gpio_output(GTP_RST_GPIO, 0);
    msleep(20);
//end add modify by jiaxin.pi for task on 11425063
    gtp_reset_guitar(client, 20);
...
//begin delete by xiongbo.huang to reduce init time on 20200310
...
//end delete by xiongbo.huang to reduce init time on 20200310
...
//begin modfied by xiongbo.huang to reduce tp_driver_init time on 20200310
        //if (gtp_ref_retries >= GTP_CHK_FS_MNT_MAX) {
        if (GTP_CHK_FS_MNT_MAX  >= 0) {//i don't want it to try so many times,just let it fail.
//end modfied by xiongbo.huang to reduce tp_driver_init time on 20200310
        ...
        /* get ref file data */
        //begin modified by xiongbo.huang tp meet cts requirement
        //flp = filp_open(GTP_BAK_REF_PATH, O_RDWR | O_CREAT, 0666);
        flp = filp_open(GTP_BAK_REF_PATH, O_RDWR | O_CREAT, 0644);
        //end modified by xiongbo.huang tp meet cts requirement
        if (IS_ERR(flp)) {
...
static void gtp_recovery_reset(struct i2c_client *client)
{
//begin delete by xiongbo.huang for apollo on 20200311
    //mutex_lock(&gtp_i2c_access);//gtp_recovery_reset is called by gtp_i2c_read,which is easy to cause deadlock
//end delete by xiongbo.huang for apollo on 20200311
    if (tpd_halt == 0) {
#ifdef CONFIG_GTP_ESD_PROTECT
...
//begin delete by xiongbo.huang for apollo on 20200311
    //mutex_unlock(&gtp_i2c_access);//gtp_recovery_reset is called by gtp_i2c_read,which is easy to cause deadlock
//end delete by xiongbo.huang for apollo on 20200311
}
...
//begin add by xiongbo.haung for apollo on 2020/0312
#if 0
        if (IS_ERR(flp)) {
            flp = filp_open(GTP_MAIN_CLK_PATH, O_RDWR | O_CREAT, 0666);
        } else {
...
#endif
//end add by xiongbo.haung for apollo on 2020/0312
send_main_clk:
...
//begin add by xiongbo.huang for gt912 device info on 2020/0310
//#if defined (CONFIG_TCT_DEVICEINFO)
extern char ctp_module_name1[256];
void get_ctp_device_infomation(void){
    u8 buf[16] = {0};
    int ret = 0;
    ...
    ret = gtp_i2c_read(i2c_client_point, buf, 3);
//begin modified by xiongbo.huang for apollo tmo device info task 9496253 on 20200605 info from ping.chen
        if(buf[2] == config[2]){
                snprintf(ctp_module_name1, 256, "GT912:DPT:0x%x:AUE1UM0A10C1",buf[2]);
        }else{
                GTP_ERROR("read fw ver from tp fail\n");
                snprintf(ctp_module_name1, 256, "GT912:DPT:0xFF");
        }
//begin modified by xiongbo.huang for apollo tmo device info task 9496253 on 20200605 info from ping.chen
    return;
}
//#endif
//end add by xiongbo.huang for gt912 device info on 2020/0310
            ...
            if (ret < 0) {
                ...
                if ((esd_buf[0] == 0xAA) || (esd_buf[1] != 0xAA)) {
                    ...
                    if ((chk_buf[0] == 0xAA) || (chk_buf[1] != 0xAA)) {
                        //begin modfied by xiongbo.huang for gt912
                        GTP_ERROR("[Esd_Check] ic register value is error\n");
                        //end modfied by xiongbo.huang for gt912
...
#endif
//begin add by xiongbo.huang to spend less time calculating clk
#ifdef CONFIG_GTP_COMPATIBLE_MODE
void setup_clk_by_log(void)//once 2nd comes do compatible here
{
        int i=0;
        for (;i < 5;i ++){
                gtp_clk_buf[i] = 51;//get these value from log
        }
        gtp_clk_buf[5] = 1;//get these value from log
        return ;
}
#endif
//end add by xiongbo.huang to spend less time calculating clk
static int touch_event_handler(void *unused)
{
    ...
    do {
        ...
        if ((finger == 0x00) && (CHIP_TYPE_GT9F == gtp_chip_type)) {
            ...
            case GTP_RQST_CONFIG:
                ...
                //begin add by xiongbo.huang for device info on 2020/0310
               // #if defined (CONFIG_TCT_DEVICEINFO)
                        get_ctp_device_infomation();
               // #endif
                //end add by xiongbo.huang for device info on 2020/0310
                goto exit_work_func;
...
//end add by xiongbo.huang to spend less time calculating clk
            ...
            case GTP_RQST_RESET:
                ...
                //begin add by xiongbo.huang for apollo on 20200310
                //mutex_unlock(&gtp_i2c_access);//cancelled lock gtp_i2c_access in gtp_recovery_reset.so cancelled it here
                //end add by xiongbo.huang for apollo on 20200310
...
//begin modified by xiongbo.huang for apollo porting on 2020/0310
    if (!gpDMABuf_va){
        GTP_INFO("[Error] Allocate DMA I2C Buffer failed!\n");
    }else{//if gpDMABuf_vais null, then no more use of it
        memset(gpDMABuf_va, 0, GTP_DMA_MAX_TRANSACTION_LENGTH);
    }
//end modified by xiongbo.huang for apollo porting on 2020/0310
#endif
    tpd->reg = regulator_get(tpd->tpd_dev, "vtouch");
    retval = regulator_set_voltage(tpd->reg, 2800000, 3300000);
    /*if (retval != 0) {
        GTP_ERROR("Failed to set reg-vgp6 voltage: %d\n", retval);
        return -1;
    }*/
