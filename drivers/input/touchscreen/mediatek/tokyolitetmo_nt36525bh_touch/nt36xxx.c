 ...
 *
 * $Revision: 58919 $
 * $Date: 2020-03-23 17:43:05 +0800 (週一, 23 三月 2020) $
 *
...
#include <linux/sched.h>	//old kernel (for mtk only)
#else
#include <uapi/linux/sched/types.h>	//new kernel (for mtk only)
#endif

#if (defined CONFIG_MTK_SPI) || (defined CONFIG_SPI_MT65XX)
#include "tpd.h"
#endif
...
//begin modify by kun.zheng for task 8998289 on 2020/03/30
//uint32_t SWRST_N8_ADDR = 0; //read from dtsi
uint32_t SWRST_N8_ADDR = 0x03F0FE;
//end modify by kun.zheng for task 8998289 on 2020/03/30
...
//begin add by kun.zheng for defect 9649920 on 2020/07/20
        unsigned long flags = 0;
        spin_lock_irqsave(&ts->irq_lock, flags);
//end add by kun.zheng for defect 9649920 on 2020/07/20
...
info_retry:
	...
	NVT_LOG("fw_ver = 0x%02X, fw_type = 0x%02X\n", ts->fw_ver, buf[14]);
        ...
        //begin add by kun.zheng for task 8998289 on 2020/03/24
        //begin modify by kun.zheng for proinfo task 8998289 on 2020/04/27
        //snprintf(ctp_module_name, PAGE_SIZE, "NT36525BH:%s:0x%x", "TDT",ts->fw_ver);
        snprintf(ctp_module_name, 256, "NT36525BH_NON_FLASH:TDT"); //PAGE_SIZE
        //end modify by kun.zheng for proinfo task 8998289 on 2020/04/27
        //end add by kun.zheng for task 8998289 on 2020/03/24
...
static struct of_device_id nvt_match_table[] = {
	{ .compatible = "mediatek,cap_touch",}, //novatek,NVT-ts-spi
	{ },
};
#endif
...
extern struct LCM_DRIVER *drv_to_distingish_tp; //pbw
...
static int nvt_local_init(void)
{
	...
	NVT_LOG("start\n");
      ...
      //begin add by kun.zheng for task 8973946 on 2020/03/04
      if(drv_to_distingish_tp != NULL){
          if (!strcmp(drv_to_distingish_tp->name, "tokyolitetmo_nt36525bh_hd_dsi_vdo")) {
              ;
          }else{
              NVT_ERR("error:lcm name is not tokyolitetmo_nt36525bh_hd_dsi_vdo,this driver shoule not probe");
              return -ENODEV;
          }
      }else{
          NVT_ERR("error:drv_to_distingish_tp is null");
      }
      //end add by kun.zheng for task 8973946 on 2020/03/04
