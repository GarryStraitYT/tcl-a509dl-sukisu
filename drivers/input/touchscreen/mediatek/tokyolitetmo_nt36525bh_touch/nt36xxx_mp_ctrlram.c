...
//begin modify by kun.zheng for task 8998289 on 2020/03/31
...
//end modify by kun.zheng for task 8998289 on 2020/03/31
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	if(of_property_read_bool(np, "novatek,mp-support-dt")) {
		 ...
		 */
		snprintf(mpcriteria, 32, "novatek-mp-criteria-%04X", ts->nvt_pid); //PAGE_SIZE
...
{
        //begin modify by kun.zheng for task 8998289 on 2020/04/02
        struct proc_dir_entry *selftest_dir;
        selftest_dir = proc_mkdir("tct",NULL);
        if(!selftest_dir){
            NVT_ERR("fail to mkdir /proc/tct\n");
            return -1;
        }
	NVT_proc_selftest_entry = proc_create("nvt_selftest", 0444, selftest_dir, &nvt_selftest_fops);
        //end modify by kun.zheng for task 8998289 on 2020/04/02
