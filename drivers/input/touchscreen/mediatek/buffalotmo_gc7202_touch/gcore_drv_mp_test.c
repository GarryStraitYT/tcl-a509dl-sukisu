...
#define MP_TEST_DATA_DIR              "/mnt/sdcard/rawdata"
...
int gcore_mp_test_item_open(struct gcore_mp_data *mp_data)
{
	...
	GTP_DEBUG("fw_xfer:%d",gdev->fw_xfer);
...
//	int cols = RAWDATA_COLUMN;
    ...
    GTP_DEBUG("raw data 0 1 %x %x", rawdata[0], rawdata[1]);
	for (i = 0; i < RAW_DATA_SIZE / 2; i++) {
...
int gcore_mp_test_item_noise(struct gcore_mp_data *mp_data)
{
	...
	GTP_DEBUG("gcore mp test item nosie");
...
int gcore_start_mp_test(void)
{
	...
	GTP_DEBUG("gcore start mp test ts_stat %d", fn_data.gdev->ts_stat);
...
static int32_t gcore_rawdata_open(struct inode *inode, struct file *file)
{
	...
	if (rawdata_node_mem == NULL) {
		rawdata_node_mem = (u16 *) kzalloc(RAW_DATA_SIZE, GFP_KERNEL);
	...
	gcore_fw_read_rawdata(rawdata_mem, RAW_DATA_SIZE);
