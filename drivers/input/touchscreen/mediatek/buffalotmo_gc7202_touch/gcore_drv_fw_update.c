...
#define FW_BIN_NAME "V750_2.1.bin"
...
int gcore_fw_event_notify(enum fw_event_type event)
{
    ...
    if (!g_ret_update) {
        ...
        GTP_DEBUG("fw update is completed,wait 100ms");
        msleep(100);
    }
...
EXPORT_SYMBOL(gcore_fw_event_notify);
...
/* gcore_reset();  //reset RESX pin */
	...
	if (!wait_for_completion_interruptible_timeout(&fw_update_complete, 1 * HZ/2)) {
...
//extern void gc7202_display_esd_recovery(void);
				...
				struct device_attribute *attr, char *buf)
{
    ...
    if ((dev_mkdir("/mnt/sdcard/rawdata", S_IRUGO | S_IWUSR)) != 0){
       GTP_DEBUG("mkdir mp directory /sdcard/rawdata/ failed");
    }
