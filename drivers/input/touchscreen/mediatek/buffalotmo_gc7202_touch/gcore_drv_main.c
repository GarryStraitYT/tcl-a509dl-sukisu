...
int gcore_input_device_init(struct gcore_dev *gdev)
{
    ...
    input_set_abs_params(gdev->input_device, ABS_MT_PRESSURE,   0, 255,0,0);
...
void gcore_touch_down(struct input_dev *dev, s32 x, s32 y, u8 id)
{
	...
	input_report_abs(dev, ABS_MT_PRESSURE, 100);
        ...
        input_report_abs(dev, ABS_MT_PRESSURE, 101);
#endif
...
void gcore_gesture_event_handler(struct input_dev *dev, int id)
{
	...
	case GESTURE_PALM:
		...
		//Begin add by bing-zhang for BUTF-793 on 2022/01/18
		input_report_key(dev, KEY_GESTURE_PALMIN, 1);
		input_sync(dev);
		input_report_key(dev, KEY_GESTURE_PALMIN, 0);
		input_sync(dev);
		//End add by bing-zhang for BUTF-793 on 2022/01/18
...
//extern void primary_display_esd_recovery(void);
//extern int gc7202_need_esd_recovery(void);
#if GCORE_WDT_RECOVERY_ENABLE
void gcore_wdt_recovery_works(struct work_struct *work)
{
	...
	GTP_ERROR("WDT timeout recovery ts now,wdt_times:%d,ts_stat:%d",wdt_times,gdev->ts_stat);
	...
	if (!gdev->ts_stat) {
		if(wdt_times < 2) {
            ...
            GTP_ERROR("%s enter trigger display esd recovery",__func__);
            GTP_ERROR("%s enter trigger display esd recovery",__func__);
            GTP_ERROR("%s exit trigger display esd recovery",__func__);
...
int gcore_touch_probe(struct gcore_dev *gdev)
{
	...
	//Begin add by bing-zhang for BUTF-793 on 2022/01/18
	#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	input_set_capability(gdev->input_device, EV_KEY, KEY_GESTURE_PALMIN);
	#endif
	//End add by bing-zhang for BUTF-793 on 2022/01/18
...
#if 1//def CONFIG_TCT_PROJECT_BUFFALOTF
...
void gcore_edge_process(unsigned int enable)
{
	if(0 == enable){
		gcore_fw_event_notify(FW_EDGE_0); //portrait
	}else if(1 == enable){
	    gcore_fw_event_notify(FW_EDGE_90); //landscape
	}else {
		GTP_DEBUG("error param %d", enable);
		return;
	}
	return;
}
...
void gcore_charge_process(unsigned int enable)
{
        charge_status = enable;
	if(0 == enable){
		gcore_fw_event_notify(FW_CHARGER_UNPLUG);//plug-out
	}else if(1 == enable){
		gcore_fw_event_notify(FW_CHARGER_PLUG);//plug-in
	}else {
		GTP_DEBUG("error param %d", enable);
		return;
	}
...
#ifdef CONFIG_TCT_TOUCHSCREEN_EDGE_PROCESS
	tct_set_edge_process_en = gcore_edge_process;
#endif

#ifdef CONFIG_TCT_TOUCHSCREEN_CHARGE_PROCESS
	tct_set_charge_process_en = gcore_charge_process;
#endif
/* End modify for TP edge process */
	
#if 1 //defined(CONFIG_TCT_DEVICEINFO_TMO) || 	defined(CONFIG_TCT_PROJECT_BUFFALOTF)
	sprintf(ctp_module_name, "GC7202:%s", "TDT");
#endif
	return 0;
}
