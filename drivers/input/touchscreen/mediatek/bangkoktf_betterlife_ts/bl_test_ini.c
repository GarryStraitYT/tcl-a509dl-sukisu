...
static int get_test_threshold_mc_sc(void)
{
	...
	btl_init_buffer(thr->rawdata_test_freq, thr->basic.rawdata_test_freq, node_num, false, 0, 0);
    /* detail threshold */
...
static void print_thr_mc_sc(void)
{
	...
	BTL_TEST_DBG("rawdata_test_freq:%d", thr->basic.rawdata_test_freq);
	...
	btl_print_buffer(thr->rawdata_test_freq, tdata->node.node_num, tdata->node.rx_num);
}
