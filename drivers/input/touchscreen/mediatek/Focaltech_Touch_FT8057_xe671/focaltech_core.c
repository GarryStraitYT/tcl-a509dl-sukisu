...
static int fts_ts_probe_entry(struct fts_ts_data *ts_data)
{
    ...
    if (ret) {
#ifdef CONFIG_HELAEYE_BSP_TP_ON
        if( !strcmp(mtk_tpchip_name,"FT8057" ))
        {
        	tp_driver_lasterrcode=TP_ERR_INIT;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        }
        FTS_ERROR("hela_boot_tp not focal IC, unregister driver,tp_driver_lasterrcode=[%d %d]",
    ...
    if (ret) {
        ...
        if( !strcmp(mtk_tpchip_name,"FT8057" ))
        {
        	...
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        	FTS_ERROR("hela_boot_tp init fw upgrade fail [%d %d]",
        	tp_driver_lasterrcode,tp_driver_lasterrcode_cmd_or_errcode);
        }
#endif
...
static int __init tpd_driver_init(void)
{
    ...
    if(lcd_ic_ft8057==0)
    {
	FTS_ERROR("focaltech tpd_driver_init exit lcd_ic_ft8057=%d", lcd_ic_ft8057);
        return -1;
    }
