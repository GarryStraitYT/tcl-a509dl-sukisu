...
int bl_update_flash(unsigned char update_type, u8* pfwbin, int fwsize)
{
    ...
    int retry = 0;
    int ret = 0;
    TPD_DEBUG("yanjingqing fw_checksum=0x%x\n", fw_checksum);
    ret = bl_get_fw_checksum(&fw_checksum);
    TPD_DEBUG("yanjingqing2 ret=%d\n", ret);
    if(ret < 0) {
    ...
    if(update_type == INIT_UPDATE) { //probe .h update
        BF_TP_LOG("btl fw update start .h init,fw checksum = 0x%x,bin checksum = 0x%x___\n", fw_checksum, BIN_CHECKSUM);
    ...
    while((retry--) && (((update_type == INIT_UPDATE) && (fw_checksum != BIN_CHECKSUM))
        ...
        if(ret < 0) {
            ...
            fw_checksum = fw_checksum - 0xFF - 0xFF + fwbin1[VERTIFY_START_OFFSET] + fwbin1[VERTIFY_END_OFFSET];
        }
...
int bl_update_fw(struct i2c_client* client)
{
    ...
    BIN_CHECKSUM = BIN_CHECKSUM1;
    bl_disable_irq();
    bl_i2c_lock(1);
    
    ret = bl_update_flash(INIT_UPDATE, (u8*)fwbin1, sizeof(fwbin1));
    bl_i2c_lock(0);
    ...
    BF_TP_LOG("ret_FW=%d fwverdata fw=%d arg=%d vendorID=%d fwbin[main]=%d fwbin[argu]=%d fwbin[vendor]=%d\n", ret_FW, fwverdata[0], fwverdata[1], fwverdata[2],fwbin[BL_FWVER_MAIN_OFFSET], fwbin[BL_FWVER_ARGU_OFFSET],fwbin[BL_FACTORY_ID_OFFSET]);
    
    if(fwverdata[2]==3) return 1;
    BIN_CHECKSUM = BIN_CHECKSUM0;
    chip_cob = 1;
	if((ret_FW < 0 || fwverdata[2]!=3) 		
	...
	{	
        bl_disable_irq();
        bl_i2c_lock(1);
        ret = bl_update_flash(INIT_UPDATE, (u8*)fwbin, sizeof(fwbin)); //cob
        bl_i2c_lock(0);
        tpd_gpio_as_int(btl_tpd_int_gpio_number);
