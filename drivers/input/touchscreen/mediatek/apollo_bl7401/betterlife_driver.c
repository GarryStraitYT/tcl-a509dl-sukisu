...
static void btl_chip_reset(void);
...
static const struct of_device_id bl_dt_match[] = {
    {.compatible = "mediatek,betterlife_ts"},
    {},
};
...
//begin add modify by jiaxin.pi for task  11395623 on 2021/08/09
//static struct timer_list key_timer;
//atomic_t report_timeout;
//end add modify by jiaxin.pi for task  11395623 on 2021/08/09
//static void bl_esd_check_func(struct work_struct*);
static u32 reg_value_F9_pre = 0xFF;
static u8 esd_mode_kernel_flag = 0;
...
int btl_i2c_read(struct i2c_client* client, unsigned char* writebuf, int writelen, unsigned char* readbuf, int readlen)
{
    ...
    if(retry<0)
    {
		...
		BF_TP_LOG("%s: btl i2c read error.", readbuf);
        return -1;
    }
	return ret;
}
...
int btl_i2c_write(struct i2c_client* client, unsigned char* writebuf, int writelen)
{
	...
	if(retry<0)
	{
        BF_TP_LOG("%s: btl i2c write error.",writebuf);
		return -1;
	}
...
static ssize_t btl_show_reg(struct device* ddri, struct device_attribute* attr, char* buf)
{
    u8 data[4] = {0, 0, 0, 0};
    bl_i2c_lock(1);
    //i2c_smbus_read_i2c_block_data(btl_i2c_client, greg_addr, 2, &(data[0]));
    btl_read_reg(btl_i2c_client, greg_addr, data);
    bl_i2c_lock(0);
    //return sprintf(buf, "%x, gfw_version=%x %x\n", greg_addr,data[0], data[1]);
    return sprintf(buf, "reg:%x, value=%x\n", greg_addr, data[0]);
}

static ssize_t btl_store_reg(struct device* dev, struct device_attribute* attr, const char* buf, size_t size)
{
    char* next;
    greg_addr = (u8)simple_strtoul(buf, &next, 16);
    TPD_DEBUG("btl greg_addr=0x%x", greg_addr);
    return size;
}
...
static void bl_esd_check_func(struct work_struct* work)
{
    ...
    //if(atomic_read(&ts->is_i2c_busy) != 1)
    {
        ...
        run_num = run_num + 1;
      TPD_DEBUG("btl bl_esd_check_func ret=%d,ret_F9=%d,reg_value=%d\n", ret, ret_F9,reg_value);
...
#if 0
            ret = bl8818_write_reg(bl_ts->driver.client, BTL_ESD_PROTECT_ADDR, 1);//esd_mode_kernel_flag);
            if(ret < 0) {
                TPD_DEBUG("bl_esd_check_func esd_check_mode BTL_ESD_PROTECT_START_ADDR i2c error\n");
                //return -1;
            }
#endif
        } else {
...
#if 0
    //esd mode check start
    bl_i2c_lock(1);
    ret = btl_write_reg(i2c_client, BTL_ESD_PROTECT_ADDR, esd_mode_kernel_flag);
    bl_i2c_lock(0);
    if(ret < 0) {
        TPD_DEBUG("bl_esd_check_func esd_check_mode BTL_ESD_PROTECT_START_ADDR i2c error\n");
        //return -1;
...
static void get_ctp_device_infomation(int retval){
   
        if(retval>=TPD_OK){
                snprintf(ctp_module_name1, 256, "BL7401:DPT:0x7401");
        }else{
     //           GTP_ERROR("read fw ver from tp fail\n");
                snprintf(ctp_module_name1, 256, "BL7401:DPT:0xFF");
        }
...
#ifdef CONFIG_MTK_I2C_EXTENSION
        btl_dma_alloct();11111111111111111111111111111
#endif//CONFIG_MTK_I2C_EXTENSION
...
#if(TS_CHIP == BL7XXX)
            BF_TP_LOG("BL7XXX check chipid  error :chip_id = 0x%02x", chip_id);
#elif(TS_CHIP == BL6XX1)
            BF_TP_LOG("BL6xx1 check chipid  error :chip_id = 0x%02x", chip_id);
#elif(TS_CHIP == BL6XX0)
            BF_TP_LOG("BL6xx0 check chipid  error :chip_id = 0x%02x", chip_id);
#else 
	    BF_TP_LOG("yan BL6xx0 check chipid  error :chip_id = 0x%02x", chip_id);
#endif
        } else {
#if(TS_CHIP == BL7XXX)
            BF_TP_LOG("BL7xxx chip_id = 0x%02x", chip_id);
#elif(TS_CHIP == BL6XX1)
            BF_TP_LOG("BL6xx1 chip_id = 0x%02x", chip_id);
#elif(TS_CHIP == BL6XX0)
            BF_TP_LOG("BL6xx0 chip_id = 0x%02x", chip_id);
#else
	    BF_TP_LOG("yan2 BL6xx0 check chipid  error :chip_id = 0x%02x", chip_id);
#endif
        }
        btl_chip_reset();
        msleep(5);//50
...
//add by xucs end
    ...
    get_ctp_device_infomation(retval);
#ifdef TIMER_DEBUG
    init_test_timer();
#endif
...
static void tpd_suspend(struct device* h)
{
...
#if 0
    bl_i2c_lock(1);
    ret = bl8818_write_reg(bl_ts->driver.client, BTL_ESD_PROTECT_ADDR, 0);//esd_mode_kernel_flag);
    bl_i2c_lock(0);
    ...
    bl_esd_check_func(NULL);  
    bl_disable_irq();

    bl_i2c_lock(1);
    BF_TP_LOG("lqw tct  22222222222 TPD enter sleep\n");
    btl_write_reg(btl_i2c_client, LPM_REG, data);  /* TP enter sleep mode */
    //printk("tct_i2c_result = %d",tct);
    BF_TP_LOG("lqw tct  3333333333 TPD enter sleep\n");
    bl_i2c_lock(0);
   //add begin by jiaxin.pi for task AGBR-3067 on 20211208
//    tpd_gpio_output(btl_tpd_rst_gpio_number, 0);
 //   msleep(5);
   //add end by jiaxin.pi for task AGBR-3067 on 20211208
