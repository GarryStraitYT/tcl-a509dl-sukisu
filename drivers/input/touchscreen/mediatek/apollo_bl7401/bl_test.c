...
//	u8 cmd_len = 0;
    ...
    cmd_get_adcdata[0] = (u8 *)FACTORY_REG_SHORT_ADDR;
...
int btl_get_panel_diff(int *data, int *voltage)
{
    int ret = 0;
    int i = 0;
	u8 drive_voltage = 0;
    ...
    ret = get_drive_voltage(FACTORY_REG_DRIVE_VOLTAGE,&drive_voltage);
    if(ret < 0) {
		goto get_drive_voltage_err;
    }
    ...
    /* set drive voltage */
	ret = set_drive_voltage(FACTORY_REG_DRIVE_VOLTAGE, voltage[0]);
	if(ret < 0) {
		goto get_rawdata_err;
	}
	...
	/* computer diff */
    for(i = 0; i < (byte_num/2); i++) {
        data[i] = frame_rawdata0[i] - frame_rawdata1[i];
    }	

restore_reg_err:
    ret = set_drive_voltage(FACTORY_REG_DRIVE_VOLTAGE, drive_voltage);
	if(ret < 0) {
		BTL_TEST_SAVE_ERR("restore drive voltege reg fail,ret=%d\n", ret);
	}
get_rawdata_err:
get_drive_voltage_err:
    btl_free(frame_rawdata1);
frame_rawdata1_err:
    btl_free(frame_rawdata0);
frame_rawdata0_err:
...
#define FTS_DATA_FILE_PATH "/sdcard/rawdata/"
...
static int btl_test_save_test_data(char *file_name, char *data_buf, int len)
{
    ...
    if(rawdata_test_flag){
        snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%ld%s",
            BTL_INI_FILE_PATH, TCT_BTL_RAWDATA_RESULT,"PASS_",rawdata_begin_time.tv_sec,file_name);
    }else{
        snprintf(filepath, FILE_NAME_LENGTH, "%s%s%s%ld%s",
            BTL_INI_FILE_PATH, TCT_BTL_RAWDATA_RESULT,"FAIL_",rawdata_begin_time.tv_sec,file_name);
    }
