...
#ifdef CFG_TCL_PALM
    __set_bit(TCL_PALM_IN, pdata->ts_input_dev->keybit);
#endif
...
#ifdef CFG_TCL_PALM
void cts_tcl_palm_report(struct cts_platform_data *pdata)
{
	input_report_key(pdata->ts_input_dev, TCL_PALM_IN, 1);
	input_sync(pdata->ts_input_dev);
	msleep(100);
	input_report_key(pdata->ts_input_dev, TCL_PALM_IN, 0);
	input_sync(pdata->ts_input_dev);
}
#endif
...
int chipone_gesture_wakeup_flag = 0;
#ifdef CFG_CTS_GESTURE
extern void (*tct_set_double_wakeup_en)(unsigned int enable);
void cts_double_wakeup_process(unsigned int enable)
{
	struct cts_device *cts_dev = &chipone_ts_data->cts_dev;
	cts_info("enable=%d\n", enable);
	if (enable) {
		cts_info("enable gesture wakeup");
		chipone_gesture_wakeup_flag = 1;
		cts_enable_gesture_wakeup(cts_dev);
	} else {
		cts_info("disable gesture wakeup");
		chipone_gesture_wakeup_flag = 0;		
...
int cts_plat_init_gesture(struct cts_platform_data *pdata)
{
	...
	tct_set_double_wakeup_en = cts_double_wakeup_process;
	...
	for (i = 0; i < pdata->gesture_num; i++) {
		...
		__set_bit(pdata->gesture_keymap[i][1], pdata->ts_input_dev->keybit);
	}
