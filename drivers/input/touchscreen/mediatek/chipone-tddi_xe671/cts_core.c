...
// add by jinggao.zhou add Model3 Hera TP driver 2023-02-07 
#ifdef CONFIG_HELAEYE_BSP_TP_ON
#include <tcl/tkperf.h>
extern char mtk_tpchip_name[32];
extern int tp_driver_lasterrcode;
extern int tp_driver_lasterrcode_cmd_or_errcode;
...
extern void sr_add_touch_ic_name(char *name);
extern void sr_add_touch_vendor_name(char *name);
extern unsigned int g_touch_firmware;
#endif
#ifdef CONFIG_TCT_DEVICEINFO
extern int lcd_ic_9916;
extern unsigned char ctp_module_name[256];
#endif
...
#ifdef CONFIG_SR_HARDWARE_INFOMATION
	g_touch_firmware = fwdata->version;
	sr_add_touch_ic_name("NL9916AC");
	sr_add_touch_vendor_name("DZX");
#endif
#ifdef CONFIG_TCT_DEVICEINFO
	if(lcd_ic_9916 == 1) {
		snprintf(ctp_module_name, 256, "ICNL9916:INX:0x%04x:178263620",fwdata->version);
	} else if(lcd_ic_9916 == 2) {
		snprintf(ctp_module_name, 256, "ICNL9916:INX1:0x%04x:178263620",fwdata->version);
	}
#endif
...
#ifdef CFG_TCL_PALM
	if (CFG_CTS_PALM_ID == touch_info->vkey_state) {
	    cts_tcl_palm_report(cts_dev->pdata);
	    cts_plat_release_all_touch(cts_dev->pdata);
	    return 0;
	}
#endif
...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	if (cts_dev->rtdata.landscape_mode > 0) {
		cts_info("Set landscape mode");
		ret = cts_dev->ops->set_panel_direction(cts_dev,(cts_dev->rtdata.landscape_mode - 1));
		if (ret) {
	...
	if (cts_dev->rtdata.charger_enable) {
		cts_info("Set charger enable");
		//ret = cts_send_command(cts_dev, CTS_CMD_CHARGER_ATTACHED);
		ret = cts_dev->ops->set_charger_plug(cts_dev, 1);

		if (ret)
		    cts_err("Send CMD_CHARGER_ATTACHED failed %d", ret);
	}
#endif

#ifdef CFG_TCL_PALM
	if (cts_dev->rtdata.palm_enable) {
		cts_info("Set palm enable");
	}
#endif
...
void chipone_set_charge_en(unsigned int enable)
{
	...
	struct cts_device *cts_dev = &tct_data->cts_dev;
	if (enable == 1) {
		...
		cts_dev->rtdata.charger_enable = false;
                ret = cts_dev->ops->set_charger_plug(cts_dev, 0);
               cts_info("tct Set dev charger %s", cts_dev->rtdata.charger_enable ? "Enable" : "Disable");
	}
    if (ret) {
...
void chipone_grip_mode_set(unsigned int screen_mode, unsigned int level)
{
    ...
    if (screen_mode >= 1 && screen_mode<=3)
	    cts_dev->rtdata.landscape_mode = screen_mode ;
    else {
    ...
    cts_info("tct Set landscape mode = %d screen_mode = %d", (cts_dev->rtdata.landscape_mode - 1),screen_mode);
...
#ifdef CFG_TCL_LANDSCAPE_MODE
    cts_dev->rtdata.landscape_mode = 0;
#endif
#ifdef CFG_TCL_CHARGER_MODE
    cts_dev->rtdata.charger_enable = false;
#endif
#ifdef CFG_TCL_PALM
    cts_dev->rtdata.palm_enable = false;
#endif
}
...
#ifdef CONFIG_HELAEYE_BSP_TP_ON
        if( !strcmp(mtk_tpchip_name,"ICNL9916" ))
        {
        	tp_driver_lasterrcode=TP_ERR_INIT;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        }
        cts_err("hela_boot_tp Get hardware id failed,tp_driver_lasterrcode=[%d %d]",
...
static void cts_esd_protection_work(struct work_struct *work)
{
...
#endif
	{
        ...
        if( !strcmp(mtk_tpchip_name,"ICNL9916" ))
        {
        	tp_driver_lasterrcode=TKPERF_TP_ESD_TIGER;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
	...
	if (cts_data->esd_check_fail_cnt >= CFG_CTS_ESD_FAILED_CONFIRM_CNT) {
        ...
        if( !strcmp(mtk_tpchip_name,"ICNL9916" ))
        {
        	tp_driver_lasterrcode=TKPERF_TP_ESD_RECOVERY_ERR;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
					heraeye_tp_fail(tp_driver_lasterrcode,mtk_tpchip_name,
					tp_driver_lasterrcode_cmd_or_errcode);
        }
