...
out:
	...
	{
		cts_err("hela_boot_tp cts_update_firmware failed %d", ret);
		if( !strcmp(mtk_tpchip_name,"ICNL9916" ))
		{
			tp_driver_lasterrcode=TP_ERR_UPGRADE_FAIL;
			tp_driver_lasterrcode_cmd_or_errcode=ret;
		}
		cts_err("hela_boot_tp cts_update_firmware failed,tp_driver_lasterrcode_errcode=[%d %d]",
...
#ifdef CFG_TCL_PALM
	cts_dev->rtdata.palm_enable = true;
