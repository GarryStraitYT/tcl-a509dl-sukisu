...
//min.luo defect 8462957:add lcd/ctp deviceinfo 20191023
static DEVICE_ATTR(ctp,     0444, ctpinfo_show, ctpinfo_store);//S_IWUSR | 
static DEVICE_ATTR(gesture,     0444, gesture_show, gesture_store);//S_IWUSR | 

//Added by min.luo defect 7217537:add ctp deviceinfo 20181210 
extern struct device* get_deviceinfo_dev(void);

static void Create_ctpinfo_node_ForMMI(void)
{
       //Added by min.luo defect 7217537:add ctp deviceinfo 20181210 begin
       struct device * ctp;
	   ...
	   //Added by min.luo defect 7217537:add ctp deviceinfo 20181210 end
	return;
}
			...
			unsigned long event, void *data)
{
	...
	//Begin modified by liujunting for sequential requirements of insell lcd
	if ((event != FB_EARLY_EVENT_BLANK)&&(event != FB_EVENT_BLANK))
	//End modified by liujunting for sequential requirements of insell lcd 
		return 0;
		...
		if (g_tpd_drv && !tpd_suspend_flag) {
			...
			g_tpd_drv->suspend(NULL);
                ...
                else if (tpd_suspend_flag == 1)
                {
                    ...
                    if ( !tpd_suspend_flag)
                    {
                        TPD_DMESG("ctp tpd_suspend_flag is zero after msleep 200ms\n");
                        err = cancel_work_sync(&touch_resume_work);
                        if (!err)
                    ...
                    TPD_DMESG("ctp tpd_suspend_flag%d\n",tpd_suspend_flag);
                #endif
                //end add by kun.zheng for defect 9494525 on 2020/06/09
...
/*for some projects, we do not use mtk framebuffer*/
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			printk("mtk-tpd, tpd_driver_list[%d].tpd_device_name = %s", i, tpd_driver_list[i].tpd_device_name);
		}
	}
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		/* add tpd driver into list */
