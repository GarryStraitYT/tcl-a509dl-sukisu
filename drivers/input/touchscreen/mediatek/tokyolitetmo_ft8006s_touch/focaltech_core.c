...
static int tpd_local_init(void)
{
    int ret = 0;
      ...
      if(drv_to_distingish_tp != NULL){
          if (!strcmp(drv_to_distingish_tp->name, "tokyolitetmo_ft8006s_hd_dsi_vdo")) {
              ;
          }else{
              FTS_ERROR("error:lcm name is not tokyolitetmo_nt36525bh_hd_dsi_vdo,this driver shoule not probe");
              return -ENODEV;
          }
      }else{
          FTS_ERROR("error:drv_to_distingish_tp is null");
      }
