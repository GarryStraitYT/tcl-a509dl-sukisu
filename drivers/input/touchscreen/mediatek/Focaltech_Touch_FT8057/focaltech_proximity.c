...
int  fts_get_prox_en(void)
{
	  return  fts_proximity_data.mode;
}


extern int fts_is_suspend(void);
extern int ps_report_interrupt_data(int *value);
int fts_sleep_en_prox_flag=0;

void  fps_enable_ps2(void)
{
  if(fts_sleep_en_prox_flag)
  	{
		fts_sleep_en_prox_flag=0;
...
int fts_proximity_readdata(struct fts_ts_data *ts_data)
{
    ...
    } else if (regvalue == 0/* 0xE0*/) {
        /* far away */
        proximity_status = PS_FAR_AWAY;
    }
    FTS_INFO("Reg %d Value is %d",FTS_REG_FACE_DEC_MODE_STATUS ,regvalue);
    fts_read_reg(0xB6, &regvalue);
    FTS_INFO("Reg0xB6 Value is %d", regvalue);
...
#if FTS_ALSPS_SUPPORT
        value1[0] =  fts_proximity_data.detect;
        ret = ps_report_interrupt_data(value1);
#else
    ...
    if (fts_proximity_data.detect == PS_NEAR) {
	FTS_DEBUG("[PROXIMITY] p-sensor Near.Don't report points");
	return 0;
    }
