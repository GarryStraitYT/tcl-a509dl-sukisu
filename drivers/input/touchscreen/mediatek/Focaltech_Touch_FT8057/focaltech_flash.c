...
/*Added by gonglinmin@sagereal.com for CIVIC-6454 light lcd&tp begin 2023-07-12*/
extern int lcd_ic_ft8057;
/*Added by gonglinmin@sagereal.com for CIVIC-6454 light lcd&tp end 2023-07-12*/
...
static int fts_fw_write_start(const u8 *buf, u32 len, bool need_reset)
{
    ...
    FTS_INFO("fw ver: 0x%02x\n", val);
#ifdef CONFIG_SR_HARDWARE_INFOMATION
    g_touch_firmware = val;
    sr_add_touch_ic_name("FT8057");
    FTS_INFO("fw module_id :%04x\n", upg->module_id);
    if (upg->module_id == FTS_MODULE_ID) {
...
/*Added by gonglinmin@sagereal.com for CIVIC-6454 light lcd&tp begin 2023-07-12*/
    ...
    info = &module_list[i];
    FTS_INFO("we will use tp MODULE_NAME: %s", info->vendor_name);
