...
extern u16 rawdata_test_min[];
extern u16 rawdata_test_max[];
extern u16 open_rawdata_min[];
extern u16 open_rawdata_max[];
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    ret = cts_rawdata_test(cts_dev, rawdata_test_min, rawdata_test_max);
    if (ret) {
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    ret = cts_open_test(cts_dev, open_rawdata_min, open_rawdata_max);
    if (ret) {
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    ret = cts_test(&cts_data->cts_dev, CTS_TEST_CFG_FILE_PATH);
#else
    ret = cts_test(&cts_data->cts_dev, NULL);
#endif
	printk("bo_liu, [%s][%d], ret=%d\n", __func__, __LINE__, ret);
	if (ret)
		count += scnprintf(buf, 256, "FAIL!\n");
	else
		count += scnprintf(buf, 256, "PASS!\n");
	return count;
