...
*****************************************************************************/
//Begin add by dingting.meng for T-xxxx on 2021.04.01 
#include <linux/time.h>
#include <linux/namei.h>
static int rawdata_test_flag = 0;//use fail for default
static struct timeval rawdata_begin_time;
//End add by dingting.meng for T-xxxx on 2021.04.01 
...
//Begin add by dingting.menf for T-XXXX on 2021.04.08
...
//End add by dingting.menf for T-XXXX on 2021.04.08
    if (NULL == pfile) {
...
//Begin add by dingting.meng for T-xxxx on 2021.04.01 
        rawdata_test_flag = 0;
//End add by kun.zheng for task 10693673 oakland R on 2021/02/03
#if defined(TEST_SAVE_FAIL_RESULT) && TEST_SAVE_FAIL_RESULT
...
//Begin add by dingting.meng for T-XXXX on 2021.04.08
        do_gettimeofday(&rawdata_begin_time);
//End add by dingting.meng for T-XXXX on 2021.04.08
...
//begin modify by kun.zheng for task 10693673 oakland R on 2021/02/03
...
{
    char file_name[] = "Conf_MultipleTest.ini";
    char mp_data_path[FILE_NAME_LENGTH];

    ssize_t err = -1;

    snprintf(buf, PAGE_SIZE, "%s", file_name);
    snprintf(mp_data_path, FILE_NAME_LENGTH, "%s%s",FTS_INI_FILE_PATH, TCT_FTS_RAWDATA_RESULT);

    err = dev_mkdir(mp_data_path, S_IRUGO | S_IWUSR);

    if (0 != err){
        printk("Create %s Fail!err = %d\n",mp_data_path,err);
        //return err;
    }

    fts_test_store(dev,attr,file_name,((size_t)strlen(file_name)+1));
...
//end modify by kun.zheng for task 10693673 oakland R on 2021/02/03
/*  test from test.ini
