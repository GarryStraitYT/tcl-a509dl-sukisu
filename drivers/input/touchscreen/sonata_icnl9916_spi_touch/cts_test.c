...
int cts_test_rawdata(struct cts_device *cts_dev, struct cts_test_param *param)
{
	...
	if (!!is_mnt_enabled) {
		ret = cts_set_mnt_enable(cts_dev, 0);
		if (ret) {
			cts_err("set mnt enable failed %d", ret);
		}
	}

	if (cts_dev->ops->set_freq_en) {
		ret = cts_dev->ops->set_freq_en(cts_dev, 0);
		if (ret) {
			cts_err("set freq_en disable failed %d", ret);
		}
	}
