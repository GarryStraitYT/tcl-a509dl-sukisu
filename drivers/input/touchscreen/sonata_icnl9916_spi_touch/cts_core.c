...
int cts_resume_device(struct cts_device *cts_dev)
{
	...
	while (--retries >= 0) {
#ifdef CFG_CTS_HAS_RESET_PIN
		ret = cts_hw_reg_writesb_retry(cts_dev, 0x300F8, tdata, sizeof(tdata), 3, 1);
		if (ret != 0)
...
/*
        ret = cts_send_command(cts_dev, CTS_CMD_PALM_ATTACHED);
        if (ret)
...
int cts_is_mnt_enabled(struct cts_device *cts_dev, u8 *enabled)
{
	if (cts_dev->ops->is_mnt_enabled) {
		return cts_dev->ops->is_mnt_enabled(cts_dev, enabled);
	}
	return -1;
}

int cts_set_mnt_enable(struct cts_device *cts_dev, u8 enabled)
{
	if (cts_dev->ops->set_mnt_enable) {
		cts_dev->ops->set_mnt_enable(cts_dev, enabled ? 1 : 0);
	}
	return -1;
}
