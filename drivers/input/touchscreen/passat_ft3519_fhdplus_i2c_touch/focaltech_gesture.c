...
#define KEY_DOUBLECLICK_WAKEUP 	 	         	0xf9
...
#ifdef TCT_FEATURE_TOUCH_DOUBLEWAKEUP
extern void (*tct_set_double_wakeup_en)(unsigned int enable);

void fts_double_wakeup_process(unsigned int enable)
{
    ...
    if (0 == enable) {
        printk("fts_ts disable gesture");
        ts_data->gesture_mode = 0;
    } else if (1 == enable) {
        printk("fts_ts enable gesture");
...
void fts_double_wakeup_process(unsigned int enable)
{
 ...
 mutex_lock(&ts_data->input_dev->mutex);
	if (0 == enable) {
		printk("yjq disable");
		writebuf[0] = 0xD0;
		writebuf[1] = 0; //disable gesture
	} else if (1 == enable) {
		printk("yjq enable");
		writebuf[0] = 0xD0;
		writebuf[1] = 1; //enable gesture
	} else {
...
static void fts_gesture_report(struct input_dev *input_dev, int gesture_id)
{
    ...
    case GESTURE_DOUBLECLICK:
        gesture = KEY_DOUBLECLICK_WAKEUP;
        break;
    case GESTURE_O:
...
int fts_gesture_init(struct fts_ts_data *ts_data)
{
    ...
    FTS_FUNC_ENTER();
    input_set_capability(input_dev, EV_KEY, KEY_DOUBLECLICK_WAKEUP);
   ...
   // input_set_capability(input_dev, EV_KEY, KEY_GESTURE_PALMOUT);
    ...
    #ifdef TCT_FEATURE_TOUCH_DOUBLEWAKEUP	
    tct_set_double_wakeup_en = fts_double_wakeup_process;
    #endif
    __set_bit(KEY_DOUBLECLICK_WAKEUP, input_dev->keybit);
