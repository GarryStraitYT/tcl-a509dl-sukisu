...
#endif
// add by jinggao.zhou add new tp driver directory OLQ-27 2022-09-27
...
*/
    ...
    if (ret) {
#ifdef CONFIG_HELAEYE_BSP_TP_ON
        if( !strcmp(mtk_tpchip_name,"FT3519" ))
        {
        	tp_driver_lasterrcode=TP_ERR_INIT;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        }
        FTS_ERROR("not focal IC, unregister driver,tp_driver_lasterrcode=[%d %d]",
    ...
    if (ret) {
        ...
        if( !strcmp(mtk_tpchip_name,"FT3519" ))
        {
        	tp_driver_lasterrcode=TP_ERR_UPGRADE_FAIL;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        	FTS_ERROR("init fw upgrade fail [%d %d]",
        	tp_driver_lasterrcode,tp_driver_lasterrcode_cmd_or_errcode);
        }
#endif
...
static int __init fts_ts_init(void)
{
    ...
    if ( ret != 0 ) {
        ...
        if( !strcmp(mtk_tpchip_name,"FT3519" ))
        {
        	tp_driver_lasterrcode=TP_ERR_I2CREG_FAIL;
        	tp_driver_lasterrcode_cmd_or_errcode=ret;
        }
        FTS_ERROR("Focaltech touch screen driver init failed! [%d %d ]",
        tp_driver_lasterrcode,tp_driver_lasterrcode_cmd_or_errcode);
