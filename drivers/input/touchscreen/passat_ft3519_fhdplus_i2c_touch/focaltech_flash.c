    ...
    u32 delay)
{
    ...
    for (i = 0; i < packet_number; i++) {
        ...
        for (j = 0; j < FTS_RETRIES_WRITE; j++) {
            ...
            ret = fts_read(&cmd , 1, val, FTS_CMD_FLASH_STATUS_LEN);
            if (ret < 0) {
      			  FTS_ERROR( "fts_read FTS_CMD_FLASH_STATUS fail");
        		  //return ret;
            }
            read_status = (((u16)val[0]) << 8) + val[1];
...
/*Begin add by dingting.meng for T-11008382 on 2021.04.07*/
...
static void deviceinfo_module_name_copy(void){
    ...
    ret = fts_read_reg(0xA6, &ver);
    if(ret < 0){
        FTS_ERROR("read firmware version failed,set version to 0!\n");
        ver = 0;
    }

    sprintf(ctp_module_name,"FT3519:%s:0x%x:AUC0670114C1","TDT",ver);
    return;
}
#endif
/*End add by dingting.meng for T-11008382 on 2021.04.07*/
