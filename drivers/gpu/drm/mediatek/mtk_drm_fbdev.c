...
// add by jinggao.zhou to resume framebuffer  ENCOREVZW-7282
#ifdef CONFIG_TCL_BOOT_DETECT
extern int mt_leds_max_brightness_set(char *name, int percent, bool enable);
extern void mtk_drm_crtc_atomic_resume(struct drm_crtc *crtc,struct drm_crtc_state *old_crtc_state);
extern int mt_leds_brightness_set(char *name, int level);
extern void mtk_dsi_encoder_enable(struct drm_encoder *encoder);
#endif
...
// add by jinggao.zhou to resume framebuffer  ENCOREVZW-7282
#ifdef CONFIG_TCL_BOOT_DETECT
void mtk_drm_fb_late_resume(void)
{
	int err;
	...
	struct mtk_drm_crtc *mtk_crtc;
	if (IS_ERR(debug_info)) {
		err = PTR_ERR(debug_info);
		printk("failed to allocate info, %d\n",err);
		return;
	}
	fb_helper = debug_info->par;
	drm_dev = fb_helper->dev;
	crtc = list_first_entry(&(drm_dev)->mode_config.crtc_list,
			typeof(*crtc), head);
	if (!crtc) {
		DDPPR_ERR("find crtc fail\n");
	...
	mtk_crtc = to_mtk_crtc(crtc);
	//printk("hang_detect mtk_crtc->enabled=%d \n",mtk_crtc->enabled);
	if ( mtk_crtc ) {
		...
		if ( output_comp ) {
			if( mtk_crtc->enabled )
			{
				printk("hang_detect mtk_ddp_comp_io_cmd aleady ENABLE \n");
			}
			else
			{
				//printk("hang_detect mtk_ddp_comp_io_cmd CONNECTOR_PANEL_ENABLE\n");
				mtk_drm_crtc_atomic_resume(crtc, NULL);
				mtk_ddp_comp_io_cmd(output_comp, NULL, CONNECTOR_PANEL_ENABLE, NULL);
			}
		  
			drm_fb_helper_blank(FB_BLANK_NORMAL,debug_info);
		}
		//printk("hang_detect drm_fb_helper_blank mt_leds_brightness_set\n");
		mt_leds_brightness_set("lcd-backlight", 1024);
	}
}
#endif
