...
static DEFINE_SPINLOCK(panel_ext_lock);//Begin modify by bing-zhang for ENCORECAN-603 on 2022/12/28
...
{
	//Begin modify by bing-zhang for ENCORECAN-603 on 2022/12/28
	unsigned long flags = 0;
	spin_lock_irqsave(&panel_ext_lock, flags);
	list_add_tail(&ctx->list, &panel_ext_list);
	spin_unlock_irqrestore(&panel_ext_lock, flags);
	//End modify by bing-zhang for ENCORECAN-603 on 2022/12/28
}
