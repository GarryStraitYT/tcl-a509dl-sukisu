...
#define PHYSICAL_WIDTH              136360
#define PHYSICAL_HEIGHT             217580
...
static int lcm_unprepare(struct drm_panel *panel)
{
    ...
    lcm_dcs_write_seq_static(ctx,0xB0,0x01);        //page1
    lcm_dcs_write_seq_static(ctx,0xC0,0x48);        //HC4
    lcm_dcs_write_seq_static(ctx,0xC1,0x48);        //HC4
    lcm_dcs_write_seq_static(ctx,0xC2,0x47);        //HC3
    lcm_dcs_write_seq_static(ctx,0xC3,0x47);        //HC3    
    lcm_dcs_write_seq_static(ctx,0xC4,0x46);        //H(ctx,
    lcm_dcs_write_seq_static(ctx,0xC5,0x46);        //HC2
    lcm_dcs_write_seq_static(ctx,0xC6,0x45);        //HC1
    lcm_dcs_write_seq_static(ctx,0xC7,0x45);        //HC1
    lcm_dcs_write_seq_static(ctx,0xC8,0x64);        //XDON
    lcm_dcs_write_seq_static(ctx,0xC9,0x64);        //XDON
    lcm_dcs_write_seq_static(ctx,0xCA,0x4F);        //RST GOA
    lcm_dcs_write_seq_static(ctx,0xCB,0x4F);        //RST GOA
    lcm_dcs_write_seq_static(ctx,0xCC,0x40);        //D2U
    lcm_dcs_write_seq_static(ctx,0xCD,0x40);        //D2U
    lcm_dcs_write_seq_static(ctx,0xCE,0x66);        //U2D);
    lcm_dcs_write_seq_static(ctx,0xCF,0x66);        //U2D
    lcm_dcs_write_seq_static(ctx,0xD0,0x4F);        //VEND
    lcm_dcs_write_seq_static(ctx,0xD1,0x4F);        //VEND
    lcm_dcs_write_seq_static(ctx,0xD2,0x41);        //STV
    lcm_dcs_write_seq_static(ctx,0xD3,0x41);        //STV       
    lcm_dcs_write_seq_static(ctx,0xD4,0x48);        //HC4
    lcm_dcs_write_seq_static(ctx,0xD5,0x48);        //HC4
    lcm_dcs_write_seq_static(ctx,0xD6,0x47);        //HC3
    lcm_dcs_write_seq_static(ctx,0xD7,0x47);        //HC3
    lcm_dcs_write_seq_static(ctx,0xD8,0x46);        //HC2
    lcm_dcs_write_seq_static(ctx,0xD9,0x46);        //HC2
    lcm_dcs_write_seq_static(ctx,0xDA,0x45);        //HC1
    lcm_dcs_write_seq_static(ctx,0xDB,0x45);        //HC1
    lcm_dcs_write_seq_static(ctx,0xDC,0x64);        //XDON
    lcm_dcs_write_seq_static(ctx,0xDD,0x64);        //XDON
    lcm_dcs_write_seq_static(ctx,0xDE,0x4F);        //RST GOA
    lcm_dcs_write_seq_static(ctx,0xDF,0x4F);        //RST  
    lcm_dcs_write_seq_static(ctx,0xE0,0x40);        //D2U
    lcm_dcs_write_seq_static(ctx,0xE1,0x40);        //D2U
    lcm_dcs_write_seq_static(ctx,0xE2,0x66);        //U2D
    lcm_dcs_write_seq_static(ctx,0xE3,0x66);        //U2D
    lcm_dcs_write_seq_static(ctx,0xE4,0x4F);        //VEND
    lcm_dcs_write_seq_static(ctx,0xE5,0x4F);        //VEND
    lcm_dcs_write_seq_static(ctx,0xE6,0x41);        //STV
    lcm_dcs_write_seq_static(ctx,0xE7,0x41);        //STV
...
#endif
#endif
	ctx->ldo1v8_gpio = devm_gpiod_get_index(ctx->dev,"ldo1v8", 0, GPIOD_OUT_HIGH);
    if (IS_ERR(ctx->bias_pos)) {
        dev_err(ctx->dev, "%s: cannot get ldo1v8_gpio %ld\n",
                __func__, PTR_ERR(ctx->ldo1v8_gpio));
        return PTR_ERR(ctx->ldo1v8_gpio);
    }
    gpiod_set_value(ctx->ldo1v8_gpio, 0);
    devm_gpiod_put(ctx->dev, ctx->ldo1v8_gpio);
	udelay(5000);
    ctx->ldoen_gpio = devm_gpiod_get_index(ctx->dev,"ldoen", 0, GPIOD_OUT_HIGH);
    if (IS_ERR(ctx->bias_pos)) {
        dev_err(ctx->dev, "%s: cannot get ldoen_gpio %ld\n",
                __func__, PTR_ERR(ctx->ldoen_gpio));
        return PTR_ERR(ctx->ldoen_gpio);
    }
    gpiod_set_value(ctx->ldoen_gpio, 0);
    devm_gpiod_put(ctx->dev, ctx->ldoen_gpio);
...
static int lcm_prepare(struct drm_panel *panel)
{
	...
	mtk_leds_init_power();
#else
	ctx->ldoen_gpio = devm_gpiod_get(ctx->dev, "ldoen", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->ldoen_gpio)) {
	dev_err(ctx->dev, "%s: cannot get ldoen_gpio %ld\n",__func__, PTR_ERR(ctx->reset_gpio));
	return 0;
	}

	ctx->ldo1v8_gpio = devm_gpiod_get(ctx->dev, "ldo1v8", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->ldo1v8_gpio)) {
	dev_err(ctx->dev, "%s: cannot get ldo1v8_gpio %ld\n",__func__, PTR_ERR(ctx->reset_gpio));
	return 0;
	}

	gpiod_set_value(ctx->ldoen_gpio, 1);
	devm_gpiod_put(ctx->dev, ctx->ldoen_gpio);
	mdelay(5);

	gpiod_set_value(ctx->ldo1v8_gpio, 1);
	devm_gpiod_put(ctx->dev, ctx->ldo1v8_gpio);
	mdelay(5);
	...
	udelay(5000); //delay at least 3ms
	//LCM_LOGD("%s() bias_pos=%d \n", __func__, gpiod_get_value(ctx->bias_pos));
...
#if defined(CONFIG_BIAS_TPS65132_I2C)
	_lcm_i2c_write_bytes(0x0, 0x09); //+5.5V
	udelay(5000);
	_lcm_i2c_write_bytes(0x1, 0x07); //-5.3V
	udelay(5000);
#endif
#endif
#endif
...
//add 90hz mode
	...
	panel->connector->display_info.width_mm = 136;
	panel->connector->display_info.height_mm = 217;

	sprintf(LCM_module_name, "HX8279D:HUAXING:1200*1920");
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	dsi->mode_flags = MIPI_DSI_MODE_VIDEO|MIPI_DSI_MODE_LPM | MIPI_DSI_MODE_EOT_PACKET
...
static const struct of_device_id lcm_of_match[] = {
	{ .compatible = "odintmo,hx8279d,tdt", },
	{ }
};
...
static struct mipi_dsi_driver lcm_driver = {
	...
	.remove = lcm_remove,
	.driver = {
		.name = "panel-odintmo-hx8279d-vdo-tdt",
		.owner = THIS_MODULE,
