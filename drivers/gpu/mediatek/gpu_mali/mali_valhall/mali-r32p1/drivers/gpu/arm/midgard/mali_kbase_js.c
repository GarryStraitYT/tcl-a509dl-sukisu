		...
		ktime_t *end_timestamp)
{
	...
	struct kbase_jd_atom *x_dep = katom->x_post_dep;
// #ifdef VENDOR_EDIT
// shu5.zhang@tcl.com 2021/07/2 add for GPU frep governor
...
// shu5.zhang@tcl.com 2021/07/2 add for GPU frep governor
	...
	if (end_timestamp != NULL) {
		...
		* zero time) */
		...
		if(kbdev->mali_dev_data.callbacks->cl_boost_update_utilization)
			kbdev->mali_dev_data.callbacks->cl_boost_update_utilization(kbdev, katom, microseconds_spent);
...
#ifdef CONFIG_GPU_DEVFREQ
	// kctx->job_time += microseconds_spent;

	strcpy(katom->kctx->pid_char, katom->kctx->comm);

	if (katom->core_req & BASE_JD_REQ_ONLY_COMPUTE) {
		strcat(katom->kctx->pid_char, "_COM");
	} else if (katom->core_req & BASE_JD_REQ_FS) {
		strcat(katom->kctx->pid_char, "_FS");
	} else if (katom->core_req & BASE_JD_REQ_CS) {
		strcat(katom->kctx->pid_char, "_CS");
	}
	mali_trace_clock_set_rate(katom->kctx->pid_char, 0);
