...
// shu5.zhang@tcl.com 2021/08/23 add for GT
	...
	if (gt_thread_tgid == katom->kctx->tgid) {
		// gt_time_busy to ns
		temp = kbdev->pm.backend.metrics.values.gt_time_busy << KBASE_PM_TIME_SHIFT;
		do_div(temp, 1000);

		katom->kctx->gpu_active_time = temp;
		kbdev->pm.backend.metrics.values.gt_time_busy = 0;
	}
#endif
// #endif /* VENDOR_EDIT */
