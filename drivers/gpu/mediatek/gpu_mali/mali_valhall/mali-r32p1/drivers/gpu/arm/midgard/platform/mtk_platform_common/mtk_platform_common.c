...
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) && IS_ENABLED(CONFIG_MTK_GPU_COMMON_DVFS)
// #ifdef VENDOR_EDIT
// shu5.zhang@tcl.com 2022/07/11 add for gmc
#ifdef CONFIG_GPU_DEVFREQ
	return curr_gpufreq_id;
#else
...
#if IS_ENABLED(CONFIG_MALI_MIDGARD_DVFS) && IS_ENABLED(CONFIG_MTK_GPU_COMMON_DVFS)
#if IS_ENABLED(GED_ENABLE_DVFS_LOADING_MODE)
	ged_dvfs_cal_gpu_utilization_ex_fp = mtk_common_get_gpu_utilization_ex;
#else
