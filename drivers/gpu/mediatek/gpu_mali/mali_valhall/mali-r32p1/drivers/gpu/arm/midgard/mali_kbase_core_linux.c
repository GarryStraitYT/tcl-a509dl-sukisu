...
static ssize_t show_frame_time(struct device *dev,
		struct device_attribute *attr, char * const buf)
{
	struct kbase_device *kbdev;
	struct kbase_context *kctx = NULL;
	...
	list_for_each_entry_safe(kctx, tmp, &kbdev->kctx_list, kctx_list_link) {
		ret += scnprintf(buf+ret, PAGE_SIZE, "pid=%d, tgid=%d\n",
			kctx->pid, kctx->tgid);
		ret += scnprintf(buf+ret, PAGE_SIZE, "gpu_active_time=%d, frame_time=%d\n",
			kctx->gpu_active_time, kctx->frame_time);
	}
	mutex_unlock(&kbdev->kctx_list_lock);
	return ret;
}

static DEVICE_ATTR(frame_time, S_IRUGO, show_frame_time, NULL);
#endif
// #endif /* VENDOR_EDIT */
