...
extern int should_cancel_reclaim(struct mm_struct *mm);
				...
				unsigned long end, struct mm_walk *walk)
{
	...
	struct reclaim_param_ext *rp = walk->private;
				...
				size_t count, loff_t *ppos)
{
	...
	if (!strncmp(type_buf, "file",4)){
		...
		reclaim_walk.mm = mm;
 		reclaim_walk.pmd_entry = reclaim_pte_range_ext;

		rp.nr_to_reclaim = INT_MAX;
		rp.nr_reclaimed = 0;
		reclaim_walk.private = &rp;
		if(!strcmp(type_buf,"anon")){
			rp.rpolicy = RECLAIM_ANON_LEVEL0;
		}
		else if(type_buf[4] == '1'){
