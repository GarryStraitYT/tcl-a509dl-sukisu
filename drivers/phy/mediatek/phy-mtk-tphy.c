...
#define PHY_MODE_RG_DP_PULLDOWN_SET 7
#define PHY_MODE_RG_DP_PULLDOWN_CLR 8
#define PHY_MODE_RG_DM_PULLDOWN_SET 9
#define PHY_MODE_RG_DM_PULLDOWN_CLR 10
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
#define PROC_FILE_SRC "src"
#define PROC_FILE_SRC_HOST "src_host"
#define PROC_FILE_TERM_SEL_HOST "term_sel_host"
#define PROC_FILE_VRT_SEL_HOST "vrt_sel_host"
#define PROC_FILE_PHY_REV6_HOST "phy_rev6_host"
#define PROC_FILE_DISCTH_HOST "discth_host"
#endif
/* End add by jin.wang */
...
/* Begin mod by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
...
/* Begin add by jin.wang for task 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static int proc_src_show(struct seq_file *s, void *unused)
{
	struct mtk_phy_instance *instance = s->private;
...
static int proc_src_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_src_show, PDE_DATA(inode));
}

static ssize_t proc_src_write(struct file *file,
...
static ssize_t proc_src_host_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
...
static const struct file_operations proc_src_fops = {
	.open = proc_src_open,
	.write = proc_src_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};

static const struct file_operations proc_src_host_fops = {
	.open = proc_src_open,
	.write = proc_src_host_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static ssize_t proc_term_sel_host_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
...
static const struct file_operations proc_term_sel_host_fops = {
	.open = proc_term_sel_open,
	.write = proc_term_sel_host_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/* Begin add by jin.wang for TITAN-2200 on 2022-5-26 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static ssize_t proc_vrt_sel_host_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
...
static const struct file_operations proc_vrt_sel_host_fops = {
	.open = proc_vrt_sel_open,
	.write = proc_vrt_sel_host_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static ssize_t proc_phy_rev6_host_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
...
static const struct file_operations proc_phy_rev6_host_fops = {
	.open = proc_phy_rev6_open,
	.write = proc_phy_rev6_host_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
static ssize_t proc_discth_host_write(struct file *file,
	const char __user *ubuf, size_t count, loff_t *ppos)
{
	struct seq_file *s = file->private_data;
	struct mtk_phy_instance *instance = s->private;
...
static const struct file_operations proc_discth_host_fops = {
	.open = proc_discth_open,
	.write = proc_discth_host_write,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	file = proc_create_data(PROC_FILE_TERM_SEL_HOST, 0644,
			phy_root, &proc_term_sel_host_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_TERM_SEL_HOST);
		ret = -ENOMEM;
		goto err1;
	}
#endif
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	file = proc_create_data(PROC_FILE_VRT_SEL_HOST, 0644,
			phy_root, &proc_vrt_sel_host_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_VRT_SEL_HOST);
		ret = -ENOMEM;
		goto err1;
	}
#endif
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	file = proc_create_data(PROC_FILE_PHY_REV6_HOST, 0644,
			phy_root, &proc_phy_rev6_host_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_PHY_REV6_HOST);
		ret = -ENOMEM;
		goto err1;
	}
#endif
...
/* Begin add by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	file = proc_create_data(PROC_FILE_DISCTH_HOST, 0644,
			phy_root, &proc_discth_host_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_DISCTH_HOST);
		ret = -ENOMEM;
		goto err1;
	}
#endif
...
/* Begin add by jin.wang for task 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	file = proc_create_data(PROC_FILE_SRC, 0644,
			phy_root, &proc_src_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_SRC);
		ret = -ENOMEM;
		goto err1;
	...
	file = proc_create_data(PROC_FILE_SRC_HOST, 0644,
			phy_root, &proc_src_host_fops, instance);
	if (!file) {
		dev_info(dev, "failed to creat proc file: %s\n", PROC_FILE_SRC_HOST);
		ret = -ENOMEM;
		goto err1;
...
/* Begin mod by jin.wang for task 2064 on 2022-4-12 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	pr_err("%s: start: fmreg=0x%x, com=0x%x\n", __func__,
			(int)fmreg, (int)com);

	if (instance->eye_src != -1
		|| instance->eye_src_host != -1) {
		pr_err("%s: %d,%d, skipped\n", __func__,
				instance->eye_src, instance->eye_src_host);
		return;
	}
#else
	/* use force value */
...
#if (!defined CONFIG_MACH_MT6781) && (!defined CONFIG_MACH_MT6768)
/* Begin mod by jin.wang for TITAN-2200 on 2022-5-26 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
    if ((instance->eye_rev6 == -1) &&
		(instance->eye_rev6_host == -1)) {
		tmp = readl(com + U3P_USBPHYACR6);
		tmp &= ~PA6_RG_U2_PHY_REV6;
	...
	writel(tmp, com + U3P_USBPHYACR6);
#endif
/* End mod by jin.wang for conflict usb eye config by platform. */
...
/* Begin mod by jin.wang for TITAN-2200 on 2022-5-26 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	if ((instance->eye_disc == -1) &&
		(instance->eye_disc_host == -1)) {
		tmp = readl(com + U3P_USBPHYACR6);
		tmp &= ~PA6_RG_U2_DISCTH;
...
/* Begin del by jin.wang for task 2064 on 2021-11-30 */
#if !IS_ENABLED(CONFIG_TCT_CHARGER)
	struct device *dev = &instance->phy->dev;
#endif
/* End del by jin.wang */
...
/* Begin add by jin.wang for jira 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
		tmp |= (P2C_FORCE_IDDIG | P2C_RG_IDDIG |
				P2C_RG_BVALID | P2C_RG_AVALID |
				P2C_RG_VBUSVALID | P2C_RG_RG_IDPULLUP);
		tmp &= ~P2C_RG_SESSEND;
#else
		tmp |= P2C_FORCE_IDDIG | P2C_RG_IDDIG;
#endif
...
/* Begin mod by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
		u2_phy_props_set(tphy, instance, 0);
#else
...
/* Begin mod by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
		u2_phy_props_set(tphy, instance, 1);
#else
...
#if IS_ENABLED(CONFIG_TCT_CHARGER)
/* Begin added by bitao.xiong for task-11599163(BYD charger) on 2021-11-08 */
	case PHY_MODE_RG_DP_PULLDOWN_SET:
		tmp = readl(u2_banks->com +  U3P_U2PHYDTM0);
		tmp |= P2C_RG_DPPULLDOWN;
		writel(tmp, u2_banks->com +  U3P_U2PHYDTM0);
		break;
	case PHY_MODE_RG_DP_PULLDOWN_CLR:
		tmp = readl(u2_banks->com +  U3P_U2PHYDTM0);
		tmp &= ~P2C_RG_DPPULLDOWN;
		writel(tmp, u2_banks->com +  U3P_U2PHYDTM0);
		break;
	case PHY_MODE_RG_DM_PULLDOWN_SET:
		tmp = readl(u2_banks->com +  U3P_U2PHYDTM0);
		tmp |= P2C_RG_DMPULLDOWN;
		writel(tmp, u2_banks->com +  U3P_U2PHYDTM0);
		break;
	case PHY_MODE_RG_DM_PULLDOWN_CLR:
		tmp = readl(u2_banks->com +  U3P_U2PHYDTM0);
		tmp &= ~P2C_RG_DMPULLDOWN;
		writel(tmp, u2_banks->com +  U3P_U2PHYDTM0);
		break;
			      ...
			      struct mtk_phy_instance *instance)
{
	...
	case PHY_TYPE_USB2:
		u2_banks->misc = NULL;
/* Begin mod by jin.wang for jira 2064 on 2022-3-23 */
#if IS_ENABLED(CONFIG_MACH_MT6765) || IS_ENABLED(CONFIG_MACH_MT6761)
		u2_banks->fmreg = instance->port_base + 0xF00 - 0x800;
#else
...
/* Begin add by jin.wang for task 2064 on 2021-12-2 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	int rc = 0;
#endif
/* End add by jin.wang */
...
/* Begin mod by jin.wang for task 2064 on 2021-11-30 */
#if IS_ENABLED(CONFIG_TCT_CHARGER)
	rc = device_property_read_u32(dev, "mediatek,eye-src",
				 &instance->eye_src);
	if (rc) {
		instance->eye_src = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,eye-vrt",
				 &instance->eye_vrt);
	if (rc) {
		instance->eye_vrt = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,eye-term",
				 &instance->eye_term);
	if (rc) {
		instance->eye_term = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,eye-rev6",
				 &instance->eye_rev6);
	if (rc) {
		instance->eye_rev6 = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,eye-disc",
				 &instance->eye_disc);
	if (rc) {
		instance->eye_disc = -1;
	}

	rc = device_property_read_u32(dev, "mediatek,host-eye-src",
			 &instance->eye_src_host);
	if (rc) {
		instance->eye_src_host = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,host-eye-vrt",
			 &instance->eye_vrt_host);
	if (rc) {
		instance->eye_vrt_host = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,host-eye-term",
			 &instance->eye_term_host);
	if (rc) {
		instance->eye_term_host = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,host-eye-rev6",
			 &instance->eye_rev6_host);
	if (rc) {
		instance->eye_rev6_host = -1;
	}
	rc = device_property_read_u32(dev, "mediatek,host-eye-disc",
			 &instance->eye_disc_host);
	if (rc) {
		instance->eye_disc_host = -1;
	}

	dev_err(dev, "bc12:%d, device:%d %d %d %d %d, host:%d %d %d %d %d\n",
			     ...
			     bool is_host)
{
	...
	void __iomem *com = u2_banks->com;
	u32 tmp;
	int src = -1, vrt = -1, term = -1, rev6 = -1, disc = -1;

	if (is_host) {
		src = instance->eye_src_host;
		vrt = instance->eye_vrt_host;
		term = instance->eye_term_host;
		rev6 = instance->eye_rev6_host;
		disc = instance->eye_disc_host;
	} else {
		src = instance->eye_src;
		vrt = instance->eye_vrt;
		term = instance->eye_term;
		rev6 = instance->eye_rev6;
		disc = instance->eye_disc;
	}
	pr_err("%s: %d: bc12:%d, %d %d %d %d %d\n",
		__func__, is_host, instance->bc12_en,
		src, vrt, term, rev6, disc);
	...
	if (src >= 0 && src <= 7) {
		tmp = readl(com + U3P_USBPHYACR5);
		tmp &= ~PA5_RG_U2_HSTX_SRCTRL;
		tmp |= PA5_RG_U2_HSTX_SRCTRL_VAL(src);
		writel(tmp, com + U3P_USBPHYACR5);
	}
	...
	if (rev6 >= 0 && rev6 <= 3) {
		tmp = readl(com + U3P_USBPHYACR6);
		tmp &= ~PA6_RG_U2_PHY_REV6;
		tmp |= PA6_RG_U2_PHY_REV6_VAL(rev6);
		writel(tmp, com + U3P_USBPHYACR6);
	}
