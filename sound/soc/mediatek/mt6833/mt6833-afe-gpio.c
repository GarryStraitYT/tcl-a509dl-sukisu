...
int AudDrv_GPIO_EXTAMP_Select(int bEnable, int mode)
{
	...
	pr_info("%s()+ hly bEnable=%d, mode=%d\n", __func__, bEnable, mode);
	...
	if (bEnable == 1) {
		...
		if (aud_gpios[GPIO_HAC_HIGH].gpio_prepare) {
			for (i = 0; i < extamp_mode; i++) {
				...
				if (retval)
					pr_info("could not set aud_gpios[GPIO_HAC_LOW] pins\n");
				udelay(2);
				retval = pinctrl_select_state(
					aud_pinctrl,
					aud_gpios[GPIO_HAC_HIGH].gpioctrl);
				if (retval)
					pr_info("could not set aud_gpios[GPIO_HAC_HIGH] pins\n");
				udelay(2);
			}
		}
	} else {
