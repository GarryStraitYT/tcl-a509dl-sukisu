...
static void inet6_send_rs_vzw(struct inet6_ifaddr *ifp)
{
    ...
    pr_info("[mtk_net][IPv6][vzw][%s] dev name:%s，accept:%d, rtr_solicits: %d, dev->flag:%d\n", __func__,dev->name,ipv6_accept_ra(ifp->idev), ifp->idev->cnf.rtr_solicits, dev->flags); 
	...
	if (ipv6_accept_ra(ifp->idev) &&
	    (dev->flags & IFF_LOOPBACK) == 0) {
		pr_info("[mtk_net][IPv6][vzw][%s] send refresh rs: dev name:%s\n",
			__func__, dev->name);
