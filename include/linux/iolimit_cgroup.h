...
static inline struct iolimit_cgroup *root_iolimitcg(void)
{
	return css_iolimit(init_css_set.subsys[iolimit_cgrp_id]);
}
...
static inline void io_read_bandwidth_control(size_t count)
{
	bool is_root;
	rcu_read_lock();
	is_root = task_css_is_root(current, iolimit_cgrp_id);
	rcu_read_unlock();
	if (likely(is_root))
		return;
	else
