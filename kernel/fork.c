					...
					int node)
{
	...
	// #ifdef VENDOR_EDIT
	// guoli.kuang@arch 2020/11/05 add for ux thread
#ifdef CONFIG_TCL_UXEXPRESS
...
// xiwu1.peng@kernel 2021/09/03 add for io acct
	...
	task_io_accounting_init(&p->io_swi[0]);
	task_io_accounting_init(&p->io_swi[1]);
	p->io_index = 0;
#endif
// #endif /* VENDOR_EDIT */
...
// guoli.kuang@arch 2020/11/05 add for ux thread
#ifdef CONFIG_TCL_UXEXPRESS
	grp_leader = p->group_leader;
	if (grp_leader->benchmark_flag == 1)
		set_ux_thread(p->pid, 0);
#endif
// #endif /* VENDOR_EDIT */
