...
// #ifdef VENDOR_EDIT
// xiwu1.peng@KERNEL 2021.11.30 add for wakeup frozen process
...
unsigned int sysctl_boost_killing = 1;
#endif
// #endif /* VENDOR_EDIT */
...
// #ifdef VENDOR_EDIT
// haiqun.yan@KERNEL, 2022/10/28, add boost kill task
#if defined(CONFIG_BOOST_KILL) && defined(CONFIG_ENERGY_MODEL) && defined(CONFIG_CPU_FREQ_GOV_SCHEDUTIL) && defined(CONFIG_MTK_SCHED_EXTENSION)
				if (sysctl_boost_killing) {
					set_user_nice(t, -20);
					struct perf_order_domain *fast_domain;
					fast_domain = container_of(perf_order_domains.next, struct perf_order_domain, perf_order_domains);
					struct cpumask cpumask_big = fast_domain->possible_cpus;
					cpumask_andnot(&cpumask_big, &cpumask_big, cpu_isolated_mask);
					if (!cpumask_empty(&cpumask_big)) {
						cpumask_copy(&t->cpus_allowed, &cpumask_big);
						t->nr_cpus_allowed = cpumask_weight(&cpumask_big);
					}
				}
#endif
// #endif /* VENDOR_EDIT */
